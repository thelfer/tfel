...
namespace tfel {
  ...
  namespace material {
    ...
    static numodis::GSystem to_numodis(const VectorType& b, const VectorType& n) {
      return numodis::GSystem{to_numodis<numodis::IBurgers>(b), to_numodis<numodis::IPlane>(n)};
    }

    template <typename VectorType, typename T>
    static VectorType from_numodis(const T& v) {
      auto r = VectorType();
      raise_if(r.size() != v.getNindices(), "from_numodis: unmatched object size");
    ...
    static SlipSystemsDescription::system from_numodis(const numodis::GSystem& g) {
      ...
      if (g.getIBurgers().getNindices() == 3u) {
        return system3d{from_numodis<vec3d>(g.getIBurgers()), from_numodis<vec3d>(g.getIPlane())};
      }
      raise_if(g.getIBurgers().getNindices() != 4u,
               "from_numodis: unexecpected Burgers' vector size");
      return system4d{from_numodis<vec4d>(g.getIBurgers()), from_numodis<vec4d>(g.getIPlane())};
    }
    ...
    std::vector<SlipSystemsDescription::system> generateSlipSystems(
      ...
      raise_if(!g.is<system_type>(), "generateSlipSystems: unexpected system description");
      ...
      for (const auto& gn : gs) {
        for (const auto& gg : generateSlipSystems<cs>(gn)) {
          ...
          raise_if((std::equal(g.burgers.begin(), g.burgers.end(), b.begin())) &&
                   ...
                   "slip system " +
                       to_string(g.burgers, '(', ')') + to_string(g.plane, '[', ']') + ' ' +
                       "is generated by systems " + to_string(b, '<', '>') +
                       to_string(p, '{', '}') + " and " + to_string(gn.burgers, '<', '>') +
                       to_string(gn.plane, '{', '}'));
        }
      }
      gs.push_back({b, p});
    ...
    SlipSystemsDescription::SlipSystemsDescription(const CrystalStructure s) : cs(s) {
    ...
    SlipSystemsDescription::size_type SlipSystemsDescription::getNumberOfSlipSystemsFamilies()
        const {
    ...
    std::vector<SlipSystemsDescription::system> SlipSystemsDescription::getSlipSystems(
        const size_type i) const {
    ...
    static std::array<long double, 3u> normal(const CrystalStructure cs,
    ...
    static std::array<long double, 3u> burgers(const CrystalStructure cs,
    ...
    static SlipSystemsDescription::tensor getOrientationTensor(
        const std::array<long double, 3u>& n, const std::array<long double, 3u>& m) {
      using tensor = SlipSystemsDescription::tensor;
      // return m^n
      // XX YY ZZ
      // XY YX XZ
      // ZX YZ ZY
      return tensor{n[0] * m[0], n[1] * m[1], n[2] * m[2], n[1] * m[0], n[0] * m[1],
                    n[2] * m[0], n[0] * m[2], n[2] * m[1], n[1] * m[2]};
    ...
    std::vector<SlipSystemsDescription::vector> SlipSystemsDescription::getSlipPlaneNormals(
        const size_type i) const {
    ...
    std::vector<SlipSystemsDescription::vector> SlipSystemsDescription::getSlipDirections(
        const size_type i) const {
      const auto ss = this->getSlipSystems(i);
      auto r = std::vector<vector>(ss.size());
      for (size_type idx = 0; idx != ss.size(); ++idx) {
    ...
    std::vector<SlipSystemsDescription::tensor> SlipSystemsDescription::getOrientationTensors(
        const size_type i) const {
      const auto ss = this->getSlipSystems(i);
    ...
    static std::array<long double, 3u> getDirection(const CrystalStructure cs,
                                                    const SlipSystemsDescription::vec& d) {
      auto throw_if = [](const bool c, const std::string& m) { raise_if(c, "getDirection: " + m); };
    ...
    std::vector<long double> SlipSystemsDescription::getSchmidFactors(const vec d,
                                                                      const size_type i) const {
      const auto ss = this->getSlipSystems(i);
      ...
      for (size_type idx = 0; idx != ss.size(); ++idx) {
        ...
        r[i] = td[8] * mn[8] + td[7] * mn[7] + td[6] * mn[6] + td[5] * mn[5] + td[4] * mn[4] +
               td[3] * mn[3] + td[2] * mn[2] + td[1] * mn[1] + td[0] * mn[0];
      }
      return r;
    }  // end of SlipSystemsDescription::getSchmidFactors

    std::vector<std::vector<long double>> SlipSystemsDescription::getSchmidFactors(
        const vec d) const {
      const auto s = this->getNumberOfSlipSystemsFamilies();
    ...
    SlipSystemsDescription::size_type SlipSystemsDescription::getNumberOfSlipSystems(
        const size_type i) const {
      return this->getSlipSystems(i).size();
    }  // end of SlipSystemsDescription::getNumberOfSlipSystems

    SlipSystemsDescription::size_type SlipSystemsDescription::getNumberOfSlipSystems() const {
      auto s = size_type{};
      for (size_type i = 0; i != this->getNumberOfSlipSystemsFamilies(); ++i) {
    ...
    const std::vector<long double>& SlipSystemsDescription::getInteractionMatrix() const {
    ...
    template <CrystalStructure cs>
    static SlipSystemsDescription::InteractionMatrixStructure buildInteractionMatrix(
      ...
      for (const auto& gs1 : gs) {
        for (const auto& g : gs1) {
          raise_if(!g.template is<typename CristalStructureSlipSystems<cs>::system_type>(),
      ...
      auto ranks = std::map<int, std::vector<std::pair<numodis::GSystem, numodis::GSystem>>>{};
      ...
      for (decltype(ims.size()) i = 0; i != ims.size(); ++i) {
        ...
        for (const auto& sp1 : ranks.at(i)) {
          const auto ssi =
              SlidingSystemsInteraction{from_numodis(sp1.first), from_numodis(sp1.second)};
    ...
    SlipSystemsDescription::InteractionMatrixStructure::InteractionMatrixStructure(
    ...
    SlipSystemsDescription::size_type SlipSystemsDescription::InteractionMatrixStructure::rank()
        const {
      return this->ranks.size();
    }  // end of SlipSystemsDescription::InteractionMatrixStructure::rank()

    SlipSystemsDescription::size_type SlipSystemsDescription::InteractionMatrixStructure::getRank(
      ...
      auto equal = [throw_if](const system& ss1, const system& ss2) {
        if (ss1.is<system3d>()) {
          ...
          return ((tfel::fsalgo::equal<3u>::exe(ss1_3d.burgers.begin(), ss2_3d.burgers.begin())) &&
                  (tfel::fsalgo::equal<3u>::exe(ss1_3d.plane.begin(), ss2_3d.plane.begin())));
        }
        throw_if(!ss1.is<system3d>(), "internal error (unsupported slip system type)");
        if (!ss2.is<system4d>()) {
          return false;
        }
        const auto& ss1_4d = ss1.get<system4d>();
        const auto& ss2_4d = ss2.get<system4d>();
        return ((tfel::fsalgo::equal<4u>::exe(ss1_4d.burgers.begin(), ss2_4d.burgers.begin())) &&
                (tfel::fsalgo::equal<4u>::exe(ss1_4d.plane.begin(), ss2_4d.plane.begin())));
      };
      auto i = size_type{};
