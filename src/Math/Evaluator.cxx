...
#ifdef __CYGWIN__
static bool isascii(const int c) { return ((c >= 0) && (c < 128)); }
#endif
  ...
  namespace math {
    ...
    static std::shared_ptr<tfel::math::parser::Expr> EvaluatorTreatPower(
      ...
      throw(std::runtime_error(
          "EvaluatorTreatPower: only exponent below 16 are supported"));
    ...
    void Evaluator::treatDiff(std::vector<std::string>::const_iterator& p,
      ...
      auto throw_if = [](const bool c, const std::string& m) {
        if (c) {
          throw(std::runtime_error("Evaluator::treatDiff: " + m));
        }
      };
    ...
    void Evaluator::FunctionGeneratorManager::checkName(
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error(
              "Evaluator::FunctionGeneratorManager::checkName: " + m));
        }
      };
    ...
    Evaluator::FunctionGeneratorManager::FunctionGeneratorManager() {
      ...
      this->bFctGenerators.insert(
          {{"max", &StandardBinaryFctGenerator<Evaluator::max>},
    ...
    void Evaluator::checkCyclicDependency(
      ...
      if (this->expr == nullptr) {
        throw(
            std::runtime_error("Evaluator::checkCyclicDependency: "
      ...
      while (*p != ">") {
        ...
        for (ps = p->begin(); ps != p->end(); ++ps) {
          if (!(isalpha(*ps) || isdigit(*ps)) || (*ps == '_') || (*ps == '-')) {
            throw(
                std::runtime_error("Evaluator::analyseParameters: "
                                   "invalid parameter '" +
                                   *p + "'"));
          }
        }
        ...
        if (*p != ">") {
          if (*p == ",") {
            ++p;
            Evaluator::checkNotEndOfExpression("Evaluator::analyseParameters",
                                               p, pe);
          } else {
            throw(
                std::runtime_error("Evaluator::analyseParameters: "
      ...
      while (test) {
        ...
        if (*p == ")") {
          if (openedParenthesis == 0) {
            throw(
                std::runtime_error("Analyser::readNextGroup: "
    ...
    std::shared_ptr<Evaluator::TLogicalExpr> Evaluator::treatLogicalExpression(
      ...
      auto throw_if = [](const bool c, const std::string& m) {
        if (c) {
          throw(std::runtime_error("Evaluator::treatLogicalExpression: " + m));
        }
      };
      ...
      while (p != pe) {
        ...
        if (openedParenthesis == 0) {
          ...
          if (*p == "<") {
            ...
            prev = p - 1;
            peo = Evaluator::getFunctionGeneratorManager().extOpGenerators.find(
                *prev);
    ...
    std::shared_ptr<Evaluator::TLogicalExpr> Evaluator::treatLogicalExpression2(
      ...
      throw(
          std::runtime_error("Evaluator::treatLogicalExpression2 : "
    ...
    std::shared_ptr<Evaluator::TExpr> Evaluator::treatGroup(
      ...
      auto throw_if = [](const bool c, const std::string& m) {
        if (c) {
          throw(std::runtime_error("Evaluator::treatGroup: " + m));
        }
      };
    ...
    std::shared_ptr<Evaluator::TExpr> Evaluator::treatGroup2(
      ...
      while (test) {
        if (*p == "diff") {
        ...
        } else if ((p4 = Evaluator::getFunctionGeneratorManager()
                             .extOpGenerators.find(*p)) !=
                   Evaluator::getFunctionGeneratorManager()
                       .extOpGenerators.end()) {
          ++p;
          ...
          if (*p == "<") {
            ...
            g->add(
                std::make_shared<TExternalOperator>(p4->second, params, args));
          } else {
            Evaluator::readSpecifiedToken("Evaluator::treatGroup2", "(", p, pe);
            vector<string> params;
            auto args = this->analyseArguments(p, pe, b);
        ...
        } else if ((p3 = Evaluator::getFunctionGeneratorManager()
                             .bFctGenerators.find(*p)) !=
                   Evaluator::getFunctionGeneratorManager()
                       .bFctGenerators.end()) {
          ...
          g->add(
              std::make_shared<TBinaryFunction>(p3->second, args[0], args[1]));
        } else if ((p2 = Evaluator::getFunctionGeneratorManager()
                             .fctGenerators.find(*p)) !=
                   Evaluator::getFunctionGeneratorManager()
                       .fctGenerators.end()) {
          // call to function
          ++p;
          ...
          if ((pt != pe) && (*pt == "(")) {
            ...
            if (b) {
              ...
              if (this->positions.find(*p) == this->positions.end()) {
                ...
                g->add(std::make_shared<TExternalFunctionExpr>(*p, args,
                                                               this->manager));
      ...
      if ((!s.empty()) && (p == pe)) {
        string msg(
            "Evaluator::treatGroup2 : unterminated group (group began with '" +
            s + "').");
        throw(runtime_error(msg));
      }
      return g;
    ...
    void Evaluator::setVariableValue(const std::string& vname,
      ...
      if (p == this->positions.end()) {
        throw(
            std::runtime_error("Evaluator::setVariableValue: "
                               "variable '" +
    ...
    double Evaluator::getValue() const {
      if (this->expr == nullptr) {
        throw(
            std::runtime_error("Evaluator::getValue: "
    ...
    void Evaluator::addExternalFunctionToGroup(
      ...
      auto args = this->analyseArguments(p, pe, b);
      g->add(
          std::make_shared<TExternalFunctionExpr>(fname, args, this->manager));
    ...
    void Evaluator::setFunction(const std::vector<std::string>& vars,
      ...
      for (const auto& v : vars) {
        if (this->positions.find(v) != this->positions.end()) {
          throw(
              std::runtime_error("Evaluator::setFunction: "
                                 "variable '" +
                                 v + "' multiply defined."));
        }
    ...
    void Evaluator::setFunction(
      ...
      auto throw_if = [](const bool c, const std::string& msg) {
        if (c) {
          throw(std::runtime_error("Evaluator::treatGroup: " + msg));
        }
      };
      this->clear();
      this->manager = m;
    ...
    Evaluator::differentiate(const std::vector<double>::size_type pos) const {
      ...
      if (this->expr == nullptr) {
        throw(
            std::runtime_error("Evaluator::differentiate: "
                               "uninitialized expression."));
      }
    ...
    std::vector<double>::size_type Evaluator::getVariablePosition(
      ...
      if (p == this->positions.end()) {
        throw(
            std::runtime_error("Evaluator::getVariablePosition: "
                               "unknown variable '" +
                               name + "'"));
      }
      return p->second;
    ...
    Evaluator::createFunctionByChangingParametersIntoVariables(
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(
              std::runtime_error("Evaluator::createFunctionByChanging"
    ...
    void Evaluator::getParametersNames(std::set<std::string>& n) const {
      if (this->expr == nullptr) {
        throw(
            std::runtime_error("Evaluator::getParametersNames: "
                               "uninitialized evaluator"));
      }
