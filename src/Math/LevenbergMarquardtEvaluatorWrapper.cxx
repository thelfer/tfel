  ...
  namespace math
  {
	...
	if(b){throw(std::runtime_error("EvaluatorBase::splitAtTokenSeperator: "+m));}
      ...
      throw_if(this->ev->getNumberOfVariables()!=
	       ...
	       "the number of variables declared in the specified evaluator is not equal "
      ...
      for(vector<double>::size_type i=0;i!=this->getNumberOfParameters();++i){
	this->dev[i]=this->ev->differentiate(this->getNumberOfVariables()+i);
      }
