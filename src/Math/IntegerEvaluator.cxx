  ...
  namespace math {
    ...
    std::shared_ptr<IntegerEvaluator::TExpr> IntegerEvaluator::treatGroup(
      ...
      auto throw_if = [](const bool c, const std::string& m) {
        if (c) {
          throw(std::runtime_error("IntegerEvaluator::treatGroup: " + m));
        }
      };
      IntegerEvaluator::checkNotEndOfExpression("IntegerEvaluator::treatGroup",
                                                p, pe);
      assert(p != pe);
    ...
    void IntegerEvaluator::setVariableValue(const std::string& vname,
      ...
      if (p == this->positions.end()) {
        throw(
            std::runtime_error("IntegerEvaluator::setVariableValue: "
                               "variable '" +
    ...
    void IntegerEvaluator::setVariableValue(
        const std::vector<int>::size_type pos, const int value) {
      using namespace std;
    ...
    int IntegerEvaluator::getValue() const {
      if (this->expr == nullptr) {
        throw(
            std::runtime_error("IntegerEvaluator::getValue: "
    ...
    IntegerEvaluator::IntegerEvaluator()
        : expr() {}  // end of IntegerEvaluator::IntegerEvaluator
    ...
    void IntegerEvaluator::setFunction(const std::vector<std::string>& vars,
      ...
      for (const auto& v : vars) {
        if (this->positions.find(v) != this->positions.end()) {
          throw(
              std::runtime_error("IntegerEvaluator::setFunction: "
                                 "variable '" +
                                 v + "' multiply defined."));
        }
    ...
    std::vector<int>::size_type IntegerEvaluator::getVariablePosition(
      ...
      if (p == this->positions.end()) {
        throw(
            std::runtime_error("IntegerEvaluator::getVariablePosition: "
                               "unknown variable '" +
                               name + "'"));
      }
      return p->second;
