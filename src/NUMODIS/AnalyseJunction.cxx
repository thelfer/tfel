...
namespace numodis
{
  ...
  namespace Dupuy2017
  {
    ...
    //----------------------------------------------------------------------------------------------------------------------
    //! analyse the interaction between two glide systems and return the parameters to be used for a line tension analysis
    //----------------------------------------------------------------------------------------------------------------------
    /*!
      Considering two interacting glide systems (gsystem1 = studied system, gsystem2 = forest), pre-compute the parameters
      that will be later used within the line tension model to estimate the strenght of the junction and the corresponding
    ...
    */
      ...
      else
	{
	  ...
	  pcrystallo->CrossProduct(gsystem1.getIPlane(),
				   gsystem2.getIPlane(),
				   ijunction);
	  ...
	  if(Coincide(gsystem1.getIBurgers(),gsystem2.getIBurgers())==0)
	    {
	      GSystem gsystem3(pcrystallo->ComputeJunctionGSystem(gsystem1,gsystem2));
    ...
    //------------------------------------------------------------------------------------
    //! compute the critical/normalized stress of a junction based on line tension
    //------------------------------------------------------------------------------------
    /*!

      In this method, we assume that the normalized stress is given by the stress 
      required to unstabilize the weakest triple node of a junction. A very simple,
      yet, straightforward average is made considering all possible symmetric geometries
      of these junctions (see Dupuy et al. 2017).

      \param nu Poisson coefficient
      \param burgers Burgers vectors of the junction (b1 = glide, b2 = forest and b3 = junction)
      \param phi angles between the Burgers vectors and the junction direction (rad)
      \return normalized junction strength.
    ...
    */
    //=========================================================================
    double computeHeuristicColinearHardeningCoefficient(const Crystallo* pcrystallo,
							double nu,
    ...
    */
    ...
    {
      if(bj<0.001)
	return computeHeuristicColinearHardeningCoefficient(pcrystallo,nu,iplane1,iplane2);
      else
