...
namespace numodis
{
  ...
  */
  //===============================================================
  HCP::HCP()
    :_ratio(1.632993162),_blattice(4)
  {
  ...
  */
  //===============================================================
  std::vector<int> HCP::Symmetry(int k,
  ...
  */
  //=====================================================================
  void HCP::GenerateEquivalentIndices(const std::vector<int>& ind0,
  ...
  */
  //===============================================================
  void HCP::GenerateOrthogonalVector(const IPlane& u,
				     IDirection& v) const
  {
	...
	// norm of this vector
	norm2=v.getIndex()[0]*v.getIndex()[0]+v.getIndex()[1]*v.getIndex()[1]+v.getIndex()[2]*v.getIndex()[2]+v.getIndex()[3]*v.getIndex()[3];
  ...
  */
			    ...
			    const IPlane& iplane1) const
  {
    ...
    for(unsigned i=0; i!=4; i++)
      ipl0sorted[i]=abs(ipl0sorted[i]);
    for(unsigned i=0; i!=4; i++)
      ipl1sorted[i]=abs(ipl1sorted[i]);

    // indices permatutions are allowed for cubic crystals for the first 3 indices
  ...
  */
			      ...
			      const IBurgers& iburgers1) const
  {
    ...
    for(unsigned i=0; i!=4; i++)
      ibur0sorted[i]=abs(ibur0sorted[i]);
    for(unsigned i=0; i!=4; i++)
      ibur1sorted[i]=abs(ibur1sorted[i]);

    // indices permatutions are allowed for hexagonal crystals for the first 3 indices
