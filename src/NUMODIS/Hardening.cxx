...
namespace numodis
{
  ...
  Hardening::Hardening(const Crystallo& pcrystallo,
    ...
    :_pcrystallo(pcrystallo),
     _gsystems(gsystems),
     _coefficients(std::vector<std::vector<int> >(gsystems.size(),std::vector<int>(gsystems.size())))
  {
	...
	const auto rankInteraction = this->getRankInteraction(gsystems[i],
							      gsystems[j]);
  ...
  int Hardening::getNinteractions() const
  {
  ...
  /*!
    ...
    \return rank of the interaction if found, number of stored interactions otherwise
  ...
  */
				    ...
				    const GSystem& gsystem2) const
  {
	...
	if(g1==_interactions[i].first &&
	   g2==_interactions[i].second)
	  return i;
    }
  ...
  std::ostream& operator << (std::ostream& os,
			     const Hardening& hardening1)
  {
    using size_type = decltype(hardening1._gsystems.size());
    os << "  => number of glide systems under consideration: " << hardening1._gsystems.size() << std::endl;

    for(size_type i=0; i!=hardening1._gsystems.size(); i++)
      os << "      #" << i
	       << ": " <<  hardening1._gsystems[i] << std::endl;
    
    os << "  => number of independant coefficients: " << hardening1.getNinteractions() << std::endl;
	...
	try
	  {
	    GSystem junction(hardening1._pcrystallo.ComputeJunctionGSystem(hardening1._interactions[i].first,
