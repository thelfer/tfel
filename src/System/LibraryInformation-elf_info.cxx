...
namespace details {
  ...
  public:
    ...
    explicit elf_info(std::shared_ptr<std::ifstream>& f) noexcept
      : f_(f)
    {}
  ...
  private:
    ...
    void sections_names_raw(std::vector<char>& snames) {
      ...
      this->f_->seekg(static_cast<std::streamoff>(elf.e_shoff + elf.e_shstrndx * sizeof(section_t)));
      read_raw(section_names_section);

      snames.resize(static_cast<std::size_t>(section_names_section.sh_size));
      this->f_->seekg(static_cast<std::streamoff>(section_names_section.sh_offset));
      read_raw(snames[0], static_cast<std::size_t>(section_names_section.sh_size));
    }
	...
	if (section.sh_type == SHT_SYMTAB_) {
	  s.resize(static_cast<std::size_t>(section.sh_size / sizeof(symbol_t)));

	  const std::ifstream::pos_type pos = this->f_->tellg();
	  this->f_->seekg(static_cast<std::streamoff>(section.sh_offset));
	  read_raw(s[0], static_cast<std::size_t>(section.sh_size - (section.sh_size % sizeof(symbol_t))) );
	  this->f_->seekg(pos);
  ...
  public:
	...
	for (; index < elf.e_shnum; ++index) {
	  section_t section;
	  this->f_->seekg(static_cast<std::streamsize>(elf.e_shoff + index * sizeof(section_t)));
	  read_raw(section);
