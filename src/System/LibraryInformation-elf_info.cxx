  ...
  namespace system {
    ...
    namespace details {
      ...
      class elf_info : public tfel::system::LibraryInformation::Implementation {
        ...
        static constexpr const unsigned char STB_GLOBAL_ =
            1;                                              /* Global symbol */
        static constexpr const unsigned char STB_WEAK_ = 2; /* Weak symbol */

        /* Symbol visibility specification encoded in the st_other field.  */
        static constexpr const unsigned char STV_DEFAULT_ =
            0; /* Default symbol visibility rules */
        static constexpr const unsigned char STV_INTERNAL_ =
            1; /* Processor specific hidden class */
        static constexpr const unsigned char STV_HIDDEN_ =
            2; /* Sym unavailable in other modules */
        static constexpr const unsigned char STV_PROTECTED_ =
        ...
        void symbols_text(std::vector<symbol_t>& s, std::vector<char>& t) {
          ...
          for (std::size_t i = 0; i < elf.e_shnum; ++i) {
            ...
            if (section.sh_type == SHT_SYMTAB_) {
              ...
              read_raw(s[0], static_cast<std::size_t>(
                                 section.sh_size -
                                 (section.sh_size % sizeof(symbol_t))));
              this->f_->seekg(pos);
          ...
          std::size_t ptrs_in_section_count = 0;
          {
            ...
            for (; index < elf.e_shnum; ++index) {
              ...
              if (!std::strcmp(&names[0] + section.sh_name, section_name)) {
                ...
                ptrs_in_section_count = static_cast<std::size_t>(
