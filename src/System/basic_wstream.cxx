  ...
  namespace system {
    ...
    void BlockingStreamWriter::write(int fd,
      ...
      if (count > static_cast<size_t>(numeric_limits<ssize_t>::max())) {
        throw(SystemError(
            "BlockingStreamReader::write : number of bytes to write too high"));
      }
      while (r > 0) {
...
#pragma message("windows port")
    ...
    void NonBlockingStreamWriter::write(int fd,
      ...
      if (count > static_cast<size_t>(numeric_limits<ssize_t>::max())) {
        throw(
            SystemError("NonBlockingStreamReader::write : number of bytes to "
                        "write too high"));
      }
      rwrite = ::write(fd, buf, count);
      if (rwrite == -1) {
        systemCall::throwSystemError(
            "NonBlockingStreamWriter::write : write failed", errno);
      }
