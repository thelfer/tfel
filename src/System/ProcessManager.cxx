...
namespace tfel
{
  ...
  namespace system
  {
	...
	this->sHandlerSIGABRT = signalManager.registerHandler(SIGABRT,sigMemFun(*this,f),action);
	this->sHandlerSIGHUP  = signalManager.registerHandler(SIGHUP,sigMemFun(*this,f),action);
	this->sHandlerSIGILL  = signalManager.registerHandler(SIGILL,sigMemFun(*this,f),action);
	this->sHandlerSIGTERM = signalManager.registerHandler(SIGTERM,sigMemFun(*this,f),action);
	this->sHandlerSIGINT  = signalManager.registerHandler(SIGINT,sigMemFun(*this,f),action);
	this->sHandlerSIGQUIT = signalManager.registerHandler(SIGQUIT,sigMemFun(*this,f),action);
      } else {
    ...
    void
    ProcessManager::closeProcessFiles(const ProcessManager::ProcessId id)
    {
      using namespace std;
	...
	throw(SystemError("ProcessManager::createProcess : empty command"));
      ...
      copy(istream_iterator<string>(args),
	   istream_iterator<string>(),back_inserter(tmp));
      // creating pipes
      if(pipe(ffd)==-1){
	string msg("ProcessManager::createProcess : pipe creation failed");
	systemCall::throwSystemError(msg,errno);
	...
	*p2 = nullptr;
    ...
    ProcessManager::ProcessId
    ProcessManager::createProcess(const std::string& cmd,
				  const ProcessManager::RedirectionType flag)
    {
      using namespace std;
	...
	throw(SystemError(msg.str()));
      ...
      if(!p->isRunning){
	ostringstream msg;
	msg << "ProcessManager::sendSignal : process " << pid << " is not running";
	throw(SystemError(msg.str()));
      }
