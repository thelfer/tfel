...
namespace details {
  ...
  typedef IMAGE_OPTIONAL_HEADER_template<details::DWORD_>      IMAGE_OPTIONAL_HEADER32_;
  typedef IMAGE_OPTIONAL_HEADER_template<details::ULONGLONG_>  IMAGE_OPTIONAL_HEADER64_;
  ...
  public:
    ...
    explicit pe_info(std::shared_ptr<std::ifstream>& f) noexcept
      : f_(f)
    {}
  ...
  private:
	...
	this->f_->seekg(0);
	read_raw(dos);
	this->f_->seekg(static_cast<std::streamoff>(static_cast<std::size_t>(dos.e_lfanew) +
						    sizeof(header_t)));
      }
  ...
  public:
	...
	this->f_->seekg(static_cast<std::streamoff>(fixed_names_addr + i * sizeof(name_offset)));
	read_raw(name_offset);
	this->f_->seekg(static_cast<std::streamoff>(get_file_offset(name_offset, h)));
	...
	if(section_begin_addr == 0 || section_end_addr == 0)
      ...
      for (std::size_t i = 0;i < exported_symbols;++i) {
	// getting ordinal
	this->f_->seekg(static_cast<std::streamoff>(fixed_ordinals_addr + i * sizeof(ordinal)));
	read_raw(ordinal);

	// getting function addr
	this->f_->seekg(static_cast<std::streamoff>(fixed_functions_addr + ordinal * sizeof(ptr)));
	read_raw(ptr);
	...
	this->f_->seekg(static_cast<std::streamoff>(fixed_names_addr + i * sizeof(ptr)));
	read_raw(ptr);
    ...
    */
    /*
      ...
      IMAGE_OPTIONAL_HEADER* image_optional_header = (IMAGE_OPTIONAL_HEADER*)((details::BYTE_*)native() + image_dos_header->e_lfanew + 24);
      ...
      IMAGE_IMPORT_DESCRIPTOR* image_import_descriptor =  (IMAGE_IMPORT_DESCRIPTOR*)((details::BYTE_*)native() + image_optional_header->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);
      ...
      std::string module_name = reinterpret_cast<char*>((details::BYTE_*)native() + image_import_descriptor->Name);
