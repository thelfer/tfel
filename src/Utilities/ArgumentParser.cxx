...
namespace tfel {
  ...
  namespace utilities {
    ...
    void ArgumentParser::registerCallBack(const std::string& k,
      ...
      if (k.empty()) {
        throw(
            std::runtime_error("ArgumentParser::registerNewCallBack: "
                               "invalid key"));
      }
#ifdef _WIN32
      if (k.size() > 2) {
    ...
    void ArgumentParser::setArguments(const int argc,
      ...
      if (argc < 1) {
        throw(
            std::runtime_error("ArgumentParser::setArguments: "
      ...
      for (auto p = args.begin(); p != args.end();) {
        ...
        if (pa != alias.end()) {
          ...
          if (pf == this->callBacksContainer.end()) {
            throw(std::runtime_error("ArgumentParser::replaceAliases: '" + pn +
                                     "' is not a known argument"));
          }
          if (pf->second.hasOption) {
            const auto p2 = p + 1;
            if (p2 == args.end()) {
              throw(
                  std::runtime_error("ArgumentParser::replaceAliases : '"
      ...
      for (auto& a : this->args) {
        ...
        if (pos != std::string::npos) {
          ...
          if (pf != this->callBacksContainer.end()) {
            if (!(pf->second.hasOption)) {
              throw(
                  std::runtime_error("ArgumentParser::stripArguments: "
                                     "argument '" +
                                     an +
                                     "' does not "
                                     "have any option"));
