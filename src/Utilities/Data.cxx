...
namespace tfel{
  ...
  namespace utilities{
	...
	try{
	  ...
	  raise<std::runtime_error>("Data::read: error while "
				    "treating key '"+k+"'.\n"+
				    std::string(e.what()));
	}
	CxxTokenizer::checkNotEndOfLine("Data::parse",p,pe);
	...
	return false;
      ...
      static bool
      apply(const std::string& x1,const std::string& x2)
      {
	return x1==x2;
      } // end of apply
      static bool apply(const int x1,
			const int x2)
      {
	return x1==x2;
      } // end of apply
	...
	return x1==x2;
      ...
      static bool
      apply(const std::vector<Data>& v1,
	...
	return e;
      ...
      static bool
      apply(const std::map<std::string,Data>& v1,
    ...
    extract<std::vector<std::string>>(const Data& d){
      auto throw_if = [](const bool b,const std::string& msg){
	raise_if(b,"extract<std::vector<std::string>>: "+msg);
      };
    ...
    extract<std::map<std::string,std::string>>(const Data& d){
      auto throw_if = [](const bool b,const std::string& msg){
	raise_if(b,"extract<std::map<std::string,"
		 "std::string>>: "+msg);
      };
