  ...
  namespace utilities {
    ...
    void CxxTokenizer::openFile(const std::string& f) {
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::openFile: " + m));
        }
      };
      std::ifstream file{f};
    ...
    void CxxTokenizer::parseStream(std::istream& in,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::parseStream: " + m));
        }
      };
    ...
    void CxxTokenizer::treatChar(Token::size_type& o,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::treatCChar: " + m));
        }
      };
    ...
    void CxxTokenizer::treatString(Token::size_type& o,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::treatString: " + m));
        }
      };
    ...
    void CxxTokenizer::readNumber(Token::size_type& o,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::readNumber: " + m));
        }
      };
    ...
    void CxxTokenizer::treatCComment(Token::size_type& o,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::treatCComment: " + m));
        }
      };
      auto get_end =
    ...
    void CxxTokenizer::treatCxxComment(Token::size_type& o,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::treatCxxComment: " + m));
        }
      };
      throw_if((p == pe) || (*p != '/'), "invalid call");
      advance(o, p, 1u);
      ...
      auto throw_if = [](const bool c, const std::string& m) {
        if (c) {
          throw(std::runtime_error(
              "CxxTokenizer::treatPreprocessorDirective: " + m));
        }
      };
      // preprocessor
      throw_if((p == pe) || (*p != '#'), "invalid call");
    ...
    void CxxTokenizer::treatStandardLine(Token::size_type& o,
      ...
      auto throw_if = [](const bool c, const std::string& m) {
        if (c) {
          throw(std::runtime_error("CxxTokenizer::treatStandardLine: " + m));
        }
      };
      ignore_space(o, p, pe);
      while (p != pe) {
    ...
    void CxxTokenizer::splitLine(const std::string& line,
      ...
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("CxxTokenizer::splitLine: " + m));
        }
      };
      auto b = line.begin();
      auto p = b;
    ...
    void CxxTokenizer::checkNotEndOfLine(
    ...
    void CxxTokenizer::readSpecifiedToken(
        const std::string& method,
    ...
    std::string CxxTokenizer::readString(
        CxxTokenizer::const_iterator& p,
        const CxxTokenizer::const_iterator pe) {
      CxxTokenizer::checkNotEndOfLine("CxxTokenizer::readString", "", p, pe);
      if (p->flag != Token::String) {
        throw(
            std::runtime_error("CxxTokenizer::readString: "
    ...
    double CxxTokenizer::readDouble(CxxTokenizer::const_iterator& p,
                                    const CxxTokenizer::const_iterator pe)

    {
      ...
      if (!is && (!is.eof())) {
        throw(
            std::runtime_error("CxxTokenizer::readInt: "
    ...
    int CxxTokenizer::readInt(CxxTokenizer::const_iterator& p,
                              const CxxTokenizer::const_iterator pe)
    ...
    void CxxTokenizer::readArray(const std::string& m,
      ...
      while (p->value != "}") {
        ...
        if ((p->value == ",") || p->value == "}" || p->value == "{") {
          throw(
              std::runtime_error(m + " : unexpected token ',' or '}' or '{'"));
        }
        v.push_back(p->value);
        ++p;
        CxxTokenizer::checkNotEndOfLine(m, p, pe);
    ...
    std::vector<std::string> CxxTokenizer::readStringArray(
      ...
      while (p->value != "}") {
        ...
        if (p->value != "}") {
          ...
          if (p->value == "}") {
            throw(
                std::runtime_error("CxxTokenizer::readStringArray: "
                                   "unexpected token '}'"));
          }
        }
      }
