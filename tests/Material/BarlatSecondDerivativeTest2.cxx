...
struct BarlatSecondDerivativeTest2 final
  : public tfel::tests::TestCase
{
  ...
  {
    // stresses in "MPa" :)
    const double v1[6]  = {1.27453824166446,0.77207083708966,0.24525337568425,
			  -1.7790370858361e-4,3.613971630283e-3,-1.7873236537153e-2};
    const double v2[6] = {1.27453824166446,1.27453824166446,0.24525337568425,
			  0,0,0};
    const double v3[6] = {0.24525337568425,0.24525337568425,1.27453824166446,
			  0,0,0};
 ...
 private:
  void test1(const double* values){
    ...
    const auto l1 = makeBarlatLinearTransformation<3u,double>(-0.069888,
							     0.936408,
							     0.079143,
							     1.003060,
							     0.524741,
							     ...
							     1.069060,
							     0.954322);
    const auto l2 = makeBarlatLinearTransformation<3u,double>(-0.981171,
							     0.476741,
							     0.575316,
							     0.866827,
							     1.145010,
    ...
    for(unsigned short i=0;i!=3;++i){
      ...
      const auto d_p = computeBarlatStressSecondDerivative<stensor<3u,double>>(vp1_p,vp2,seq,8);
      const auto dPhi_dsvp1_p = d_p.dPhi_dsvp1;
      const auto dPhi_dsvp2_p = d_p.dPhi_dsvp2;
      auto vp1_m  = vp1;
      vp1_m[i]   -= ds;
      const auto d_m = computeBarlatStressSecondDerivative<stensor<3u,double>>(vp1_m,vp2,seq,8);
    ...
    for(size_type i=0;i!=9;++i){
      ...
      if(e>eps1){
	std::cout << "d2Phi_dsvp1dsvp2: " << i << " "
		  << nd2Phi_dsvp1dsvp2[i] << " "
		  << d.d2Phi_dsvp1dsvp2[i]
		  << " " << e << std::endl;
      }
      TFEL_TESTS_ASSERT(e<eps1);
    }
  ...
  void test2(const double* values){
    ...
    for(unsigned short i=0;i!=3;++i){
      ...
      vp2_p[i]   += ds;
      // seq is only a normalisation factor, not recomputed
      const auto d_p = computeBarlatStressSecondDerivative<stensor<3u,double>>(vp1,vp2_p,seq,8);
      const auto dPhi_dsvp2_p = d_p.dPhi_dsvp2;
      const auto dPhi_dsvp1_p = d_p.dPhi_dsvp1;
      auto vp2_m  = vp2;
      vp2_m[i]   -= ds;
      const auto d_m = computeBarlatStressSecondDerivative<stensor<3u,double>>(vp1,vp2_m,seq,8);
      const auto dPhi_dsvp2_m = d_m.dPhi_dsvp2;
      const auto dPhi_dsvp1_m = d_m.dPhi_dsvp1;
      if(i==0){
