...
struct LinearHomogenizationBoundsTest final : public tfel::tests::TestCase {
 ...
 private:
  ...
  void testHS_3D() {
    //we just compare a direct formula for two phases and the function for N=2 phases
    ...
    const auto pair = computeIsotropicHashinShtrikmanBounds<3u,2,real, stress>(tab_f,tab_K,tab_mu);
    ...
    const auto d=3;
    const auto K_Lbis=phi1*K1+phi2*K2-phi1*phi2*(K1-K2)*(K1-K2)/(phi1*K2+phi2*K1+2*(d-1)/real(d)*mu1);
    const auto H1=mu1*(d*K1/2+(d+1)*(d-2)*mu1/real(d))/(K1+2*mu1);
    const auto mu_Lbis=phi1*mu1+phi2*mu2-phi1*phi2*(mu1-mu2)*(mu1-mu2)/(phi1*mu2+phi2*mu1+H1);
    const auto K_Ubis=phi1*K1+phi2*K2-phi1*phi2*(K1-K2)*(K1-K2)/(phi1*K2+phi2*K1+2*(d-1)/real(d)*mu2);
    const auto H2=mu2*(d*K2/2+(d+1)*(d-2)*mu2/real(d))/(K2+2*mu2);
    const auto mu_Ubis=phi1*mu1+phi2*mu2-phi1*phi2*(mu1-mu2)*(mu1-mu2)/(phi1*mu2+phi2*mu1+H2);
 ...
 private:
  ...
  //we just compare a direct formula for two phases and the function for N=2 phases
    ...
    tab_mu[0]=young1/2/(1+nu1);
    tab_mu[1]=young2/2/(1+nu2);
    const auto pair = computeIsotropicHashinShtrikmanBounds<2u,2,real, stress>(tab_f,tab_K,tab_mu);
    const auto LB=std::get<0>(pair);
    const auto UB=std::get<1>(pair);
    const auto K_L=std::get<0>(LB);
    const auto mu_L=std::get<1>(LB);
