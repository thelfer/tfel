...
struct EshelbyBasedHomogenizationTest final : public tfel::tests::TestCase {
 ...
 private:
  ...
  void test1() {
    ...
    // '\n';
    TFEL_TESTS_ASSERT(my_abs(ESphere_DS_0 - ESphere_DS_2) < stress{eps});
    TFEL_TESTS_ASSERT(my_abs(nuSphere_DS_0 - nuSphere_DS_2) < eps);
    TFEL_TESTS_ASSERT(my_abs(ESphere_MT_1 - ESphere_MT_3) < stress{eps});
 ...
 private:
  template <typename real, typename stress, typename length>
  void test3(){{constexpr auto eps = 10 * std::numeric_limits<real>::epsilon();
  using namespace tfel::material::homogenization::elasticity;
  const auto young = stress{1e9};
  const auto nu = real{0.3};
  const auto young_i = stress{150e9};
...
const tfel::math::tvector<3u, real> n_a = {0., 0., 1.};
const tfel::math::tvector<3u, real> n_b = {1., 0., 0.};
// TransverseIsotropicDiluteScheme must be equal to OrientedDiluteScheme when
// b=c
const auto Chom2 = computeTransverseIsotropicDiluteScheme<real, stress, length>(
...
private:
template <typename real, typename stress, typename length>
void test5(){{constexpr auto eps = 10 * std::numeric_limits<real>::epsilon();
using namespace tfel::material::homogenization::elasticity;
const auto young = stress{1e9};
const auto nu = real{0.3};
const auto young_i = stress{150e9};
...
// TransverseIsotropicMoriTanakaScheme and OrientedMoriTanakaScheme when a=b=c
const auto pair0 =
    computeSphereMoriTanakaScheme<real, stress>(young, nu, f, young_i, nu_i);
...
const tfel::math::tvector<3u, real> n_a = {0., 0., 1.};
const tfel::math::tvector<3u, real> n_b = {1., 0., 0.};
// IsotropicDiluteScheme when a is near b must be near IsotropicDiluteScheme
// when a=b
...
const auto Chom0 = computeTransverseIsotropicDiluteScheme<real, stress, length>(
    young, nu, f, young_i, nu_i, n_a, a, b, b);
...
const auto Chom1 = computeTransverseIsotropicDiluteScheme<real, stress, length>(
...
const auto Chom3 = computeTransverseIsotropicDiluteScheme<real, stress, length>(
