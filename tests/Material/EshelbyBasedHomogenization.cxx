...
struct EshelbyBasedHomogenizationTest final : public tfel::tests::TestCase {
  ...
  using stress =
    ...
    const auto A = homogenization::elasticity::computeSphereLocalisationTensor<real,stress>(young,nu,young_i,nu_i);
 ...
 private:
  ...
  void test1() {
    //tests computeDiluteScheme and computeMoriTanakaScheme for spheres comparing to
    //computeSphereDiluteScheme and computeSphereMoriTanakaScheme
    ...
    const auto nuSphere_DS_0 = (3*khom0-2*muhom0)/(2*muhom0+6*khom0);
    ...
    const auto nuSphere_MT_1 = (3*khom1-2*muhom1)/(2*muhom1+6*khom1);
 ...
 private:
  ...
  void test3() {
    ...
    const tfel::math::tvector<3u,real> n_a = {0.,0.,1.};
    const tfel::math::tvector<3u,real> n_b = {1.,0.,0.};
    //computeSphereDiluteScheme must be equal to IsotropicDiluteScheme, TransverseIsotropicDiluteScheme and OrientedDiluteScheme when a=b=c
