...
struct IsotropicCazacuYieldTest final
  : public tfel::tests::TestCase
{
  ...
  tfel::tests::TestResult execute() override
  {
    ...
    const double v5[6]  = {1.27453824166446,0.77207083708966,0.24525337568425,
...
private:
  ...
  void test(const tfel::math::stensor<N,double>& t){
    ...
    auto normal = [fseq](const stensor& sig)
      -> stensor{
      const auto s    = deviator(sig);
      const auto J2   = (s|s)/2;
  ...
  void test2(const tfel::math::stensor<N,double>& t){
    ...
    const auto nb   = (sqrt(J2)/2)*dJ2-(c/3)*dJ3;
    const auto dnb  = 1/(4*sqrt(J2))*(dJ2^dJ2)+
      (sqrt(J2)/2)*d2J2-(c/3)*d2J3;
...
TFEL_TESTS_GENERATE_PROXY(IsotropicCazacuYieldTest,
			  "IsotropicCazacuYieldTest");
