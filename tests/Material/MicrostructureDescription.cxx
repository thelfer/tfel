 ...
 private:
  ...
  void user_defined_distribution() {
    ...
    const tfel::math::stensor<3u, real> A2 =
        tfel::math::TransverseIsotropicWalpoleBasis<real>::p(n_a);
    const tfel::math::st2tost2<3u, real> A4 = A2 ^ A2;
    UserDefinedDistributionOfSpheroids<stress> distrib1(spheroid1, real(0.5),
                                                        KGi, A2, A4);
    const auto A_orient = distrib1.computeMeanLocalisator(KG0);
    const auto Aref_orient =
        computeAxisymmetricalEllipsoidLocalisationTensor<stress>(KG0, KGi, n_a,
                                                                 a / b);

    TFEL_TESTS_ASSERT(tfel::material::relative_error(A_orient, Aref_orient) /
                          mu0 <
                      eps / stress(1));
    ...
    UserDefinedDistributionOfSpheroids<stress> distrib3(spheroid1, real(0.5),
                                                        KGi, A2iso, A4iso);
    ...
    UserDefinedDistributionOfSpheroids<stress> distrib5(spheroid1, real(0.5),
                                                        KGi, A2TI, A4TI);
