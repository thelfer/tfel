...
struct HomogenizationSecondMomentsTest final : public tfel::tests::TestCase {
 ...
 private:
  ...
  requires(tfel::math::checkUnitCompatibility<tfel::math::unit::Stress,
    ...
    const auto em2 =
        computeMeanSquaredHydrostaticStrain(kg, f, kgi, real(1), real(1));
    const auto em20 = std::get<0>(em2);
    const auto em2i = std::get<1>(em2);
    const auto eeq2 =
        computeMeanSquaredEquivalentStrain(kg, f, kgi, real(1), real(1));
    ...
    const auto HS =
        computeIsotropicHashinShtrikmanBounds<3, stress>(tab_f, tab_k, tab_mu);
    const auto LB = std::get<0>(HS);

    const auto HS_dmu0 = computeIsotropicHashinShtrikmanBounds<3, stress>(
        tab_f, tab_k, tab_dmu0);
    const auto LB_dmu0 = std::get<0>(HS_dmu0);
    const auto dk_dmu0 = (std::get<0>(LB_dmu0) - std::get<0>(LB)) / eps_d / mu0;
    const auto dmu_dmu0 =
        (std::get<1>(LB_dmu0) - std::get<1>(LB)) / eps_d / mu0;

    const auto HS_dmui = computeIsotropicHashinShtrikmanBounds<3, stress>(
        tab_f, tab_k, tab_dmui);
    const auto LB_dmui = std::get<0>(HS_dmui);
    const auto dmu_dmui =
        (std::get<1>(LB_dmui) - std::get<1>(LB)) / eps_d / mui;

    const auto HS_dk0 = computeIsotropicHashinShtrikmanBounds<3, stress>(
        tab_f, tab_dk0, tab_mu);
    ...
    const auto HS_dki = computeIsotropicHashinShtrikmanBounds<3, stress>(
        tab_f, tab_dki, tab_mu);
 ...
 private:
  ...
  requires(tfel::math::checkUnitCompatibility<tfel::math::unit::Stress,
    ...
    const auto dmu_dmu0 =
        (std::get<1>(UB_dmu0) - std::get<1>(UB)) / eps_d / mu0;

    const auto HS_dmui = computeIsotropicHashinShtrikmanBounds<3, stress>(
        tab_f, tab_k, tab_dmui);
    const auto UB_dmui = std::get<1>(HS_dmui);
    const auto dmu_dmui =
        (std::get<1>(UB_dmui) - std::get<1>(UB)) / eps_d / mui;
