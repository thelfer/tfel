...
struct AbaqusTangentOperator2 final
  : public tfel::tests::TestCase
{
 ...
 private:
  ...
  void check1(){
    ...
    const auto CSE = [&l0,&m0](const tensor& F)
      -> st2tost2
    {
	...
	for(size_type j=0;j!=3;++j){
	  ...
	  for(size_type k=0;k!=3;++k){
		...
		continue;
	      ...
	      set(C_SE,rij,rkl,
		  l0*get(iC,rij)*get(iC,rkl)+m*(get(iC,rik)*get(iC,rjl)+
    ...
    const auto Ca2 = [nhb,&CSE](const tfel::math::tensor<N,real>& F)
      -> st2tost2
    {
      return tfel::material::convert<TangentOperator::ABAQUS,
                                     TangentOperator::DS_DEGL>(CSE(F),tensor::Id(),F,nhb(F));
    };
...
TFEL_TESTS_GENERATE_PROXY(AbaqusTangentOperator2,
			  "AbaqusTangentOperator2");
