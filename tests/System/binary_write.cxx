...
struct BinaryReadWriteTest final
  : public tfel::tests::TestCase
{
  ...
  execute() override
  {
    ...
    this->test4<std::map<std::string,int>>(fd,{{"Lena",1972},{"Bettie",1955}});
    return this->result;
  } // end of execute
 private:
  template<typename T>
  ...
  void test2(const int* fd,const T& v){
    using namespace tfel::system;
    binary_write(fd[1],v);
    TFEL_TESTS_ASSERT(std::abs(binary_read<T>(fd[0])-v)<10*std::numeric_limits<T>::epsilon());
  }
  template<typename T>
