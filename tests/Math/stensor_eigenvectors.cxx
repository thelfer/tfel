...
private:
  ...
  void test(void)
  {
    ...
    TFEL_TESTS_ASSERT(abs(vp1-static_cast<T>(4.16709379934921))<20*numeric_limits<T>::epsilon());  
    TFEL_TESTS_ASSERT(abs(vp2+static_cast<T>(1.68923153093191))<20*numeric_limits<T>::epsilon());  
    TFEL_TESTS_ASSERT(abs(vp3-static_cast<T>(1.50793773158270))<20*numeric_limits<T>::epsilon());  

    TFEL_TESTS_ASSERT(abs(vec1[0]-static_cast<T>(0.6208263966073649))<20*numeric_limits<T>::epsilon());
    TFEL_TESTS_ASSERT(abs(vec1[1]-static_cast<T>(0.6185290894233862))<20*numeric_limits<T>::epsilon());
    TFEL_TESTS_ASSERT(abs(vec1[2]-static_cast<T>(0.4816599950303030))<20*numeric_limits<T>::epsilon());

    TFEL_TESTS_ASSERT(abs(vec2[0]+static_cast<T>(0.6378665158240716))<20*numeric_limits<T>::epsilon());
    TFEL_TESTS_ASSERT(abs(vec2[1]-static_cast<T>(0.0413740968617118))<20*numeric_limits<T>::epsilon()); 
    TFEL_TESTS_ASSERT(abs(vec2[2]-static_cast<T>(0.7690347795121735))<20*numeric_limits<T>::epsilon()); 

    TFEL_TESTS_ASSERT(abs(vec3[0]-static_cast<T>(0.4557421346177839))<20*numeric_limits<T>::epsilon());
    TFEL_TESTS_ASSERT(abs(vec3[1]+static_cast<T>(0.7846718738721010))<20*numeric_limits<T>::epsilon());
    TFEL_TESTS_ASSERT(abs(vec3[2]-static_cast<T>(0.4202251266738716))<20*numeric_limits<T>::epsilon());
  }
  ...
  void test2(void)
  {
    ...
    for(i=0;i<nb_boucles;++i){
      ...
      s(2)=static_cast<T>((static_cast<T>(std::rand())/static_cast<T>(RAND_MAX)-one_half)*2);
