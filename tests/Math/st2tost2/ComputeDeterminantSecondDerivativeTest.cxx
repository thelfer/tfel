  ...
  : public tfel::tests::TestCase
{
  ComputeDeterminantSecondDerivativeTest()
    : tfel::tests::TestCase("TFEL/Math","ComputeDeterminantSecondDerivative")
  {} // end of ComputeDeterminantSecondDerivativeTest
  
  tfel::tests::TestResult execute() override
  {
    const double Id[6] = {1.,1.,1.,0.,0.,0.};
    const double v[6]  = {1.27453824166446,0.77207083708966,0.24525337568425,
 ...
 private:
  ...
  void test3(const double* values){
    ...
    const st2tost2<N,double> dJ2_2 =
      Stensor4::dsquare(s)-(s^id)-i1*Stensor4::Id()
      +(id^(i1*id-s));
    for(size_type i=0;i!=s.size();++i){
      TFEL_TESTS_ASSERT(std::abs(c(i))<10*std::numeric_limits<double>::epsilon());
      for(size_type j=0;j!=s.size();++j){
