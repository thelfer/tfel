  ...
  : public tfel::tests::TestCase
{
  ...
  tfel::tests::TestResult execute() override
  {
	...
	if(abs(nd(i,j)-d(i,j))>prec){
	  std::cout << "Error " << N << " nd (" << i << ", " << j << ") " << "[" << i*StensorDimeToSize<N>::value+j << "]: "
		    << nd(i,j) << " vs " << d(i,j) << " " << std::abs(nd(i,j)-d(i,j)) << std::endl;
	}
      }
    }
    return this->result;
...
using SymmetricStensorProductDerivativeTest_1D = SymmetricStensorProductDerivativeTest<1u>;
using SymmetricStensorProductDerivativeTest_2D = SymmetricStensorProductDerivativeTest<2u>;
using SymmetricStensorProductDerivativeTest_3D = SymmetricStensorProductDerivativeTest<3u>;
