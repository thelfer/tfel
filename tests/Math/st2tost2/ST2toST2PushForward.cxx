...
struct ST2toST2PushForwardTest
...
private:
  ...
  void test(){
    ...
    const st2tost2<N,double> D =
      lambda*st2tost2<N,double>::IxI()+2*mu*st2tost2<N,double>::Id();
  ...
  void test2(){
    ...
    auto get = [](const stensor<3u,double>& s,
		  const size_type i){
      const double cste = std::sqrt(2.);
      if(i>2){
	return s(i)/cste;
	...
	for(size_type k=0;k!=3;++k){
	  for(size_type l=0;l!=3;++l){
	    ...
	    set(C_SE,rij,rkl,
		lambda*get(iC,rij)*get(iC,rkl)+mu*(get(iC,rik)*get(iC,rjl)+
