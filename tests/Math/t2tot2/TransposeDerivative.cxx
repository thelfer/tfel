...
struct TransposeDerivativeTest
  : public tfel::tests::TestCase
{
 ...
 private:
  ...
  void execute(){
    ...
    const t2tot2<N,double> dt  =
      t2tot2<N,double>::transpose_derivative();
    t2tot2<N,double> ndt;
    for(unsigned short i=0;i!=F.size();++i){
      tensor<N,double> dF(0.);
      dF[i]+=eps;
      const tensor<N,double> dtp = transpose(dF);      
      dF[i]-=2*eps;
      const tensor<N,double> dtm = transpose(dF);      
	...
	if(std::abs(ndt(i,j)-dt(i,j))>prec){
	  std::cout << i << " " << j << " " << ndt(i,j) << " " << dt(i,j) << std::endl;
	}
...
TFEL_TESTS_GENERATE_PROXY(TransposeDerivativeTest,
			  "TransposeDerivativeTest");
