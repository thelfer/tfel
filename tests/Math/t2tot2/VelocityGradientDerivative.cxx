...
struct VelocityGradientDerivativeTest
  : public tfel::tests::TestCase
{
 ...
 private:
  ...
  void execute(){
    ...
    const tfel::math::tensor<N,double> iF = invert(F);
    const tfel::math::t2tot2<N,double> dL  = computeVelocityGradientDerivative(F);
	...
	if(std::abs(ndL(i,j)-dL(i,j))>prec){
	  std::cout << i << " " << j << " " << ndL(i,j) << " " << dL(i,j) << std::endl;
	}
  ...
  void execute2(){
    ...
    const tensor<N,double> iF = invert(F);
    const t2tot2<N,double> dL  = t2tot2<N,double>::tprd(iF);
    t2tot2<N,double> ndL;
    for(unsigned short i=0;i!=F.size();++i){
      tensor<N,double> dF(0.);
      dF[i]+=eps;
      const tensor<N,double> Lp = iF*dF;
      dF[i]-=2*eps;
      const tensor<N,double> Lm = iF*dF;
      for(unsigned short j=0;j!=F.size();++j){
	ndL(j,i) = (Lp(j)-Lm(j))/(2*eps);
      }
    }
