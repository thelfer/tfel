  ...
  : public tfel::tests::TestCase
{
 ...
 private:
  ...
  void execute(){
    ...
    const t2tot2<N,double> dW = computeSpinRateDerivative(F);
    t2tot2<N,double> ndW;
  ...
  void execute2(){
    ...
    const tensor<N,double> F(values);
    const tensor<N,double> iF = invert(F);
    const t2tot2<N,double> d =
      t2tot2<N,double>::tprd(transpose(iF),t2tot2<N,double>::transpose_derivative());
    t2tot2<N,double> nd;
    for(unsigned short i=0;i!=F.size();++i){
      tensor<N,double> dFp(0.);
      tensor<N,double> dFm(0.);
    ...
    for(unsigned short i=0;i!=F.size();++i){
      for(unsigned short j=0;j!=F.size();++j){
    	if(std::abs(nd(i,j)-d(i,j))>prec){
    	  std::cout << i << " " << j << " " << nd(i,j) << " " << d(i,j) << std::endl;
    	}
...
TFEL_TESTS_GENERATE_PROXY(SpinRateDerivativeTest,
			  "SpinRateDerivativeTest");
