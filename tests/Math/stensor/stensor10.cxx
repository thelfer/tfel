...
private:
  void check(const tfel::math::stensor<1u,double>& s,
	     const double prec){
    using namespace std;
    using namespace tfel::math;
...
private:
  void check(const tfel::math::stensor<2u,double>& s,
	     const double prec){
    using namespace std;
    using namespace tfel::math;
    const double eps  = 1.e-4;
  ...
  : public tfel::tests::TestCase
{
  ...
  tfel::tests::TestResult execute() override
  {
    ...
    const double v4[6] = {8.2,4.5,-7.2,3.14*sqrt2,-6.42*sqrt2,2.3*sqrt2};
...
private:
  void check(const tfel::math::stensor<3u,double>& s,
	     const double prec){
    using namespace std;
    using namespace tfel::math;
    const double eps  = 1.e-4;
...
private:
  void check(const tfel::math::stensor<1u,double>& s,
    ...
    computeStensorDecompositionInPositiveAndNegativeParts(dpp,dnp,pp,np,s,1.e-10);
    for(unsigned short i=0;i!=3u;++i){
      stensor<1u,double> ls(s);
      ls[i] +=   eps;
      const auto ppf = positive_part(ls);
