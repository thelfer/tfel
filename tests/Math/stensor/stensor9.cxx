  ...
  : public tfel::tests::TestCase
{
  ...
  execute() override
  {
    ...
    stensor<3u,double>::computeEigenTensorsDerivatives(dn0,dn1,dn2,vp,m,1.e-7);
    stensor<3u,double> exp_s(s.buildFromEigenValuesAndVectors(exp(vp(0)),exp(vp(1)),exp(vp(1)),m));
    st2tost2<3u,double> dexp;
    dexp=exp(vp(0))*((n0^n0)+dn0)+exp(vp(1))*((n1^n1)+dn1)+exp(vp(2))*((n2^n2)+dn1);
