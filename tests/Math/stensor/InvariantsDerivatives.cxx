...
getNumericalApproximation2(const F& f,
			   const tfel::math::stensor<N,real>& C,
			   const real e){
...
struct InvariantsDerivatives final
...
private:
  ...
  void check(const tfel::math::stensor<N,double>& s1){
    ...
    auto I3 = [](const stensor& v){
      return det(v);
    };
    auto iJb = [&I3](const stensor& v){
      return 1/std::cbrt(I3(v));
    };
    auto dI1 = [](const stensor&) -> stensor{
      return stensor::Id();
    };
    ...
    const auto d2iJb_ds2  =
      d2iJb_dI32*(dI3_ds^dI3_ds)+diJb_dI3*d2I3_ds2;
    ...
    for(unsigned short i=0;i!=s.size();++i){
      ...
      if(std::abs(diJb_ds(i)-ndiJb_ds(i))>prec2){
      	std::cout << "diJb_ds : " << i << " " << diJb_ds(i) << " " << ndiJb_ds(i)
      		  << " " << std::abs(diJb_ds(i)-ndiJb_ds(i)) << std::endl;
      }
