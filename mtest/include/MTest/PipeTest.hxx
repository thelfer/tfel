...
namespace mtest {
  ...
  struct MTEST_VISIBILITY_EXPORT PipeTest : public SingleStructureScheme {
   ...
   private:
     ...
     * \param[in] s: current state
     */
    void setIntegrationPointPositionForEvolutionsEvaluation(
        const CurrentState&) const override;
    //! list of elements
    std::vector<std::shared_ptr<PipeElement>> elements;
    ...
    std::shared_ptr<GasEquationOfState> gseq;
