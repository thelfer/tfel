...
namespace mtest
{
  ...
  struct NonLinearConstraint::Constraint
  {
    ...
    struct ConstraintDerivative
    {
      ...
      std::map<std::shared_ptr<Variable>,
	       std::shared_ptr<tfel::math::Evaluator>> dd_dv;
      //! second derivative with respect to thermodynamic forces
      std::map<std::shared_ptr<Variable>,
	       std::shared_ptr<tfel::math::Evaluator>> dd_tf;
  ...
  NonLinearConstraint::Constraint::Variable::Variable(const std::string& n,
    ...
    : name(n),p(pos)
  {} // end of NonLinearConstraint::Constraint::Variable::Variable      
				   ...
				   const real dt) const
  {
    ...
    for(const auto& e:this->c->evs){
      ev.setVariableValue(e.first,
			  e.second->operator()(t+dt));
    }
    return ev.getValue();
  } // end of eval
    ...
    : b(b_)
  {
	...
	add_variable(this->c->tfs,v,b.getThermodynamicForceComponentPosition(v));
      } else {
    ...
    throw_if((this->c->dvs.empty())&&(this->c->tfs.empty()),"invalid constraint '"+f+"', "
	...
	d.v = v;
	d.d = std::dynamic_pointer_cast<Evaluator>(this->c->c->differentiate(v->name));
	for(const auto& v2 : this->c->dvs){
	  auto dd = std::dynamic_pointer_cast<Evaluator>(d.d->differentiate(v2->name));
  				      ...
  				      const real a) const
  {
	...
	for(unsigned short i=0;i!=ndv;++i){
	  ...
	  for(const auto& d2: d.dd_tf){
	    ...
	    for(unsigned short j=0;j!=ndv;++j){
	      K(i,j)  -= nf*d2c_dsds2*k(ps,i)*k(ps2,j)*l; // missing term dk(ps_i)_dj
	    }
	  }
	}
      }
  						   ...
  						   const real dt) const
  {
    ...
    return "imposed constraint not reached "
      "(constraint value: "+std::to_string(cv)+","
      " criteria : "+std::to_string(ev)+")";
  }
