...
namespace mtest{
    ...
    : StandardBehaviourBase(h,l,b)
  {
	...
	if((h==ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRAIN)||
	  ...
	  mps.insert(mps.end(),{"YoungModulus1","YoungModulus2","YoungModulus3",
	  ...
	  tfel::raise("GenericBehaviour::GenericBehaviour : "
	...
	mps.insert(mps.end(),{"ThermalExpansion1",
  ...
  std::pair<bool,real> GenericBehaviour::integrate(CurrentState& s,
  ...
  std::pair<bool,real>
  GenericBehaviour::call_behaviour(tfel::math::matrix<real>& Kt,
				   CurrentState& s,
				   BehaviourWorkSpace& wk,
				   const real dt,
				   ...
				   const bool b) const
  {
	...
	for(decltype(s.e1.size()) i=0;i!=s.e1.size();++i){
      ...
      applyRotation(d.s0.gradients,this->dvtypes,
		    this->getHypothesis(),s.r);
      applyRotation(d.s1.gradients,this->dvtypes,
		    this->getHypothesis(),s.r);
  ...
  void GenericBehaviour::setOptionalMaterialPropertiesDefaultValues(EvolutionManager&,
