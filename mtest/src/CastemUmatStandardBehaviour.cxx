...
namespace mtest{
  ...
  CastemUmatStandardBehaviour::buildCastemUmatStandardBehaviour(const std::string& l,
								const std::string& f,
    ...
    if(mp.count("FCT_LOI")==1u){
      ...
      throw_if(fct.size()>16,"invalid parameter 'FCT_LOI': must be smaller than 16 characters");
    ...
    throw_if(mp.count("SYMETRIE")!=1u,"the material symmetry is not defined, "
	     "you shall use the 'SYMETRIE' (in french) parameter. The 'SYMETRIE' "
	     "parameter must be string whose value is either 'ISOTROPE' or 'ORTHOTROPE'");
    ...
    for(decltype(mpnames.size()) i=0;i!=mmpnames.size();++i){
      throw_if(mpnames[i]!=mmpnames[i],
	       "invalid "+std::to_string(i)+"th material property: "
	       "expected '"+mmpnames[i]+"', but '"+mpnames[i]+"' declared");
    }
    auto pmp = mpnames.begin()+mpnames.size();
    for(const auto& omp : ompnames){
      if(pmp==mpnames.end()){break;}
  ...
  } // end of CastemUmatStandardBehaviour::setOptionalMaterialPropertiesDefaultValues
