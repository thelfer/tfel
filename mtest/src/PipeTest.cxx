...
namespace mtest {
  ...
  struct IntegralTest : public PipeTest::UTest {
    IntegralTest(const std::string& n,
      ...
      if (n == "InnerDisplacement") {
        ...
        throw(
            std::runtime_error("IntegralTest::IntegralTest: "
    ...
    virtual void check(const StudyCurrentState& s,
      ...
      const auto uv = [](const tfel::math::vector<real>& u, const Variable vn) {
        ...
        throw(
            std::runtime_error("IntegralTest::check: "
                               "unsupported variable"));
      }(s.u1, this->v);
  ...
  void PipeTest::setInnerRadius(const real r) {
    if (this->mesh.outer_radius > 0) {
      if (r >= this->mesh.outer_radius) {
        throw(
            std::runtime_error("PipeTest::setInnerRadius: invalid value. "
  ...
  void PipeTest::setOuterRadius(const real r) {
    if (this->mesh.inner_radius > 0) {
      if (r <= this->mesh.inner_radius) {
        throw(
            std::runtime_error("PipeTest::setOuterRadius: invalid value. "
                               "Inner radius would be greater than the "
                               "external radius"));
      }
    }
  ...
  static void checkValue(const T& v, const char* const n) {
    if (v < T(0)) {
      throw(
          std::runtime_error("PipeTest::completeInitialisation: "
  ...
  static void setCurrentPosition(EvolutionManager& evm, const real r) {
    ...
    if (p == evm.end()) {
      throw(
          std::runtime_error("setCurrentPosition: "
  ...
  void PipeTest::addIntegralTest(const std::string& n,
    ...
    if ((n == "InnerDisplacement") || (n == "OuterDisplacement") || (n == "AxialGrowth")) {
      ...
      throw(
          std::runtime_error("PipeTest::addIntegralTest: "
                             "unsupported variable '" +
                             n + "'"));
    }
  ...
  void PipeTest::addProfileTest(const std::string& n,
    ...
    if (n == "SRR") {
      ...
      if (piv == this->ivfullnames.end()) {
        throw(
            std::runtime_error("PipeTest::addProfileTest: "
  ...
  void PipeTest::completeInitialisation() {
    ...
    if (this->rl == IMPOSEDOUTERRADIUS) {
      ...
      if ((this->rl != TIGHTPIPE) && (this->al != IMPOSEDAXIALFORCE) &&
        ...
        if ((this->inner_pressure == nullptr) && (this->outer_pressure == nullptr)) {
          throw(
              std::runtime_error("PipeTest::completeInitialisation: "
  ...
  PipeTest::size_type PipeTest::getNumberOfNodes() const {
    if (this->mesh.number_of_elements <= 0) {
      throw(
          std::runtime_error("PipeTest::getNumberOfNodes: "
  ...
  void PipeTest::setDisplacementEpsilon(const real e) {
    if (this->options.eeps > 0) {
      throw(
          std::runtime_error("PipeTest::setDisplacementEpsilon: "
  ...
  void PipeTest::setResidualEpsilon(const real s) {
    if (this->options.seps > 0) {
      throw(
          std::runtime_error("PipeTest::setResidualEpsilon: the epsilon "
  ...
  void PipeTest::initializeCurrentState(StudyCurrentState& s) const {
    if (this->b == nullptr) {
      throw(
          std::runtime_error("PipeTest::initializeCurrentState: "
    ...
    if ((this->mesh.number_of_elements == -1) || (this->mesh.inner_radius < 0) ||
    ...
    if (this->mesh.etype == PipeMesh::LINEAR) {
      ...
      throw(
          std::runtime_error("PipeTest::getNumberOfUnknowns: "
                             "unknown element type"));
    }
    ...
    for (auto& cs : ss.istates) {
      ...
      if ((this->iv_t0.size() > cs.iv_1.size()) || (this->iv_t0.size() > cs.iv0.size()) ||
  ...
  void PipeTest::prepare(StudyCurrentState& state, const real t, const real dt) const {
    ...
    for (size_type i = 0; i != ne; ++i) {
      if (this->mesh.etype == PipeMesh::LINEAR) {
        ...
        throw(
            std::runtime_error("PipeTest::prepare: "
                               "unknown element type"));
      }
    }
  ...
  std::pair<bool, real> PipeTest::computeStiffnessMatrixAndResidual(
    ...
    if (this->rl == TIGHTPIPE) {
      ...
      if (pT == this->evm->end()) {
        throw(
            std::runtime_error("PipeTest::computeStiffnessMatrixAndResidual: "
  ...
  void PipeTest::checkBehaviourConsistency(const std::shared_ptr<Behaviour>& bp) {
    ...
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setBehaviour: " + m));
      }
    };
    throw_if(bp == nullptr, "null behaviour pointer");
    if (this->hpp) {
      throw_if(bp->getBehaviourType() != MechanicalBehaviourBase::SMALLSTRAINSTANDARDBEHAVIOUR,
               "the behaviour must be a small strain one "
               "when performing small strain analysis");
    } else {
      throw_if(bp->getBehaviourType() != MechanicalBehaviourBase::FINITESTRAINSTANDARDBEHAVIOUR,
  ...
  void PipeTest::performSmallStrainAnalysis() {
    ...
    if (this->b != nullptr) {
      if (this->b->getBehaviourType() != MechanicalBehaviourBase::SMALLSTRAINSTANDARDBEHAVIOUR) {
        throw(
            std::runtime_error("PipeTest::performSmallStrainAnalysis: "
  ...
  static PipeTest::size_type getNumberOfGaussPoints(const PipeMesh& m) {
    ...
    throw(
        std::runtime_error("getNumberOfGaussPoints: "
                           "unknown element type"));
  ...
  void PipeTest::addOutput(const std::string& t, const std::string& n) {
    if (t == "minimum_value") {
      ...
      throw(
          std::runtime_error("PipeTest::addAdditionalOutput: "
  ...
  void PipeTest::addProfile(const std::string& f, const std::vector<std::string>& cn) {
    ...
    if (!(*(ph.out))) {
      throw(
          std::runtime_error("PipeTest::addProfile: "
                             "can't open file '" +
                             f + "'"));
    }
  ...
  void PipeTest::setModellingHypothesis(const std::string& h) {
    if (h != "AxisymmetricalGeneralisedPlaneStrain") {
      throw(
          std::runtime_error("PipeTest::setModellingHypothesis: "
                             "invalid modelling hypothesis '" +
                             h + "'"));
    }
  ...
  void PipeTest::setDefaultModellingHypothesis() {
    if (this->hypothesis != ModellingHypothesis::UNDEFINEDHYPOTHESIS) {
      throw(
          std::runtime_error("PipeTest::setDefaultModellingHypothesis: "
                             "modelling hypothesis already defined"));
    }
  ...
  void PipeTest::setInnerPressureEvolution(std::shared_ptr<Evolution> p) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setInnerPressureEvolution: " + m));
      }
    };
  ...
  void PipeTest::setOuterPressureEvolution(std::shared_ptr<Evolution> p) {
    if (this->outer_pressure != nullptr) {
      throw(
          std::runtime_error("PipeTest::setOuterPressureEvolution: "
  ...
  void PipeTest::setAxialForceEvolution(std::shared_ptr<Evolution> f) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setAxialForceEvolution: " + m));
      }
    };
  ...
  void PipeTest::setAxialGrowthEvolution(std::shared_ptr<Evolution> f) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setOuterRadiusEvolution: " + m));
      }
    };
  ...
  void PipeTest::setGasEquationOfState(const std::string& e) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setGasEquationOfState: " + m));
      }
    };
  ...
  void PipeTest::setFillingPressure(const real p) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setFillingPressure: " + m));
      }
    };
    throw_if(this->rl != TIGHTPIPE,
  ...
  void PipeTest::setFillingTemperature(const real T) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("PipeTest::setFillingTemperature: " + m));
      }
    };
    throw_if(this->rl != TIGHTPIPE,
