...
namespace mtest {
  ...
  void ConstantEvolution::setValue(const real, const real) {
    throw(
        std::runtime_error("ConstantEvolution::setValue : "
  ...
  LPIEvolution::LPIEvolution(const std::vector<real>& t,
    ...
    if (t.size() != v.size()) {
      throw(
          std::runtime_error("LPIEvolution::LPIEvolution : "
  ...
  void LPIEvolution::setValue(const real) {
    throw(
        std::runtime_error("LPIEvolution::setValue : "
                           "this method does not makes sense "
                           "for LPI evolution"));
  }
  ...
  real LPIEvolution::operator()(const real t) const {
    if (this->values.empty()) {
      throw(
          std::runtime_error("LPILoadingEvolution::getValue: "
                             "no values specified"));
    }
