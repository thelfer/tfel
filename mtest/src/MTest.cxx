...
namespace mtest {
  ...
  void MTest::setDrivingVariableEpsilon(const real e) {
    if (this->options.eeps > 0) {
      throw(
          std::runtime_error("MTest::setDrivingVariableEpsilon: the epsilon "
                             "value has already been declared"));
    }
    if (e < 100 * std::numeric_limits<real>::min()) {
      throw(
          std::runtime_error("MTest::setDrivingVariableEpsilon:"
                             " invalid value"));
    }
    this->options.eeps = e;
  }
  ...
  void MTest::setThermodynamicForceEpsilon(const real s) {
    if (this->options.seps > 0) {
      throw(
          std::runtime_error("MTest::setThermodynamicForceEpsilon: the epsilon "
                             "value has already been declared"));
    }
  ...
  void MTest::setRotationMatrix(const tfel::math::tmatrix<3u, 3u, real>& r,
    ...
    if ((this->isRmDefined) && (!bo)) {
      throw(
          std::runtime_error("MTest::setRotationMatrix: "
  ...
  void MTest::setDefaultModellingHypothesis() {
    ...
    if (this->hypothesis != ModellingHypothesis::UNDEFINEDHYPOTHESIS) {
      throw(
          std::runtime_error("MTest::setDefaultModellingHypothesis: "
  ...
  void MTest::setDrivingVariablesInitialValues(const std::vector<real>& v) {
    if (!this->e_t0.empty()) {
      throw(
          std::runtime_error("MTest::setDrivingVariablesInitialValues: "
  ...
  void MTest::setThermodynamicForcesInitialValues(const std::vector<real>& v) {
    if (!this->s_t0.empty()) {
      throw(
          std::runtime_error("MTest::setThermodynamicForcesInitialValues: "
                             "the initial values of the strains have "
                             "already been declared"));
    }
  ...
  void MTest::completeInitialisation() {
    ...
    if (ivdes.size() != this->b->getInternalStateVariablesSize()) {
      throw(
          std::runtime_error("MTest::completeInitialisation : internal error "
    ...
    if (pev != this->evm->end()) {
      ...
      if (!ev.isConstant()) {
        throw(
            runtime_error("MTest::completeInitialisation: "
  ...
  void MTest::initializeCurrentState(StudyCurrentState& s) const {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(std::runtime_error("MTest::initializeCurrentState: " + m));
      }
    };
  ...
  void MTest::initializeWorkSpace(SolverWorkSpace& wk) const {
    if (!this->initialisationFinished) {
      throw(
          std::runtime_error("MTest::initializeWorkSpace: "
  ...
  size_t MTest::getNumberOfUnknowns() const {
    ...
    if (!this->initialisationFinished) {
      throw(
          std::runtime_error("MTest::getNumberOfUnknowns: "
                             "object not initialised"));
    }
  ...
  void MTest::setTangentOperatorComparisonCriterium(const real v) {
    if (v < 100 * std::numeric_limits<real>::min()) {
      throw(
          std::runtime_error("MTest::setTangentOperatorComparisonCriterium: "
  ...
  void MTest::prepare(StudyCurrentState& state,
    ...
    if (scs.istates.size() != 1u) {
      throw(
          std::runtime_error("MTest::prepare: "
                             "invalid state"));
    }
  ...
  std::pair<bool, real> MTest::computePredictionStiffnessAndResidual(
    ...
    for (const auto& c : this->constraints) {
      c->setValues(k, r, state.u0, state.u0, pos,
