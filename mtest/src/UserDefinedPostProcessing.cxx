...
namespace mtest{
  ...
  UserDefinedPostProcessing::UserDefinedPostProcessing(const Behaviour& b,
  ...
  void UserDefinedPostProcessing::exe(const CurrentState& s, const real t, const real dt) {
    this->out << t + dt << " ";
    for (const auto& p : this->postprocessings) {
      ...
      for(const auto& vn : vns){
        ...
        if (pe != this->extractors.end()) {
          ...
          if(pev==this->evm.end()){
            tfel::raise("UserDefinedPostProcessing::exe: unknown variable '" + vn + "'");
          }
