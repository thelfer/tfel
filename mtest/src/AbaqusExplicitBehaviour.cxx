...
namespace mtest {
  ...
  std::string AbaqusExplicitBehaviour::getBehaviourName(const std::string& b,
                                                        const Hypothesis h) {
  ...
  AbaqusExplicitBehaviour::AbaqusExplicitBehaviour(const Hypothesis h,
                                                   ...
                                                   const std::string& b)
      : UmatBehaviourBase(
            h, l, AbaqusExplicitBehaviour::getBehaviourName(b, h)) {
    auto throw_if = [](const bool c, const std::string& m) {
      if (c) {
        throw(
            std::runtime_error("AbaqusExplicitBehaviour::"
  ...
  void AbaqusExplicitBehaviour::getDrivingVariablesDefaultInitialValues(
  ...
  bool AbaqusExplicitBehaviour::doPackagingStep(CurrentState& s,
    ...
    for (AbaqusInt i = 0; i != ndir + nshr; ++i) {
      ...
      (this->fct)(&nblock, &ndir, &nshr, &nstatv, &nfieldv, &nprops, nullptr,
                  ...
                  nstatv == 0 ? nullptr : &(s.iv0[0]), &s.iv0[nstatv],
                  &s.iv0[nstatv + 1], &(wk.evs[0]), stretchNew, defgradNew,
                  wk.evs.size() == 1 ? nullptr : &(wk.evs[0]) + 1, stressNew,
                  nstatv == 0 ? nullptr : &(s.iv1[0]), &s.iv1[nstatv],
                  &s.iv1[nstatv + 1], 0);
      if (h == ModellingHypothesis::PLANESTRESS) {
  ...
  std::pair<bool, real> AbaqusExplicitBehaviour::integrate(
    ...
    if ((h == ModellingHypothesis::PLANESTRESS) ||
      ...
      throw(
          std::runtime_error("AbaqusExplicitBehaviour::integrate: "
