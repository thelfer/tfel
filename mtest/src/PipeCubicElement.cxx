...
namespace mtest{
  ...
  real
  PipeCubicElement::interpolate(const real v0,const real v1,
				const real v2,const real v3,
				const real x)
  {
  ...
  constexpr real
  PipeCubicElement::jacobian(const real r0,const real r1,
  ...
  void
  PipeCubicElement::computeStrain(StructureCurrentState& scs,
				  const PipeMesh& m,
  ...
  PipeCubicElement::updateStiffnessMatrixAndInnerForces(tfel::math::matrix<real>& k,
	...
	for(const auto l : {0,1,2,3}){
	  for(const auto j : {0,1,2,3}){
	    ...
	    k(3*i+l,3*i+j) += w*(rg*dsfv[l]/J*(bk(0,0)*de0_du+bk(0,2)*de2_du)+
				 sfv[l]*(bk(2,0)*de0_du+bk(2,2)*de2_du));
	  }
	  k(3*i+l,n) += w*(rg*dsfv[l]/J*bk(0,1)+bk(2,1)*sfv[l]);
	} // loop over nodes
	for(const auto j : {0,1,2,3}){
	  const auto de0_du = dsfv[j]/J;
	  const auto de2_du = sfv[j]/rg;
