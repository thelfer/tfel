...
namespace mtest
{
  ...
  static MistralBehaviourDescription
  readMistralFile(const std::string& l,
		  const std::string& f,
    ...
    const auto mp = parameters.get<std::map<std::string,
					    MistralBehaviour::Parameters>>();
    ...
    if(ISOEL){
      ...
      throw_if(true,"orthotropic elastic material properties are not supported yet");
    }
    if(h==ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRAIN){
      throw_if(!ISOEL,"orthotropic elastic material properties are not supported yet in 1D");
      md.mpnames.insert(md.mpnames.begin(),{"YoungModulus","PoissonRatio","ThermalExpansion"});
    } else if(h==ModellingHypothesis::PLANESTRESS){
      md.mpnames.insert(md.mpnames.begin(),{
	  "YoungModulus1","YoungModulus2","PoissonRatio12",
	    "ShearModulus12","V1X","V1Y","YoungModulus3",
	    "PoissonRatio23","PoissonRatio13",
    ...
    md.ivnames.insert(md.ivnames.end(),{"EEL1","EEL2","EEL3","EEL4","EEL5","EEL6"});
    if(NDPI > 0){md.ivnames.insert(md.ivnames.end(),{"EP01","EP02","EP03","EP04","EP05","EP06"});}
    if(NDVP > 0){md.ivnames.insert(md.ivnames.end(),{"EP11","EP12","EP13","EP14","EP15","EP16"});}
    if(NDVP > 1){md.ivnames.insert(md.ivnames.end(),{"EP21","EP22","EP23","EP24","EP25","EP26"});}
    if(NDVP > 2){md.ivnames.insert(md.ivnames.end(),{"EP31","EP32","EP33","EP34","EP35","EP36"});}
    if(NDVP > 3){md.ivnames.insert(md.ivnames.end(),{"EP41","EP42","EP43","EP44","EP45","EP46"});}
    throw_if(NDVP > 4,"unsupported case (NDVP > 4)");
    md.ivnames.insert(md.ivnames.end(),{"ECR1","ECR2","ECR3","ECR4","ECR5","ECR6"});
    if(NDPI > 0){md.ivnames.insert(md.ivnames.end(),{"EP0E","EP0Q"});}
    if(NDVP > 0){md.ivnames.insert(md.ivnames.end(),{"EP1E","EP1Q"});}
    if(NDVP > 1){md.ivnames.insert(md.ivnames.end(),{"EP2E","EP2Q"});}
    if(NDVP > 2){md.ivnames.insert(md.ivnames.end(),{"EP3E","EP3Q"});}
    if(NDVP > 3){md.ivnames.insert(md.ivnames.end(),{"EP4E","EP4Q"});}
    throw_if(NDVP > 4,"unsupported case (NDVP > 4)");
    if(NXX  > 0){md.ivnames.insert(md.ivnames.end(),{"X11 ","X12 ","X13 ","X14 ","X15 ","X16 "});}
    if(NXX  > 1){md.ivnames.insert(md.ivnames.end(),{"X21 ","X22 ","X23 ","X24 ","X25 ","X26 "});}
    if(NXX  > 2){md.ivnames.insert(md.ivnames.end(),{"X31 ","X32 ","X33 ","X34 ","X35 ","X36 "});}
    throw_if(NXX > 3,"unsupported case (NXX > 3)");
    md.ivnames.emplace_back("FIT");
    if(NDP  > 0){md.ivnames.emplace_back("PSI");}
    md.ivnames.emplace_back("DTA");
    if(NDPI > 0){md.ivnames.insert(md.ivnames.end(),{"SEA0","RRA0","VEA0","KPLA"});}
    if(NDP  > 0){md.ivnames.emplace_back("EPSE");}
    md.ivnames.insert(md.ivnames.end(),{"VET1","VET2","VET3","VET4","VET5","VET6"});
    if(NDP  > 0){md.ivnames.insert(md.ivnames.end(),{"VEI1","VEI2","VEI3","VEI4","VEI5","VEI6"});}
    if(NDPI > 0){md.ivnames.emplace_back("VP0E");}
    if(NDVP > 0){md.ivnames.emplace_back("VP1E");}
    if(NDVP > 1){md.ivnames.emplace_back("VP2E");}
    if(NDVP > 2){md.ivnames.emplace_back("VP3E");}
    if(NDVP > 3){md.ivnames.emplace_back("VP4E");}
    throw_if(NDVP > 4,"unsupported case (NDVP > 4)");
    if(NVIU > 0){
  ...
  MistralBehaviour::buildMistralBehaviour(const std::string& l,
					  const std::string& f,
					  const MistralBehaviour::Parameters& p,
					  const Hypothesis h)
  {
    const auto md = readMistralFile(l,f,p,h);
    std::shared_ptr<MistralBehaviour>
      ptr(new MistralBehaviour(md,md.cvalues,
			       md.young_modulus,
			       md.poisson_ratio,
