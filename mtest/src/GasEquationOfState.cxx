...
namespace mtest{
  ...
  struct GasEquationOfState::GasEquationOfStateMembers
  {
	...
	n  += dn;
      ...
      const auto eps = 100*std::numeric_limits<double>::min()*nref;
      auto n  = nref;
      auto dn = double();
      iter(n,dn);
      unsigned short i = 0;
	...
	P  += dP;
      ...
      const auto eps = 100*std::numeric_limits<double>::min()*Pref;
      auto P  = Pref;
      auto dP = double();
      iter(P,dP);
      unsigned short i = 0;
  ...
  private:
    static double eval(tfel::math::Evaluator& e,const double P,const double V,
  ...
  double GasEquationOfState::computeIsothermalBulkModulus(const double V,const double n,const double T){
