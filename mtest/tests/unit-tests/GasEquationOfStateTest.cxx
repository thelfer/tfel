...
struct GasEquationOfStateTest final
  : public tfel::tests::TestCase
{
 ...
 private:
  void test1(){
    ...
    TFEL_TESTS_ASSERT(std::abs(s.computeNumberOfMoles(P,V,T)-0.98696171678913)<1.e-13);
    TFEL_TESTS_ASSERT(std::abs(s.computePressure(V,n,T)-101321.05257875)<1e-6);
    TFEL_TESTS_ASSERT(std::abs(s.computeIsothermalBulkModulus(V,n,T)-101321.05257875)<1e-6);
  } // end of test1
  void test2(){
    mtest::EvolutionManager m;
    m.insert({"R",mtest::make_evolution(8.31415)});
    mtest::GasEquationOfState s("P*V-(1+2.343e-9*(1-exp(-0.0128*(T-400)))*P+5.2e-9*T**-3*P**2)*n*R*T",m);
    const double P = 4.e7;
    const double V = 0.022414;
    const double n = 1;
    const double T = 1023.15;
    TFEL_TESTS_ASSERT(std::abs(s.computeNumberOfMoles(P,V,T)-95.6874860501282)<1.e-13);
    TFEL_TESTS_ASSERT(std::abs(s.computePressure(V,n,T)-379860.66477839)<1e-6);
  } // end of test2
};
