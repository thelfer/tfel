...
namespace tfel
{
  ...
  namespace utilities
  {
    ...
    static void
    writeMarkdownHeader(std::ostream& log,
    ...
    static std::string
    getIndexKey(const std::string& s){
      if(s.empty()){
	return "";
      }
      if(s[0]=='!'){
    ...
    static void
    writeMarkdownDescription(std::ostream& out,
	...
	if(t.models.size()==1){
	  ...
	  out << "### " << getTranslation("Tested model",l) << "\n\n"
	      << getTranslation("The model tested is:",l)
	      << '`' << m.first << "`.\n\n";
	} else {
    ...
    static void
    writeMarkdownSection(std::ostream& out,
		      const std::string& s,
    ...
    void
    writeMarkdownFile(std::ostream& out,
		      const std::map<std::string,std::vector<TestDocumentation> >& tests,
		      const GeneratorOptions& o)
    {
