...
namespace tfel {
  ...
  namespace utilities {
      ...
      try {
        while (p != pe) {
          if (p->flag != Token::String) {
            throw(
                runtime_error("TestDocParser::addDocumentation : "
          ...
          if (c.empty()) {
            throw(
                runtime_error("no category defined for test '" + t.name + "'"));
          }
          tests[c].push_back(t);
        }
      } catch (exception& e) {
    ...
    void TestDocParser::treatTest(TestDocumentation& t,
      ...
      while (p->value != "}") {
        ...
        if (key == "author") {
          ...
          throw(
              runtime_error("TestDocParser::treatTest : "
                            "unsupported keyword '" +
                            key + "'"));
        }
        this->readSpecifiedToken(";", p);
    ...
    void TestDocParser::getStringValue(std::string& s,
      ...
      this->checkNotEndOfFile(p);
      if (!s.empty()) {
        throw(std::runtime_error("TestDocParser::getStringValue : '" + v +
    ...
    void TestDocParser::treatDescription(std::map<std::string, std::string>& dm,
      ...
      if (dm.find(l) != dm.end()) {
        throw(
            std::runtime_error("TestDocParser::treatDescription : "
      ...
      while (p->value != "}") {
        ...
        if (p->value == "@") {
          ...
          if (find(keys.begin(), keys.end(), key1) != keys.end()) {
            ...
            msg += "key '" + key1 + "' already defined for entry '" + key2 +
                   "' for test '" + n + "'";
