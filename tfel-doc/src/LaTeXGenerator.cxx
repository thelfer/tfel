...
namespace tfel
{
  ...
  namespace utilities
  {
    ...
    static void
    writeLaTexDescription(std::ostream& out,
	...
	out << " &:& \\textcolor{red}{" << getTranslation("unknown author",l) << "} \\\\"<< endl;
      }
      out << capitalize(getTranslation("date",l));
      if(!t.date.empty()){
	out << " &:& " << t.date   << "\\\\"<< endl;
      } else {
	out << " &:& \\textcolor{red}{" << getTranslation("unspecified date",l) << "} \\\\"<< endl;
      }
      if(!t.src.empty()){
	out << capitalize(getTranslation("path in source",l));
	out << " &:& \\texttt{\\$\\{srcdir\\}/" << toLaTeX(t.src) << "}\\\\"<< endl;
      }
      if(!t.install.empty()){
	out << capitalize(getTranslation("path in install",l));
	out << " &:& ";
	if(prefix.empty()){
	  out << "\\texttt{\\$\\{prefix\\}/" << toLaTeX(t.install) << "}\\\\"<< endl;
	} else {
    ...
    static void
    writeLaTeXEndOfDocument(std::ostream& out,
			    const std::string& l){
      out << "\\clearpage\n";
      out << "\\newpage\n";
    ...
    void
    writeLaTeXFile(std::ostream& out,
