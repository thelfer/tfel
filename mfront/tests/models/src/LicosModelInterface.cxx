...
namespace mfront{
  ...
  void
  MFrontModelInterface::generateOutputFiles(const FileDescription& fd,
					...
					const ModelDescription& md)
  {
	...
	applyHeaders.insert(static_cast<unsigned short>(f.usedVariables.size()));
      }
    }
    ...
    this->headerFile << "struct MFRONT_SHAREDOBJ " << md.className << endl;
	...
	if(f.usedVariables.size()<=5){
	  this->headerFile << ": public pleiades::field::ScalarFieldFunctor" << f.usedVariables.size() << endl;
	}
      }
      this->headerFile << "{\n\n";
      if((f.modifiedVariables.size()==1)&&
	...
	if(f.usedVariables.size()>5){
	  ...
	  for(i=1,p4=f.usedVariables.begin();p4!=f.usedVariables.end();++i,++p4){
	...
	if(f.usedVariables.size()==0){
	  ...
	  for(i=1,p4=f.usedVariables.begin();p4!=f.usedVariables.end();++i){
	...
	this->headerFile << getVariableType(p->type) << " " << p->name << ";\n";
      ...
      for(p4=f.constantMaterialProperties.begin();p4!=f.constantMaterialProperties.end();++p4){
	p = MFrontModelInterface::findVariableDescription(md.constantMaterialProperties,*p4);
	if(getDebugMode()){
	  this->headerFile << "#line " << p->lineNumber << " \"" 
			   << fd.fileName << "\"\n";
	}
    ...
    this->headerFile << "}; // end of struct " << md.className << endl << endl;
				     ...
				     const ModelDescription& md)
  {
	...
	for(i=0,p12=f.usedVariables.begin();p12!=f.usedVariables.end();++p12,++i){
	  this->srcFile << "const real " << *p12 << " = values[" << i<< "];\n";
	}
	for(i=0,p12=f.modifiedVariables.begin();p12!=f.modifiedVariables.end();++p12,++i){
      ...
      for(i=0,p12=f.usedVariables.begin();p12!=f.usedVariables.end();++p12,++i){
	found = false;
	for(p19=md.physicalBounds.begin();
	    (p19!=md.physicalBounds.end())&&(!found);){
	...
	found = false;
	for(p20=md.bounds.begin();
	    (p20!=md.bounds.end())&&(!found);){
	  found = (p20->varName==*p12);
	  if(!found){
	    ++p20;
	  }
	}
	if((p19!=md.physicalBounds.end())||
	   (p20!=md.bounds.end())){
	...
	if((p19!=md.physicalBounds.end())||(p20!=md.bounds.end())){
	...
	this->writeStaticVariableInitialization(fd,md,*p10);
      }
      this->srcFile << endl;
    }
    // Constructors
	...
	throw(runtime_error(msg));
      ...
      for(p15=md.domains.begin();p15!=md.domains.end();++p15){
    ...
    found = false;
    for(auto p11=md.functions.begin();(p11!=md.functions.end())&&(!found);++p11){
	...
	this->srcFile << "this->" << functor << ".dt=dt;\n";
      ...
      for(p15=f.usedVariables.begin();p15!=f.usedVariables.end();++p15){
	pair<string,unsigned short> dv = decomposeVariableName(md,*p15);
	this->srcFile << "const NFieldHolder& " << "f_" << *p15 
		      << " = mm.getNFieldHolder(" << this->getVariableName(dv.first,md)
		      << "," << dv.second << ");\n";
      }
      for(p15=f.modifiedVariables.begin();p15!=f.modifiedVariables.end();++p15){
	this->srcFile << "NFieldHolder& " << "f_" << *p15 
		      << " = mm.getNFieldHolder(" << this->getVariableName(*p15,md) << ");\n";
      }
      for(p15=f.constantMaterialProperties.begin();
	  p15!=f.constantMaterialProperties.end();++p15){
	this->srcFile << "this->" << functor << "." << *p15 
      ...
      this->srcFile << "<< \"executing function '" << f.name << "' on domain '\" << *ptr << \"'\" << endl;\n";
      this->srcFile << "}\n";
      if((f.modifiedVariables.size()==1)&&
	 (f.usedVariables.size()<TFEL_MFRONTPLEAIDESPARSER_MAXUSEDVARIABLESFORUSINGAPPLY)){
	string output = *(f.modifiedVariables.begin());
	if(f.usedVariables.size()<=5){
	...
	if(f.usedVariables.size()!=0){
	  this->srcFile << ",\n";
	  for(p15=f.usedVariables.begin();p15!=f.usedVariables.end();){
	    this->srcFile << "f_" << *p15;
	    if(++p15!=f.usedVariables.end()){
	      this->srcFile << ",";
	    }
	  }
	}
	...
	if(f.usedVariables.size()!=0){
	  ...
	  for(p15=f.modifiedVariables.begin();p15!=f.modifiedVariables.end();++p15){
	    ...
	    this->srcFile << "f_" << *p15 << " = NFieldHolder(shared_ptr<VNField<real> >(new VNField<real>(support,"
			  << this->getVariableName(*p15,md) <<",f_"
			  << *p15 << ".get<real>())));\n";
	    this->srcFile << "}\n";
	  }
	  ...
	  for(i=0,p15=f.usedVariables.begin();p15!=f.usedVariables.end();++p15,++i){
	    this->srcFile << "if(f_" << *p15<< ".is<VNField<real> >()){\n";
	  ...
	  this->srcFile << "}\n";
	  for(i=0,p15=f.modifiedVariables.begin();p15!=f.modifiedVariables.end();++p15,++i){
	  ...
	  for(i=0,p15=f.modifiedVariables.begin();p15!=f.modifiedVariables.end();++p15,++i){
	    this->srcFile << "f_" << *p15 << " = NFieldHolder(UNField<real>(support,"
			  << this->getVariableName(*p15,md) << ",results[" << i <<"]));\n";
	  }
	  this->srcFile << "}\n";
	} else {
	  // no input variables
