...
namespace mfront {
  ...
  MFrontGerminalModelInterface::treatKeyword(const std::string& k,
    ...
    if (std::find(i.begin(), i.end(), "germinal") != i.end()) {
      throw(
          std::runtime_error("MFrontGerminalModelInterface::treatKeyword: "
                             "unsupported key '" +
                             k + "'"));
    }
    return {false, c};
  ...
  void MFrontGerminalModelInterface::writeGetConstantMaterialProperty(
    ...
    if (md.hasGlossaryName(v.name)) {
      ...
      name2 = "this->getMeshZoneName()+\".\"+GlossaireParam::" +
              md.getGlossaryName(v.name);
    } else {
  ...
  void MFrontGerminalModelInterface::writeInitializeMethod(
    ...
    for (const auto& v : md.outputs) {
      ...
      if (md.hasGlossaryName(v.name)) {
        ...
        iname = "GlossaireField::" + md.getGlossaryName(v.name) +
                "+\".InitialValue\"";
      } else {
        name = "\"" + md.getExternalName(v.name) + "\"";
