  ...
  : public tfel::tests::TestCase
{
  ...
  execute() override
  {
    ...
    this->check("DefaultDSL","@Material m;"
		"@Material m2;");
    // multiple call to @Material
    this->check("DefaultDSL","@Material m;"
		"@Material m2;");
    ...
    for(const auto& r : {"eto","sig","deto","T","dT","dt",
      ...
      for(const auto& t : {"MaterialProperty","StateVariable",
	    ...
	    "StaticVariable"}){
	this->check("DefaultDSL","@"+std::string(t)+" real "+std::string(r)+";");
      }
    ...
    this->check("DefaultDSL","@RequiresStiffnessTensor;"
		"@MaterialProperty real D;");
    for(const auto& dsl : {"MaterialLaw","Model"}){
      this->check(dsl,"@Input a;"
		  "@Input a;");
    }
    // unknown state variable method test
    this->check("DefaultDSL","@StateVariable real a;"
