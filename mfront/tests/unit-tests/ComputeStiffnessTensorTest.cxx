...
struct ComputeStiffnessTensorTest final
  : public tfel::tests::TestCase
{
 ...
 private:
  void test1(){
    ...
    this->check("Implicit","@OrthotrophicBehaviours;"
		"@ComputeStiffnessTensor {150e9,0.3};");
    this->check("Implicit","@ComputeStiffnessTensor {150e9,0.3};"
		"@MaterialProperty real nu;"
		"nu.setGlossaryName(\"PoissonRatio\");");
    this->check("Implicit","@MaterialProperty real nu;"
		"nu.setGlossaryName(\"PoissonRatio\");"
		"@ComputeStiffnessTensor {150e9,0.3};");
    this->check("Implicit","@OrthotropicBehaviour;"
		"@ComputeStiffnessTensor {150e9,,0.3};");
    this->check("Implicit",
  ...
  void test3(){
    ...
    auto& f = mfront::DSLFactory::getDSLFactory();
    auto dsl = std::dynamic_pointer_cast<mfront::AbstractBehaviourDSL>(f.createNewParser("Implicit"));
    dsl->analyseString("@OrthotropicBehaviour;"
		       "@ComputeStiffnessTensor {150e9,150e9,150e9,"
		       "                         0.3,0.3,0.3,"
		       "                         150e9,150e9,150e9};");
    const auto& bd = dsl->getBehaviourDescription().getBehaviourData(h);
    TFEL_TESTS_ASSERT(bd.getParameters().size()==9u);
    for(const auto& v: std::map<std::string,std::string>{
	{"young1","YoungModulus1"},{"young2","YoungModulus2"},
	{"young3","YoungModulus3"},{"nu12","PoissonRatio12"},
	{"nu23","PoissonRatio23"},{"nu13","PoissonRatio13"},
	{"mu12","ShearModulus12"},{"mu23","ShearModulus23"},
	{"mu13","ShearModulus13"}}){
