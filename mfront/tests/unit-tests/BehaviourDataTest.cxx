...
struct BehaviourDataTest final
...
private:
  ...
  void test2(){
    ...
    TFEL_TESTS_ASSERT(bd.getExternalNames(bd.getMaterialProperties()).size()==2u);
    {
      ...
      TFEL_TESTS_CHECK_THROW(bd.setGlossaryName("nu","SofteningSlope"), runtime_error);
      // PoissonRatio can't be used as an entry name
      TFEL_TESTS_CHECK_THROW(bd.setEntryName("nu",Glossary::PoissonRatio), runtime_error);
    ...
    {
      // Softening slople can't be used as a glossary name
      TFEL_TESTS_CHECK_THROW(bd.setGlossaryName("p","SofteningSlope"), runtime_error);
      // PoissonRatio can't be used as an entry name
      TFEL_TESTS_CHECK_THROW(bd.setEntryName("p",Glossary::PoissonRatio), runtime_error);
      // YoungModulus is already used
      TFEL_TESTS_CHECK_THROW(bd.setGlossaryName("p",Glossary::YoungModulus), runtime_error);
    }
    bd.setEntryName("p","AStateVariable");
    {
      ...
      TFEL_TESTS_ASSERT(find(enames.begin(),enames.end(),Glossary::YoungModulus)!=enames.end());
      TFEL_TESTS_ASSERT(find(enames.begin(),enames.end(),Glossary::PoissonRatio)!=enames.end());
    }
    {
    ...
    {
      // Softening slople can't be used as a glossary name
      TFEL_TESTS_CHECK_THROW(bd.setGlossaryName("seq","SofteningSlope"), runtime_error);
      // PoissonRatio can't be used as an entry name
      TFEL_TESTS_CHECK_THROW(bd.setEntryName("seq",Glossary::PoissonRatio), runtime_error);
      // YoungModulus is already used
      TFEL_TESTS_CHECK_THROW(bd.setGlossaryName("seq",Glossary::YoungModulus), runtime_error);
      // p is already used
      TFEL_TESTS_CHECK_THROW(bd.setEntryName("seq","p"), runtime_error);
      // AStateVariable is already used
      TFEL_TESTS_CHECK_THROW(bd.setEntryName("seq","AStateVariable"), runtime_error);
    }
    ...
    TFEL_TESTS_ASSERT(bd.getVariableNameFromGlossaryNameOrEntryName(Glossary::YoungModulus)=="young");
    TFEL_TESTS_ASSERT(bd.getVariableNameFromGlossaryNameOrEntryName(Glossary::PoissonRatio)=="nu");
    TFEL_TESTS_ASSERT(bd.getVariableNameFromGlossaryNameOrEntryName("AStateVariable")=="p");
    TFEL_TESTS_ASSERT(bd.getVariableNameFromGlossaryNameOrEntryName("SomeStressNorm")=="seq");
    TFEL_TESTS_CHECK_THROW(bd.getVariableNameFromGlossaryNameOrEntryName("unknown"),runtime_error);
    TFEL_TESTS_CHECK_THROW(bd.getPersistentVariableHandler("unknown"),runtime_error);
    TFEL_TESTS_CHECK_THROW(bd.getIntegrationVariableHandler("unknown"),runtime_error);
    TFEL_TESTS_CHECK_THROW(bd.getStateVariableHandler("unknown"),runtime_error);
    TFEL_TESTS_CHECK_THROW(bd.getVariableHandler(bd.getMaterialProperties(),"unknown"),runtime_error);
    TFEL_TESTS_CHECK_THROW(bd.getVariableHandler(bd.getLocalVariables(),"unknown"),runtime_error);
    TFEL_TESTS_ASSERT(bd.getVariableHandler(bd.getMaterialProperties(),"young").name=="young");
    TFEL_TESTS_ASSERT(bd.getVariableHandler(bd.getMaterialProperties(),"young").type=="stress");
    TFEL_TESTS_ASSERT(bd.getVariableHandler(bd.getMaterialProperties(),"nu").name=="nu");
    TFEL_TESTS_ASSERT(bd.getVariableHandler(bd.getMaterialProperties(),"nu").type=="real");
  ...
  void test3(void){
    ...
    TFEL_TESTS_CHECK_THROW(bd.getAttribute<unsigned short>("attr1"),runtime_error);
    ...
    bd.setAttribute("attr1",string("value1"),true);
    TFEL_TESTS_ASSERT(bd.getAttribute<string>("attr1")=="value2");
    TFEL_TESTS_CHECK_THROW(bd.setAttribute("attr1",string("value1"),false),runtime_error);
