...
namespace umat{
    ...
    : protected UMATInterfaceBase
  {
	       ...
	       UMATInt  *const KINC)
    {
      ...
      if(*NDI==2){
	UMATInterface::callBehaviour<MH::TRIDIMENSIONAL>(NTENS,DTIME,DROT,DDSOE,STRAN,DSTRAN,
							 TEMP,DTEMP,PROPS,NPROPS,PREDEF,DPRED,
							 STATEV,NSTATV,STRESS,KINC);
      } else if(*NDI==0){
	UMATInterface::callBehaviour<MH::AXISYMMETRICAL>(NTENS,DTIME,DROT,DDSOE,STRAN,DSTRAN,
							 TEMP,DTEMP,PROPS,NPROPS,PREDEF,DPRED,
							 STATEV,NSTATV,STRESS,KINC);
      } else if(*NDI==-1){
	UMATInterface::callBehaviour<MH::PLANESTRAIN>(NTENS,DTIME,DROT,DDSOE,STRAN,DSTRAN,
						      TEMP,DTEMP,PROPS,NPROPS,PREDEF,DPRED,
						      STATEV,NSTATV,STRESS,KINC);
      } else if(*NDI==-2){
	UMATInterface::callBehaviour<MH::PLANESTRESS>(NTENS,DTIME,DROT,DDSOE,STRAN,DSTRAN,
						      TEMP,DTEMP,PROPS,NPROPS,PREDEF,DPRED,
						      STATEV,NSTATV,STRESS,KINC);
      } else if(*NDI==-3){
	UMATInterface::callBehaviour<MH::GENERALISEDPLANESTRAIN>(NTENS,DTIME,DROT,DDSOE,STRAN,DSTRAN,
								 TEMP,DTEMP,PROPS,NPROPS,PREDEF,DPRED,
								 STATEV,NSTATV,STRESS,KINC);
      } else if(*NDI==14){
	UMATInterface::callBehaviour<MH::AXISYMMETRICALGENERALISEDPLANESTRAIN>(NTENS,DTIME,DROT,DDSOE,STRAN,DSTRAN,
									       TEMP,DTEMP,PROPS,NPROPS,PREDEF,DPRED,
									       STATEV,NSTATV,STRESS,KINC);
      } else {
