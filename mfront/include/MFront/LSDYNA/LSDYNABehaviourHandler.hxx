...
namespace lsdyna
{
    ...
    : public LSDYNAInterfaceExceptions
  {
    ...
    struct TFEL_VISIBILITY_LOCAL StiffnessOperatorInitializer
    {
      ...
      TFEL_LSDYNA_INLINE static void
	exe(BData& data,const LSDYNAReal * const props){
	...
	LSDYNAComputeStiffnessTensor<LSDYNATraits<BV>::btype,H,
				     LSDYNATraits<BV>::etype,buas>::exe(data.getStiffnessTensor(),
									props);
      } // end of exe
	...
	LSDYNAComputeThermalExpansionCoefficientTensor<LSDYNATraits<BV>::btype,H,
						       LSDYNATraits<BV>::stype>::exe(props+o,
    ...
    struct TFEL_VISIBILITY_LOCAL DoNothingInitializer
    {
      ...
      typedef typename BV::BehaviourData  BData;
      TFEL_LSDYNA_INLINE static void
	exe(BData&,const LSDYNAReal * const)
      {}
	...
	: behaviour(&(d.DTIME),d.TEMP,d.DTEMP,
	 ...
	 this->behaviour.setLSDYNABehaviourDataThermodynamicForces(d.STRESS,d.DROT);
	...
	auto r = BV::SUCCESS;
	const auto smflag = LSDYNATangentOperatorFlag<LSDYNATraits<BV>::btype>::value;
	...
	*(d.PNEWDT) = tsf.second;
    ...
    TFEL_LSDYNA_INLINE2 static void
    checkNPROPS(const LSDYNAInt NPROPS)
    {
      using namespace tfel::material;
      typedef Behaviour<H,LSDYNAReal,false> BV;
      typedef MechanicalBehaviourTraits<BV> Traits;
      const unsigned short offset  = (LSDYNATraits<BV>::elasticPropertiesOffset+
				      LSDYNATraits<BV>::thermalExpansionPropertiesOffset);
    ...
    TFEL_LSDYNA_INLINE2 static void
    checkNSTATV(const LSDYNAInt NSTATV)
    {
      typedef Behaviour<H,LSDYNAReal,false> BV;
      typedef tfel::material::MechanicalBehaviourTraits<BV> Traits;
    ...
    TFEL_LSDYNA_INLINE2 static void
    checkNTENS(const LSDYNAInt ntens)
    {
      typedef Behaviour<H,LSDYNAReal,false> BV;
      typedef tfel::material::MechanicalBehaviourTraits<BV> Traits;
