...
namespace abaqus
{
  ...
  struct TFEL_VISIBILITY_LOCAL AbaqusRotation2D
  {
    ...
    void rotateStressesForward(const real *const,
			             real *const);
    // Compute stresses back in the global space
    void rotateStressesBackward(const real *const,
                                      real *const);
    // compute the deformation dradient in the material space
    void rotateDeformationGradientForward(const real *const,
					         real *const);
    // Compute strain in the material space
    void rotateStrainsForward(const real *const,
                                    real *const);
    // Compute stresses back in the global space
    void rotateStrainsBackward(const real *const,
                                     real *const);
