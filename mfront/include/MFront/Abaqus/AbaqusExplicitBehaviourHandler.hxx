/*! 
 * \file  mfront/include/MFront/Abaqus/AbaqusExplicitBehaviourHandler.hxx
 * \brief
...
#ifndef LIB_MFRONT_ABAQUS_ABAQUSEXPLICITBEHAVIOURHANDLER_H_
#define LIB_MFRONT_ABAQUS_ABAQUSEXPLICITBEHAVIOURHANDLER_H_ 
...
namespace abaqus
{
  ...
  struct TFEL_VISIBILITY_LOCAL AbaqusExplicitBehaviourHandler
    : public AbaqusInterfaceExceptions
  {
      ...
      struct TFEL_VISIBILITY_LOCAL Integrator
    {
      ...
      TFEL_ABAQUS_INLINE Integrator(const AbaqusExplicitData& d)
	: behaviour(&(d.DTIME),d.TEMP,d.DTEMP,
	 ...
	 this->behaviour.initialize();
      ...
      TFEL_ABAQUS_INLINE2
      void exe(const AbaqusExplicitData& d)
      {
	using namespace tfel::material;
	typedef MechanicalBehaviourTraits<BV> Traits;
	typedef typename std::conditional<
	...
	*(d.PNEWDT) = tsf.second;
    ...
    TFEL_ABAQUS_INLINE2 static void
    checkNFIELD(const AbaqusInt NFIELD)
    {
      typedef Behaviour<H,AbaqusReal,false> BV;
      typedef tfel::material::MechanicalBehaviourTraits<BV> Traits;
      const unsigned short nfield  = Traits::external_variables_nb;
      const bool is_defined_       = Traits::is_defined;
      //Test if the nb of state variables matches Behaviour requirements
      if((nfield!=NFIELD)&&is_defined_){
	throwUnMatchedNumberOfExternalStateVariables(Traits::getName(),
						     nfield,NFIELD);
      }
    } // end of checkNFIELD
    
  }; // end of struct AbaqusExplicitBehaviourHandler
  
} // end of namespace abaqus

#endif /* LIB_MFRONT_ABAQUS_ABAQUSEXPLICITBEHAVIOURHANDLER_H_ */
