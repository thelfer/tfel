...
namespace ansys
{
    ...
    : public AnsysInterfaceExceptions
  {
    ...
    struct TFEL_VISIBILITY_LOCAL StiffnessOperatorInitializer
    {
      ...
      TFEL_ANSYS_INLINE static void
	exe(BData& data,const AnsysReal * const props){
	...
	AnsysComputeStiffnessTensor<AnsysTraits<BV>::btype,H,
				     AnsysTraits<BV>::etype,buas>::exe(data.getStiffnessTensor(),
									props);
      } // end of exe
	...
	const unsigned short o =
	  AnsysTraits<BV>::elasticPropertiesOffset;
	AnsysComputeThermalExpansionCoefficientTensor<AnsysTraits<BV>::btype,H,
						       AnsysTraits<BV>::stype>::exe(props+o,
    ...
    struct TFEL_VISIBILITY_LOCAL DoNothingInitializer
    {
      ...
      typedef typename BV::BehaviourData  BData;
      TFEL_ANSYS_INLINE static void
	exe(BData&,const AnsysReal * const)
      {}
	...
	: behaviour(&(d.DTIME),d.TEMP,d.DTEMP,
	 ...
	 this->behaviour.setANSYSBehaviourDataThermodynamicForces(d.STRESS,d.DROT);
	...
	auto r = BV::SUCCESS;
	const auto smflag = AnsysTangentOperatorFlag<AnsysTraits<BV>::btype>::value;
	try{
    ...
    checkNPROPS(const AnsysInt NPROPS)
    {
      using namespace std;
      using namespace tfel::material;
      typedef Behaviour<H,AnsysReal,false> BV;
    ...
    TFEL_ANSYS_INLINE2 static void
    checkNSTATV(const AnsysInt NSTATV)
    {
      typedef Behaviour<H,AnsysReal,false> BV;
      typedef tfel::material::MechanicalBehaviourTraits<BV> Traits;
