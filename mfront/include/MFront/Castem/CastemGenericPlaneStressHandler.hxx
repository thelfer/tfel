...
#ifndef LIB_MFRONT_CASTEM_CASTEMGENERICPLANESTRESSHANDLER_HXX
#define LIB_MFRONT_CASTEM_CASTEMGENERICPLANESTRESSHANDLER_HXX
...
namespace castem {
  ...
  struct CastemGenericPlaneStressHandler : public CastemInterfaceExceptions {
     ...
     */
    TFEL_CASTEM_INLINE2 static void exe(const CastemReal *const DTIME,
      ...
      Handler::exe(DTIME, DROT, DDSDDE, STRAN, DSTRAN, TEMP, DTEMP, PROPS, NPROPS, PREDEF, DPRED,
   ...
   private:
    ...
    TFEL_CASTEM_INLINE2 static void exe2(const CastemReal c1,
                                         const CastemReal c2,
                                         const CastemReal c3,
      ...
      typedef tfel::material::MechanicalBehaviourTraits<BV> Traits;
      const unsigned short NSTATV_ = Traits::internal_variables_nb + 1u;
      typedef typename std::conditional<(NSTATV_ < 20), tfel::math::tvector<NSTATV_, CastemReal>,
      ...
      CastemGenericPlaneStressHandler::template iter<GeneralisedPlaneStrainBehaviour>(
          DTIME, DROT, DDSDDE, TEMP, DTEMP, PROPS, NPROPS, PREDEF, DPRED, STATEV, STRESS, PNEWDT,
          STRAN, DSTRAN, dez, &v[0], s, eto, deto, op, sfeh);
      x[1] = dez;
      f[1] = s[2];
      ...
      i = 2;
      while ((abs(c3 * s[2]) > 1.e-12) && (i < iterMax)) {
        x[0] = x[1];
        f[0] = f[1];
        x[1] = dez;
        f[1] = s[2];
    ...
    template <typename GeneralisedPlaneStrainBehaviour>
    TFEL_CASTEM_INLINE2 static void iter(const CastemReal *const DTIME,
                                         const CastemReal *const DROT,
                                         CastemReal *const DDSDDE,
                                         const CastemReal *const TEMP,
                                         const CastemReal *const DTEMP,
      ...
      typedef Behaviour<H, CastemReal, false> BV;
      typedef tfel::material::MechanicalBehaviourTraits<BV> Traits;
      const CastemInt nNSTATV =
          Traits::internal_variables_nb == 0 ? 1 : Traits::internal_variables_nb;
      const unsigned short NSTATV_ = Traits::internal_variables_nb + 1u;
      copy<4>::exe(STRESS, s);
      ...
      GeneralisedPlaneStrainBehaviour::exe(DTIME, DROT, DDSDDE, eto, deto, TEMP, DTEMP, PROPS,
                                           NPROPS, PREDEF, DPRED, v, &nNSTATV, s, PNEWDT, op, sfeh);
    }
    ...
    struct TreatPlaneStressIsotropicBehaviour {
      TFEL_CASTEM_INLINE2 static void exe(const CastemReal *const DTIME,
        ...
        CastemGenericPlaneStressHandler::template exe2<BehaviourHandler>(
            c1, c1, c3, DTIME, DROT, DDSDDE, STRAN, DSTRAN, TEMP, DTEMP, nPROPS, &nb, PREDEF,
    ...
    struct TreatPlaneStressOrthotropicBehaviour {
      TFEL_CASTEM_INLINE2 static void exe(const CastemReal *const DTIME,
        ...
        typedef Behaviour<H, CastemReal, false> BV;
        typedef CastemOrthotropicBehaviourHandler<STANDARDSTRAINBASEDBEHAVIOUR, H, Behaviour>
            BehaviourHandler;
        constexpr const auto offset = CastemTraits<BV>::propertiesOffset;
        constexpr const auto nprops = MechanicalBehaviourTraits<BV>::material_properties_nb;
        ...
        const CastemReal detS = S11 * S22 * S33 + 2 * S23 * S13 * S12 - S11 * S23 * S23 -
        ...
        CastemGenericPlaneStressHandler::template exe2<BehaviourHandler>(
            c1, c2, S22, DTIME, DROT, DDSDDE, STRAN, DSTRAN, TEMP, DTEMP, nPROPS, NPROPS, PREDEF,
            DPRED, STATEV, NSTATV, STRESS, PNEWDT, op, sfeh);
      }  // end of exe
...
#endif /* LIB_MFRONT_CASTEM_CASTEMGENERICPLANESTRESSHANDLER_HXX */
