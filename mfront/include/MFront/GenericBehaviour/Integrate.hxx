...
namespace mfront {
  ...
  namespace gb {
    ...
    {
      template <typename Behaviour>
      static inline bool exe(mfront_gb_BehaviourData& d,
			     Behaviour& b){
	exportTangentOperator(d.K, b.getTangentOperator());
	return true;
      }
	...
	typename std::conditional<MTraits::hasComputeDissipatedEnergy,
      ...
      try {
        ...
        if (Ke < -0.25) {
          if (bs) {
	    *(d.speed_of_sound) =
              b.computeSpeedOfSound(*(d.s0.mass_density));
	  }
	  PredictionOperatorComputer<MTraits::hasPredictionOperator>::exe(d,b,f);
	}
	const auto smt = [&Ke] {
			   if (Ke < 0.5) {
	...
	if (Ke > 0.5) {
	  if(!ConsistentTangentOperatorComputer<MTraits::hasConsistentTangentOperator>::exe(d,b)){
	    return -1;
	  }
	}
