...
namespace dianafea
{
    ...
    : protected DianaFEAInterfaceExceptions
  {
    ...
    TFEL_DIANAFEA_INLINE2 static
    int exe(const DianaFEAData& d)
    {
	...
	DianaFEAInterfaceExceptions::treatDianaFEAException(MTraits::getName(),e);
	return -2;
      }
      catch(const tfel::material::OutOfBoundsException& e){
	DianaFEAInterfaceExceptions::treatMaterialException(MTraits::getName(),e);
	return -3;
      }
	...
	DianaFEAInterfaceExceptions::treatStandardException(MTraits::getName(),e);
	return -7;
      }
      catch(...){
  ...
  private:
    ...
    {
      TFEL_DIANAFEA_INLINE2 static void
      exe(const DianaFEAData&)
      {
	using BV = Behaviour<H,DianaFEAReal,false>;
	using MTraits = tfel::material::MechanicalBehaviourTraits<BV>;
