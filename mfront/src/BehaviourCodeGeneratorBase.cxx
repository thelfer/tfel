...
namespace mfront {
  ...
  void BehaviourCodeGeneratorBase::checkArraySizeOfMainVariables(
      const std::string_view m,
      const std::pair<Gradient, ThermodynamicForce>& mv) {
    if (mv.first.arraySize != mv.second.arraySize) {
      tfel::raise(std::string{m} + ": the gradient '" + mv.first.name +
                  "' and the thermodynamic force '" + mv.second.name +
                  "' don't have the same array size");
    }
  }  // end of checkArraySizeOfMainVariables
    ...
    for (const auto& v : this->bd.getMainVariables()) {
      ...
      for (unsigned short idx = 0; idx != v.first.arraySize; ++idx) {
        ...
        if (Gradient::isIncrementKnown(v.first)) {
          os << "os << \"" << displayName(v.first) << aidx << " : \" << b."
             << v.first.name << aidx << " << '\\n';\n";
          ...
          if (getUnicodeOutputOption()) {
            os << "os << \"" << displayName(v.first) << "\u2080" << aidx
               << " : \" << b." << v.first.name << "0" << aidx << " << '\\n';\n"
               << "os << \"" << displayName(v.first) << "\u2081" << aidx
               << " : \" << b." << v.first.name << "1" << aidx
               << " << '\\n';\n";
          } else {
            os << "os << \"" << displayName(v.first) << "0" << aidx
               << " : \" << b." << v.first.name << "0" << aidx << " << '\\n';\n"
               << "os << \"" << displayName(v.first) << "1" << aidx
               << " : \" << b." << v.first.name << "1" << aidx
               << " << '\\n';\n";
          }
        }
