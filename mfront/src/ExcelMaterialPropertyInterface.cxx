...
namespace mfront
{
  ...
  static void
  readExcelInterface(std::vector<ExcelInterface>& interfaces,
						   ...
						   const FileDescription&)
  {
    ...
    const auto name  = (mpd.material.empty() ? "" : mpd.material+"_")+ mpd.className;
    struct stat buffer; // for call to stat
    if(access(fname.c_str(),F_OK)==0){
      throw_if(stat(fname.c_str(),&buffer)==-1,"can't stat file '"+fname+"'");
      throw_if(!S_ISREG(buffer.st_mode),"'"+fname+"' is not a regular file");
	...
	file << "Declare Function " << f.function 
	     << " Lib \"" << libName << ".dll\" (";
