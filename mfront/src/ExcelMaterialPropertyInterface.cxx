...
namespace mfront {
    ...
    if (!line.empty()) {
      ...
      if (tokens.size() < 2) {
        throw(std::runtime_error(
            "ExcelMaterialPropertyInterface::readExcelInterface: "
            "invalid line '" +
            line + "'"));
      }
      ExcelInterface i;
  ...
  ExcelMaterialPropertyInterface::treatKeyword(
    ...
    if (std::find(i.begin(), i.end(), "excel") != i.end()) {
      throw(
          std::runtime_error("ExcelMaterialPropertyInterface::treatKeyword: "
                             "unsupported key '" +
                             k + "'"));
    }
  ...
  void ExcelMaterialPropertyInterface::getTargetsDescription(
      TargetsDescription&, const MaterialPropertyDescription&) {
  }  // end of ExcelMaterialPropertyInterface::getTargetsDescription

  void ExcelMaterialPropertyInterface::writeOutputFiles(
      const MaterialPropertyDescription& mpd, const FileDescription&) {
    ...
    const auto lib =
        "Excel" + getMaterialLawLibraryNameBase(mpd.library, mpd.material);
