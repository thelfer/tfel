...
namespace mfront {
  ...
  void JavaMaterialPropertyInterface::getTargetsDescription(
    ...
    const auto lib =
        getMaterialLawLibraryNameBase(mpd.library, mpd.material) + "-java";
  ...
  void JavaMaterialPropertyInterface::writeOutputFiles(
    ...
    srcFile
        ...
        << "};\n";
    // material laws
    writeMaterialLaws("JavaMaterialPropertyInterface::writeOutputFile", srcFile,
                      mpd.materialLaws);
    // static variables
    ...
    for (const auto& p : mpd.parameters) {
      ...
      srcFile << "static " << constexpr_c << " double " << p << " = "
              << p6->second << ";\n";
    }
    ...
    srcFile
        << "jdouble " << mpd.output << ";\n"
        << "try{\n"
        << mpd.f.body << "} catch(exception& cpp_except){\n"
