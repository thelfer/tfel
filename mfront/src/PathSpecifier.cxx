...
namespace mfront {
  ...
  bool parsePathSpecifierArguments(std::vector<PathSpecifier>& paths,
    ...
    auto set_behaviour_identifier = [&s](const std::string& n) {
    ...
    auto set_model_identifier = [&s](const std::string& n) {
    ...
    if (a[0] != '-') {
      ...
      set_behaviour_identifier(get_substring("--behaviour="));
    } else if (a == "--all-models") {
      set_model_identifier(".+");
    } else if (tfel::utilities::starts_with(a, "--model=")) {
      set_model_identifier(get_substring("--model="));
