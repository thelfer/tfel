...
namespace mfront{
				     ...
				     const bool b)
  {
	...
	f << "this->"  << n  << "_ = this->"  << n  << "+" << p << "*(this->" << n << "1-this->" << n << "0);\n";
      }
    }
  }
  ...
  RungeKuttaDSLBase::RungeKuttaDSLBase()
  {
    ...
    this->registerNewCallBack("@MaterialLaw",&RungeKuttaDSLBase::treatMaterialLaw);
  ...
  void RungeKuttaDSLBase::writeBehaviourLocalVariablesInitialisation(std::ostream& os,
  ...
  void RungeKuttaDSLBase::writeBehaviourParserSpecificTypedefs(std::ostream& os) const
  {
    this->checkBehaviourFile(os);
  }

  void RungeKuttaDSLBase::writeBehaviourParserSpecificMembers(std::ostream& os,
							      const Hypothesis h) const
  {
    this->checkBehaviourFile(os);
  ...
  void RungeKuttaDSLBase::writeBehaviourUpdateStateVariables(std::ostream&,
							     const Hypothesis) const
  {
    // Disabled (makes no sense for this parser)
  } // end of writeBehaviourUpdateStateVariables

  void RungeKuttaDSLBase::writeBehaviourUpdateAuxiliaryStateVariables(std::ostream& os,
								      const Hypothesis h) const
  {
  ...
  void RungeKuttaDSLBase::writeBehaviourEulerIntegrator(std::ostream& os,
							const Hypothesis h) const
  {
    const auto& d = this->mb.getBehaviourData(h);
    if(this->mb.hasCode(h,BehaviourData::ComputeStress)){
  ...
  void RungeKuttaDSLBase::writeBehaviourRK2Integrator(std::ostream& os,
						      const Hypothesis h) const
  {
    const auto& d = this->mb.getBehaviourData(h);
    auto uvs = d.getCodeBlock(BehaviourData::ComputeDerivative).members;
    ...
    for(const auto& iv : d.getStateVariables()){
      if(uvs.find(iv.name)!=uvs.end()){
	os << "this->" << iv.name << "_ += cste1_2*(this->d" << iv.name << "_K1);\n";
      }
    }
  ...
  void RungeKuttaDSLBase::writeBehaviourRK54Integrator(std::ostream& os,
						       const Hypothesis h) const
  {
    ...
    if(shallUpdateInternalStateValues){
      os << "TFEL_CONSTEXPR const auto cste3544_2565   = Type(3544)/Type(2565);\n"
	 << "TFEL_CONSTEXPR const auto cste11_40       = Type(11)/Type(40);\n"
	 << "TFEL_CONSTEXPR const auto cste1859_4104   = Type(1859)/Type(4104);\n"
	 << "TFEL_CONSTEXPR const auto cste8_27        = Type(8)/Type(27);\n"	
	 << "TFEL_CONSTEXPR const auto cste845_4104    = Type(845)/Type(4104);\n"
	 << "TFEL_CONSTEXPR const auto cste3680_513    = Type(3680)/Type(513);\n"
    ...
    os << "TFEL_CONSTEXPR const auto cste1_4         = real{1}/real{4};\n"
       ...
       << "TFEL_CONSTEXPR const auto cste6656_12825  = Type(6656)/Type(12825);\n"
       << "TFEL_CONSTEXPR const auto cste28561_56430 = Type(28561)/Type(56430);\n"
       ...
       << "TFEL_CONSTEXPR const auto cste2197_75240  = Type(2197)/Type(75240);\n"
    ...
    for(const auto& v: d.getStateVariables()){
      if(uvs.find(v.name)!=uvs.end()){
	os << "this->" << v.name << "_ += cste1_4*(this->d" << v.name << "_K1);\n";
      }
    }
							   ...
							   const Hypothesis h) const
  {
    ...
    for(const auto& v : d.getStateVariables()){
      if(uvs.find(v.name)!=uvs.end()){
	os << "this->" << v.name << "_ += cste1_2*(this->d" << v.name << "_K1);\n";
      }
    }
    writeExternalVariablesCurrentValues(os,this->mb,h,"cste1_2");
    if((this->mb.getAttribute<bool>(BehaviourDescription::computesStiffnessTensor,false))&&
  ...
  void RungeKuttaDSLBase::writeBehaviourRK42Integrator(std::ostream& os,
						       const Hypothesis h) const
  {
    auto get_enf = [](const VariableDescription& v){
      return v.getAttribute<std::string>(VariableDescription::errorNormalisationFactor);      
  ...
  void RungeKuttaDSLBase::writeBehaviourRK4Integrator(std::ostream& os,
						      const Hypothesis h) const
  {
    const auto& d = this->mb.getBehaviourData(h);
    auto uvs = d.getCodeBlock(BehaviourData::ComputeDerivative).members;
    ...
    for(const auto& iv : d.getStateVariables()){
      if(uvs.find(iv.name)!=uvs.end()){
	os << "this->" << iv.name << "_ = " << "this->" << iv.name
						   ...
						   const Hypothesis h) const
  {
    ...
    if(this->mb.getAttribute(BehaviourData::profiling,false)){
      writeStandardPerformanceProfilingBegin(os,
					     mb.getClassName(),
    ...
    if(algorithm == "Euler"){
      ...
      this->throwRuntimeError("RungeKuttaDSLBase::writeBehaviourIntegrator",
			      "internal error\n'"+
