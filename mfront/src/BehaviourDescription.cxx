...
namespace mfront {
  ...
  std::vector<std::string> BehaviourDescription::AnalyticMaterialProperty::getVariablesNames() const {
    return tfel::math::Evaluator(this->f).getVariablesNames();
  }  // end of AnalyticMaterialProperty::getVariablesNames

  template <typename Arg1>
  void BehaviourDescription::callBehaviourData(const Hypothesis h,
  ...
  static void checkElasticMaterialProperty(BehaviourDescription& bd,
  ...
  static void checkThermalExpansionCoefficientArgument(BehaviourDescription& bd,
  ...
  const char* const BehaviourDescription::requiresStiffnessTensor = "requiresStiffnessTensor";

  const char* const BehaviourDescription::computesStiffnessTensor = "computesStiffnessTensor";

  const char* const BehaviourDescription::requiresUnAlteredStiffnessTensor = "requiresUnAlteredStiffnessTensor";

  const char* const BehaviourDescription::requiresThermalExpansionCoefficientTensor =
  ...
  bool BehaviourDescription::allowsNewUserDefinedVariables() const {
    ...
    return this->getAttribute(h, BehaviourData::allowsNewUserDefinedVariables, true);
  ...
  const BehaviourData& BehaviourDescription::getBehaviourData(const Hypothesis& h) const {
  ...
  BehaviourData& BehaviourDescription::getBehaviourData2(const ModellingHypothesis::Hypothesis& h) {
    // check that the given hypothesis is supported
    if (h == ModellingHypothesis::UNDEFINEDHYPOTHESIS) {
      return this->d;
    }
  ...
  BehaviourDescription::MaterialPropertyInput::Category BehaviourDescription::getMaterialPropertyInputCategory(
  ...
  std::vector<BehaviourDescription::MaterialPropertyInput> BehaviourDescription::getMaterialPropertyInputs(
    ...
    for (const auto& v : i) {
      if (v == "T") {
        inputs.push_back({"T", tfel::glossary::Glossary::Temperature, MaterialPropertyInput::TEMPERATURE});
      } else {
        ...
        if (t == MaterialPropertyInput::STATICVARIABLE) {
          ...
          for (const auto h : hs) {
            ...
            throw_if(vd2.getExternalName() != en, "the variable '" + v +
                                                      "' has two different "
  ...
  std::vector<BehaviourDescription::MaterialPropertyInput> BehaviourDescription::getMaterialPropertyInputs(
    ...
    for (const auto& v : mpd.inputs) {
      ...
      if (vn == tfel::glossary::Glossary::Temperature) {
        ...
        const auto n = this->getVariableDescriptionByExternalName(*(hs.begin()), vn).name;
        ...
        for (const auto h : hs) {
          ...
          throw_if(this->getMaterialPropertyInputCategory(h, n) != t, "the external name '" + vn +
                                                                          "' has two different "
  ...
  void BehaviourDescription::setIntegrationScheme(const BehaviourDescription::IntegrationScheme s) {
  ...
  BehaviourDescription::IntegrationScheme BehaviourDescription::getIntegrationScheme() const {
    tfel::raise_if(this->ischeme == UNDEFINEDINTEGRATIONSCHEME,
  ...
  void BehaviourDescription::appendToMembers(const Hypothesis h, const std::string& c, const bool b) {
  ...
  void BehaviourDescription::appendToPrivateCode(const Hypothesis h, const std::string& c, const bool b) {
  ...
  BehaviourDescription::BehaviourType BehaviourDescription::getBehaviourType() const {
    return this->type;
  ...
  bool BehaviourDescription::isMaterialPropertyConstantDuringTheTimeStep(const MaterialProperty& mp) const {
  ...
  const std::vector<BehaviourDescription::MaterialProperty>& BehaviourDescription::getElasticMaterialProperties()
      const {
    tfel::raise_if(!this->areElasticMaterialPropertiesDefined(),
  ...
  void BehaviourDescription::setElasticMaterialProperties(const std::vector<MaterialProperty>& emps) {
    ...
    throw_if(!this->allowsNewUserDefinedVariables(), "new variables are can't be defined after the first code block.");
    ...
    this->setAttribute(ModellingHypothesis::UNDEFINEDHYPOTHESIS, BehaviourDescription::requiresStiffnessTensor, false);
    throw_if(!this->elasticMaterialProperties.empty(), "elastic material property already declared");
    auto lemps = emps;  // local copy, swap to data member if no exceptions is thrown
    if (emps.size() == 2u) {
      if (this->isElasticSymmetryTypeDefined()) {
        throw_if(this->getElasticSymmetryType() != mfront::ISOTROPIC, "inconsistent elastic symmetry type");
      } else {
      ...
      throw_if(this->getSymmetryType() != mfront::ORTHOTROPIC, "the behaviour is not orthotropic.");
      if (this->isElasticSymmetryTypeDefined()) {
        throw_if(this->getElasticSymmetryType() != mfront::ORTHOTROPIC, "inconsistent elastic symmetry type");
      } else {
  ...
  void BehaviourDescription::addHillTensor(const VariableDescription& v, const std::vector<MaterialProperty>& hcs) {
    auto throw_if = [](const bool c, const std::string& m) {
  ...
  const std::vector<BehaviourDescription::HillTensor>& BehaviourDescription::getHillTensors() const {
  ...
  void BehaviourDescription::addLocalDataStructure(const LocalDataStructure& lds,
  ...
  void BehaviourDescription::addMainVariable(const DrivingVariable& dv, const ThermodynamicForce& f) {
  ...
  const std::vector<std::pair<DrivingVariable, ThermodynamicForce>>& BehaviourDescription::getMainVariables() const {
  ...
  DrivingVariable& BehaviourDescription::getDrivingVariable(const std::string& n) {
    ...
    const auto p = std::find_if(this->mvariables.begin(), this->mvariables.end(),
  ...
  const DrivingVariable& BehaviourDescription::getDrivingVariable(const std::string& n) const {
    using value_type = std::pair<DrivingVariable, ThermodynamicForce>;
    const auto p = std::find_if(this->mvariables.begin(), this->mvariables.end(),
                                [&n](const value_type& v) { return v.first.name == n; });
    tfel::raise_if(p == this->mvariables.end(),
  ...
  const ThermodynamicForce& BehaviourDescription::getThermodynamicForce(const std::string& n) const {
    using value_type = std::pair<DrivingVariable, ThermodynamicForce>;
    const auto p = std::find_if(this->mvariables.begin(), this->mvariables.end(),
                                [&n](const value_type& v) { return v.second.name == n; });
    tfel::raise_if(p == this->mvariables.end(),
  ...
  std::pair<SupportedTypes::TypeSize, SupportedTypes::TypeSize> BehaviourDescription::getMainVariablesSize() const {
  ...
  void BehaviourDescription::setThermalExpansionCoefficients(MaterialProperty a1,
    ...
    throw_if(!this->allowsNewUserDefinedVariables(), "new variables are can't be defined after the first code block.");
    throw_if(this->areThermalExpansionCoefficientsDefined(), "thermal expansion coefficient already defined");
    throw_if(this->getSymmetryType() != mfront::ORTHOTROPIC, "the behaviour is not orthotropic.");
    this->setAttribute(ModellingHypothesis::UNDEFINEDHYPOTHESIS,
                       BehaviourDescription::requiresThermalExpansionCoefficientTensor, false);
  ...
  void BehaviourDescription::addStressFreeExpansion(const Hypothesis h, const StressFreeExpansionDescription& sfed) {
    auto throw_if = [](const bool c, const std::string& m) {
      tfel::raise_if(c, "BehaviourDescription::addStressFreeExpansion: " + m);
    };
    throw_if(!this->allowsNewUserDefinedVariables(), "new variables are can't be defined after the first code block.");
    if ((sfed.is<BehaviourData::AxialGrowth>()) || (sfed.is<BehaviourData::OrthotropicStressFreeExpansion>()) ||
      ...
      throw_if(this->getSymmetryType() != mfront::ORTHOTROPIC, "axial growth is only valid for orthotropic behaviour");
    } else {
      throw_if((!sfed.is<BehaviourData::VolumeSwellingStressFreeExpansion>()) &&
                   (!sfed.is<BehaviourData::Relocation>()) && (!sfed.is<BehaviourData::IsotropicStressFreeExpansion>()),
  ...
  const std::vector<BehaviourDescription::MaterialProperty>& BehaviourDescription::getThermalExpansionCoefficients()
      const {
    tfel::raise_if(!this->areThermalExpansionCoefficientsDefined(),
  ...
  void BehaviourDescription::setSlipSystems(const std::vector<SlipSystem>& ss) {
    ...
    for (const auto& s : ss) {
      ...
      if (s.is<SlipSystemsDescription::system3d>()) {
        ...
        throw_if(!s.is<SlipSystemsDescription::system4d>(), "internal error (unsupported slip system definition)");
      ...
      StaticVariableDescription v("int", "Nss" + std::to_string(nb), 0u, static_cast<int>(css.size()));
      this->addStaticVariable(uh, v, BehaviourData::UNREGISTRED);
    }
    const auto& ssd = this->gs.get<SlipSystemsDescription>();
    auto n = int{};
    for (SlipSystemsDescription::size_type i = 0; i != ssd.getNumberOfSlipSystemsFamilies(); ++i) {
  ...
  const tfel::material::SlipSystemsDescription& BehaviourDescription::getSlipSystems() const {
  ...
  BehaviourDescription::InteractionMatrixStructure BehaviourDescription::getInteractionMatrixStructure() const {
    tfel::raise_if(!this->areSlipSystemsDefined(),
  ...
  bool BehaviourDescription::useQt() const { return this->use_qt; }  // end of BehaviourDescription::useQt
  ...
  std::string BehaviourDescription::getTangentOperatorType() const {
    if (this->type == GENERALBEHAVIOUR) {
      ...
      std::ostringstream t;
      t << "tfel::math::tmatrix<" << msizes.first << "," << msizes.second << ",real>";
      return t.str();
  ...
  std::string BehaviourDescription::getStressFreeExpansionType() const {
    if ((this->type == STANDARDSTRAINBASEDBEHAVIOUR) || (this->type == STANDARDFINITESTRAINBEHAVIOUR)) {
  ...
  bool BehaviourDescription::isStressFreeExansionAnisotropic(const Hypothesis h) const {
  ...
  void BehaviourDescription::checkModellingHypothesis(const Hypothesis& h) const {
  ...
  const std::set<BehaviourDescription::Hypothesis>& BehaviourDescription::getModellingHypotheses() const {
  ...
  std::set<BehaviourDescription::Hypothesis> BehaviourDescription::getDistinctModellingHypotheses() const {
  ...
  bool BehaviourDescription::isModellingHypothesisSupported(const Hypothesis h) const {
    return this->getModellingHypotheses().count(h) != 0u;
  }

  void BehaviourDescription::setModellingHypotheses(const std::set<Hypothesis>& mh, const bool b) {
    ...
    throw_if(!this->hypotheses.empty(), "supported modelling hypotheses have already been declared");
    ...
    if (this->hypotheses.empty()) {
      if ((this->stypeIsDefined) && (this->getSymmetryType() == mfront::ORTHOTROPIC) && (this->oacIsDefined) &&
          (this->getOrthotropicAxesConvention() == OrthotropicAxesConvention::PLATE)) {
        for (const auto h : mh) {
          throw_if((h != ModellingHypothesis::TRIDIMENSIONAL) && (h != ModellingHypothesis::PLANESTRESS) &&
  ...
  void BehaviourDescription::addModelDescription(const ModelDescription& md) {
    ...
    for (auto ov : md.outputs) {
      VariableDescription dov{ov.type, "d" + ov.name, ov.arraySize, ov.lineNumber};
  ...
  void BehaviourDescription::addMaterialProperties(const Hypothesis h,
                                                   const VariableDescriptionContainer& v,
                                                   const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
    mptr f = &BehaviourData::addMaterialProperty;
    this->addVariables(h, v, s, f);
  }  // end of BehaviourDescription::addMaterialProperties

  void BehaviourDescription::addMaterialProperty(const Hypothesis h,
  ...
  void BehaviourDescription::addIntegrationVariables(const Hypothesis h,
                                                     const VariableDescriptionContainer& v,
                                                     const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addStateVariables(const Hypothesis h,
                                               const VariableDescriptionContainer& v,
                                               const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addIntegrationVariable(const Hypothesis h,
                                                    const VariableDescription& v,
                                                    const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addStateVariable(const Hypothesis h,
                                              const VariableDescription& v,
                                              const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addAuxiliaryStateVariables(const Hypothesis h,
                                                        const VariableDescriptionContainer& v,
                                                        const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addAuxiliaryStateVariable(const Hypothesis h,
                                                       const VariableDescription& v,
                                                       const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addExternalStateVariables(const Hypothesis h,
                                                       const VariableDescriptionContainer& v,
                                                       const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addExternalStateVariable(const Hypothesis h,
                                                      const VariableDescription& v,
                                                      const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::addLocalVariables(const Hypothesis h,
                                               const VariableDescriptionContainer& v,
                                               const BehaviourData::RegistrationStatus s) {
    using mptr =
        void (mfront::BehaviourData::*)(const mfront::VariableDescription&, const BehaviourData::RegistrationStatus);
  ...
  void BehaviourDescription::setVariableAttribute(
      const Hypothesis h, const std::string& v, const std::string& n, const VariableAttribute& a, const bool b) {
    auto set = [&v, &n, &a, b](BehaviourData& bd) { bd.setVariableAttribute(v, n, a, b); };
    if (h == ModellingHypothesis::UNDEFINEDHYPOTHESIS) {
      set(this->d);
      for (auto md : this->sd) {
        set(*(md.second));
  ...
  bool BehaviourDescription::hasGlossaryName(const Hypothesis h, const std::string& v) const {
    return this->getData(h, &BehaviourData::hasGlossaryName, v);
  }  // end of BehaviourDescription::hasGlossaryName

  bool BehaviourDescription::hasEntryName(const Hypothesis h, const std::string& v) const {
    return this->getData(h, &BehaviourData::hasEntryName, v);
  }  // end of BehaviourDescription::hasEntryName

  bool BehaviourDescription::hasParameter(const Hypothesis h, const std::string& v) const {
  ...
  void BehaviourDescription::setParameterDefaultValue(const Hypothesis h, const std::string& n, const double v) {
  ...
  void BehaviourDescription::setParameterDefaultValue(const Hypothesis h,
                                                      ...
                                                      const double v) {
    void (BehaviourData::*m)(const std::string&, const unsigned short, const double) =
  ...
  void BehaviourDescription::setParameterDefaultValue(const Hypothesis h, const std::string& n, const int v) {
  ...
  unsigned short BehaviourDescription::getUnsignedShortParameterDefaultValue(const Hypothesis h,
  ...
  int BehaviourDescription::getIntegerParameterDefaultValue(const Hypothesis h, const std::string& n) const {
  ...
  double BehaviourDescription::getFloattingPointParameterDefaultValue(const Hypothesis h, const std::string& n) const {
  ...
  double BehaviourDescription::getFloattingPointParameterDefaultValue(const Hypothesis h,
                                                                      const std::string& n,
                                                                      const unsigned short i) const {
    return this->getBehaviourData(h).getFloattingPointParameterDefaultValue(n, i);
  }  // end of BehaviourDescription::getFloattingPointParameterDefaultValue

  void BehaviourDescription::addStaticVariable(const Hypothesis h,
  ...
  int BehaviourDescription::getIntegerConstant(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::getIntegerConstant, n);
  }  // end of BehaviourDescription::getIntegerConstant

  void BehaviourDescription::addVariables(const Hypothesis h,
                                          const VariableDescriptionContainer& v,
                                          const BehaviourData::RegistrationStatus s,
  ...
  void BehaviourDescription::addVariable(const Hypothesis h,
                                         const VariableDescription& v,
                                         const BehaviourData::RegistrationStatus s,
                                         void (BehaviourData::*m)(const VariableDescription&,
                                                                  const BehaviourData::RegistrationStatus)) {
  ...
  void BehaviourDescription::addVariables(BehaviourData& b,
                                          const VariableDescriptionContainer& v,
                                          const BehaviourData::RegistrationStatus s,
                                          void (BehaviourData::*m)(const VariableDescription&,
                                                                   const BehaviourData::RegistrationStatus)) {
  ...
  void BehaviourDescription::addVariable(BehaviourData& b,
                                         const VariableDescription& v,
                                         const BehaviourData::RegistrationStatus s,
                                         void (BehaviourData::*m)(const VariableDescription&,
                                                                  const BehaviourData::RegistrationStatus)) {
  ...
  bool BehaviourDescription::hasSpecialisedMechanicalData(const Hypothesis h) const {
    tfel::raise_if(this->getModellingHypotheses().find(h) == this->getModellingHypotheses().end(),
  ...
  void BehaviourDescription::requiresTVectorOrVectorIncludes(bool& b1, bool& b2, const BehaviourData& bd) const {
  ...
  void BehaviourDescription::declareExternalStateVariableProbablyUnusableInPurelyImplicitResolution(
  ...
  void BehaviourDescription::setUsableInPurelyImplicitResolution(const Hypothesis h, const bool b) {
    void (BehaviourData::*m)(const bool) = &BehaviourData::setUsableInPurelyImplicitResolution;
    this->callBehaviourData(h, m, b, true);
  }  // end of BehaviourDescription::declareExternalStateVariableProbablyUnusableInPurelyImplicitResolution

  bool BehaviourDescription::isMemberUsedInCodeBlocks(const Hypothesis h, const std::string& v) const {
    return this->getData(h, &BehaviourData::isMemberUsedInCodeBlocks, v);
  }  // end of BehaviourDescription::isMaterialPropertyName

  bool BehaviourDescription::isMaterialPropertyName(const Hypothesis h, const std::string& v) const {
    return this->getData(h, &BehaviourData::isMaterialPropertyName, v);
  }  // end of BehaviourDescription::isMaterialPropertyName

  bool BehaviourDescription::isLocalVariableName(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::isLocalVariableName, n);
  }  // end of BehaviourDescription::isLocalVariableName

  bool BehaviourDescription::isPersistentVariableName(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::isPersistentVariableName, n);
  }  // end of BehaviourDescription::isPersistentVariableName

  bool BehaviourDescription::isIntegrationVariableName(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::isIntegrationVariableName, n);
  }  // end of BehaviourDescription::isIntegrationVariableName

  bool BehaviourDescription::isIntegrationVariableIncrementName(const Hypothesis h, const std::string& n) const {
  ...
  bool BehaviourDescription::isStateVariableName(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::isStateVariableName, n);
  }  // end of BehaviourDescription::isStateVariableName

  bool BehaviourDescription::isStateVariableIncrementName(const Hypothesis h, const std::string& n) const {
  ...
  bool BehaviourDescription::isAuxiliaryStateVariableName(const Hypothesis h, const std::string& n) const {
  ...
  bool BehaviourDescription::isExternalStateVariableName(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::isExternalStateVariableName, n);
  }  // end of BehaviourDescription::isExternalStateVariableName

  bool BehaviourDescription::isExternalStateVariableIncrementName(const Hypothesis h, const std::string& n) const {
  ...
  bool BehaviourDescription::isParameterName(const Hypothesis h, const std::string& v) const {
    return this->getData(h, &BehaviourData::isParameterName, v);
  }  // end of BehaviourDescription::isParameterName

  bool BehaviourDescription::isStaticVariableName(const Hypothesis h, const std::string& n) const {
  ...
  void BehaviourDescription::setCode(
      const Hypothesis h, const std::string& n, const CodeBlock& c, const Mode m, const Position p, const bool b) {
    if (h == ModellingHypothesis::UNDEFINEDHYPOTHESIS) {
      if (getVerboseMode() >= VERBOSE_DEBUG) {
        auto& log = getLogStream();
        log << "BehaviourDescription::setCode : setting '" << n << "' on default hypothesis" << std::endl;
      }
      this->d.setCode(n, c, m, p, b);
      for (const auto& pd : sd) {
        if (getVerboseMode() >= VERBOSE_DEBUG) {
          auto& log = getLogStream();
          log << "BehaviourDescription::setCode : setting '" << n << "' on hypothesis '"
  ...
  const CodeBlock& BehaviourDescription::getCodeBlock(const Hypothesis h, const std::string& n) const {
    return this->getBehaviourData(h).getCodeBlock(n);
  }  // end of BehaviourDescription::getCode

  std::string BehaviourDescription::getCode(const Hypothesis h, const std::string& n) const {
    const auto b = this->getAttribute(BehaviourData::profiling, false);
    return this->getBehaviourData(h).getCode(n, this->getClassName(), b);
  }  // end of BehaviourDescription::getCode

  bool BehaviourDescription::hasCode(const Hypothesis h, const std::string& n) const {
  ...
  void BehaviourDescription::updateAttribute(const Hypothesis h, const std::string& n, const BehaviourAttribute& a) {
    if (h == ModellingHypothesis::UNDEFINEDHYPOTHESIS) {
  ...
  bool BehaviourDescription::hasAttribute(const Hypothesis h, const std::string& n) const {
    return this->getBehaviourData(h).hasAttribute(n);
  }  // end of BehaviourDescription::hasAttribute

  std::vector<std::string> BehaviourDescription::getCodeBlockNames(const Hypothesis h) const {
    return this->getBehaviourData(h).getCodeBlockNames();
  }

  std::vector<std::string> BehaviourDescription::getExternalNames(const Hypothesis h, const VarContainer& v) const {
  ...
  void BehaviourDescription::setGlossaryName(const Hypothesis h, const std::string& n, const std::string& g) {
    this->callBehaviourData(h, &BehaviourData::setGlossaryName, n, g, true);
  }  // end of BehaviourDescription::setGlossaryName

  bool BehaviourDescription::isGlossaryNameUsed(const Hypothesis h, const std::string& n) const {
    return this->getBehaviourData(h).isGlossaryNameUsed(n);
  }  // end of BehaviourDescription::isGlossaryName

  void BehaviourDescription::setEntryName(const Hypothesis h, const std::string& n, const std::string& g) {
    this->callBehaviourData(h, &BehaviourData::setEntryName, n, g, true);
  }  // end of BehaviourDescription::setEntryName

  bool BehaviourDescription::isUsedAsEntryName(const Hypothesis h, const std::string& n) const {
    return this->getBehaviourData(h).isUsedAsEntryName(n);
  }  // end of BehaviourDescription::isEntryName

  std::string BehaviourDescription::getExternalName(const Hypothesis h, const std::string& n) const {
    return this->getData(h, &BehaviourData::getExternalName, n);
  }  // end of BehaviourDescription::getGlossaryName

  std::string BehaviourDescription::getVariableNameFromGlossaryNameOrEntryName(const Hypothesis h,
                                                                               const std::string& n) const {
  ...
  const VariableDescription& BehaviourDescription::getVariableDescriptionByExternalName(const Hypothesis h,
                                                                                        const std::string& n) const {
  ...
  void BehaviourDescription::setBounds(const Hypothesis h, const std::string& n, const VariableBoundsDescription& b) {
    auto throw_if = [](const bool c, const std::string& m) {
      tfel::raise_if(c, "BehaviourDescription::setBounds: " + m);
    };
    if (this->isDrivingVariableName(n)) {
      throw_if(h != ModellingHypothesis::UNDEFINEDHYPOTHESIS, "invalid modelling hypothesis");
  ...
  void BehaviourDescription::setAttribute(const std::string& n, const BehaviourAttribute& a, const bool b) {
    if (b) {
  ...
  const std::map<std::string, BehaviourAttribute>& BehaviourDescription::getAttributes() const {
    return this->attributes;
  }  // end of BehaviourDescription::getAttributes

  void BehaviourDescription::reserveName(const Hypothesis h, const std::string& n) {
  ...
  void BehaviourDescription::registerMemberName(const Hypothesis h, const std::string& n) {
    this->callBehaviourData(h, &BehaviourData::registerMemberName, n, true);
  }  // end of BehaviourDescription::registerMemberName

  void BehaviourDescription::registerStaticMemberName(const Hypothesis h, const std::string& n) {
  ...
  void BehaviourDescription::addMaterialLaw(const std::string& m) {
    if (std::find(this->materialLaws.begin(), this->materialLaws.end(), m) == this->materialLaws.end()) {
      this->materialLaws.push_back(m);
    }
  }  // end of BehaviourDescription::getMaterialLaws

  const std::vector<std::string>& BehaviourDescription::getMaterialLaws() const { return this->materialLaws; }

  std::pair<bool, bool> BehaviourDescription::checkVariableExistence(const std::string& v) const {
  ...
  std::pair<bool, bool> BehaviourDescription::checkVariableExistence(const std::string& n,
                                                                     const std::string& c,
                                                                     const bool b) const {
    const auto& mh = this->getDistinctModellingHypotheses();
    std::pair<bool, bool> r = {true, false};
  ...
  void BehaviourDescription::checkVariableGlossaryName(const std::string& n, const std::string& g) const {
    auto throw_if = [](const bool c, const std::string& m) {
      tfel::raise_if(c,
                     "BehaviourDescription::"
    ...
    for (const auto& h : this->getDistinctModellingHypotheses()) {
      const auto& bdata = this->getBehaviourData(h);
      throw_if(!bdata.hasGlossaryName(n), "no glossary name associated with variable '" + n + "'");
  ...
  void BehaviourDescription::checkVariableEntryName(const std::string& n, const std::string& e) const {
    auto throw_if = [](const bool c, const std::string& m) {
      tfel::raise_if(c,
                     "BehaviourDescription::"
    ...
    for (const auto& h : this->getDistinctModellingHypotheses()) {
      const auto& bdata = this->getBehaviourData(h);
      throw_if(!bdata.hasEntryName(n), "no entry name associated with variable '" + n + "'");
      const auto& en = bdata.getExternalName(n);
      throw_if(en != e,
  ...
  void BehaviourDescription::checkVariablePosition(const std::string& n, const std::string& c, const size_t p) {
    for (const auto& h : this->getDistinctModellingHypotheses()) {
      const auto& bdata = this->getBehaviourData(h);
  ...
  void BehaviourDescription::setOrthotropicAxesConvention(const OrthotropicAxesConvention c) {
    ...
    if (c == OrthotropicAxesConvention::PLATE) {
      if (this->areModellingHypothesesDefined()) {
        for (const auto h : this->getModellingHypotheses()) {
          tfel::raise_if((h != ModellingHypothesis::TRIDIMENSIONAL) && (h != ModellingHypothesis::PLANESTRESS) &&
  ...
  tfel::material::OrthotropicAxesConvention BehaviourDescription::getOrthotropicAxesConvention() const {
    tfel::raise_if(this->getSymmetryType() != mfront::ORTHOTROPIC,
  ...
  void BehaviourDescription::setStrainMeasure(const StrainMeasure sm) {
    tfel::raise_if(this->getBehaviourType() != BehaviourDescription::STANDARDSTRAINBASEDBEHAVIOUR,
  ...
  BehaviourDescription::StrainMeasure BehaviourDescription::getStrainMeasure() const {
    tfel::raise_if(!this->isStrainMeasureDefined(),
  ...
  void setElasticSymmetryType(BehaviourDescription& bd, const BehaviourSymmetryType s) {
