...
namespace mfront {
  ...
  void IsotropicStrainHardeningMisesCreepCodeGenerator::
    ...
    os << this->bd.getCode(h, BehaviourData::FlowRule) << "return true;\n}\n\n"
       ...
       << "(iter<(this->iterMax))){\n"
       << "this->seq=std::max(this->seq_e-mfront_internal_3_mu*(this->theta)*(this->dp), "
          "stress(0."
          "f));\n"
    ...
    os << "if(iter==0u){\n"
       ...
       << "newton_ddp = (this->seq_e / (mfront_internal_3_mu * (this->theta))) / 2;\n"
       << "} // end of if(iter==0u)\n"
       << "this->dp += newton_ddp;\n"
       ...
       << "newton_df = "
          "1-(this->theta)*(this->dt)*((this->df_dp)-mfront_internal_3_mu*(this->df_dseq));\n"
