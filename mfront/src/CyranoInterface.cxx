...
#error "CyranoInterface::getGlobalIncludes : unsuported architecture"
    ...
    insert_if(d[lib].link_libraries,
              tfel::getLibraryInstallName("CyranoInterface"));
    if (this->generateMTestFile) {
      insert_if(d[lib].link_libraries,
  ...
  void CyranoInterface::writeUMATxxBehaviourTypeSymbols(
      std::ostream& out,
      const std::string& name,
      const BehaviourDescription& mb) const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(b,
                     "CyranoInterface::writeUMATxxBehaviourTypeSymbols: " + m);
    };
  ...
  void CyranoInterface::writeUMATxxBehaviourKinematicSymbols(
      std::ostream& out,
      const std::string& name,
      const BehaviourDescription& mb) const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(
          b, "CyranoInterface::writeUMATxxBehaviourKinematicSymbols: " + m);
    };
    out << "MFRONT_SHAREDOBJ unsigned short " << this->getFunctionName(name)
        << "_BehaviourKinematic = ";
    if (mb.getBehaviourType() ==
  ...
  void CyranoInterface::writeStandardCyranoFunction(
    ...
    if (this->generateMTestFile) {
      ...
      this->generateMTestFile2(
          out, BehaviourDescription::STANDARDSTRAINBASEDBEHAVIOUR, n, "", mb);
      out << "}\n";
    }
    out << "}\n\n";
