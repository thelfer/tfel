...
namespace ansys
{
								      ...
								      const AnsysReal Fzz)
  {
    ...
    const AnsysReal p4 = (NTENS==6) ? STRESS[4] : AnsysReal(0);
    const AnsysReal p5 = (NTENS==6) ? STRESS[5] : AnsysReal(0);
									...
									const AnsysReal Fzz)
  {
    ...
    const AnsysReal p4 = (NTENS==6) ? s[4] : AnsysReal(0);
    const AnsysReal p5 = (NTENS==6) ? s[5] : AnsysReal(0);
  ...
  AnsysFiniteStrain::computeAnsysTangentOperatorFromCSE(AnsysReal* const D,
    ...
    if(NTENS==4){
      ...
      const auto C =
	UmatNormaliseTangentOperator<2u,AnsysReal>::exe(CSE);
      ...
      auto F1 = tensor::buildFromFortranMatrix(F);
      const auto C =
	UmatNormaliseTangentOperator<3u,AnsysReal>::exe(CSE);
							...
							const AnsysInt NTENS)
  {
    ...
    if(NTENS==3u){
      ...
      throw(std::runtime_error("AnsysFiniteStrain::applyNativeFiniteStrainCorrection: "
