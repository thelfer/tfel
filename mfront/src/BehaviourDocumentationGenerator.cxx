...
namespace mfront {
  ...
  static void getData(std::vector<Data>& data,
    ...
    for (auto pv = mdata.begin(); pv != mdata.end(); ++pv) {
      ...
      if (pd == data.end()) {
        ...
        if ((pd->name != pv->name) || (pd->type != pv->type) ||
            (pd->externalName != d.getExternalName(pv->name)) ||
            (pd->arraySize != pv->arraySize)) {
          throw(
              std::runtime_error("getData : inconsistent data across "
  ...
  void BehaviourDocumentationGenerator::treatWeb() {
    ...
    if (this->otype != FULL) {
      throw(
          runtime_error("BehaviourDocumentationGenerator::treatWeb: "
  ...
  void BehaviourDocumentationGenerator::exe() {
    ...
    if (this->otype == FULL) {
      ...
      throw(
          runtime_error("BehaviourDocumentationGenerator::exe: "
  ...
  void BehaviourDocumentationGenerator::writeWebOutput(
    ...
    if (!f) {
      throw(
          std::runtime_error("BehaviourDocumentationGenerator::writeWebOutput: "
                             "can't open file '" +
                             this->file + "'"));
    }
