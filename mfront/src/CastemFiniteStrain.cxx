...
namespace castem
{
  ...
  void
  CastemFiniteStrain::computeGreenLagrangeStrain(CastemReal* const e,
								      ...
								      const CastemReal Fzz)
  {
    ...
    // determinant
    const CastemReal J  = F0*(F4*F8-F5*F7)-F3*(F1*F8-F2*F7)+(F1*F5-F2*F4)*F6;
  ...
  } // end of CastemFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress

  void
  CastemFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress(CastemReal* const sk2,
								      ...
								      const CastemReal Fzz)
  {
    ...
    CastemFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress(sk2,F,NTENS,NDI,Fzz);
  } // end of CastemFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress

  void
  CastemFiniteStrain::computeCauchyStressFromSecondPiolaKirchhoffStress(CastemReal* const s,
								      const CastemReal* const F,
								      const CastemInt NTENS,
  								      const CastemInt NDI,
								      const CastemReal Fzz)
  {
    ...
    const CastemReal F8 = NDI!=-2 ? F[8] : Fzz;
    // determinant
    const CastemReal iJ = 1/(F0*(F4*F8-F5*F7)-F3*(F1*F8-F2*F7)+(F1*F5-F2*F4)*F6);
    ...
    s[0] = iJ*(F3*(p5*F6+p1*F3+p3*F0)+F0*(p4*F6+p3*F3+p0*F0)+F6*(p2*F6+p5*F3+p4*F0));
    // maxima :   F4*(p5*F7+p1*F4+p3*F1)+F1*(p4*F7+p3*F4+p0*F1)+F7*(p2*F7+p5*F4+p4*F1) 
    s[1] = iJ*(F4*(p5*F7+p1*F4+p3*F1)+F1*(p4*F7+p3*F4+p0*F1)+F7*(p2*F7+p5*F4+p4*F1));
    // maxima :   F5*(p5*F8+p1*F5+p3*F2)+F2*(p4*F8+p3*F5+p0*F2)+F8*(p2*F8+p5*F5+p4*F2) 
    s[2] = iJ*(F5*(p5*F8+p1*F5+p3*F2)+F2*(p4*F8+p3*F5+p0*F2)+F8*(p2*F8+p5*F5+p4*F2));
    if(NTENS==4){
      if(NDI==-2){
	s[2] = 0;
      }
      // maxima :   F3*(p5*F7+p1*F4+p3*F1)+F0*(p4*F7+p3*F4+p0*F1)+F6*(p2*F7+p5*F4+p4*F1) 
      s[3] = iJ*(F3*(p5*F7+p1*F4+p3*F1)+F0*(p4*F7+p3*F4+p0*F1)+F6*(p2*F7+p5*F4+p4*F1));
    } else if(NTENS==6){
      // maxima :   F3*(p5*F7+p1*F4+p3*F1)+F0*(p4*F7+p3*F4+p0*F1)+F6*(p2*F7+p5*F4+p4*F1) 
      s[3] = iJ*(F3*(p5*F7+p1*F4+p3*F1)+F0*(p4*F7+p3*F4+p0*F1)+F6*(p2*F7+p5*F4+p4*F1));
      // maxima :   F3*(p5*F8+p1*F5+p3*F2)+F0*(p4*F8+p3*F5+p0*F2)+F6*(p2*F8+p5*F5+p4*F2) 
      s[4] = iJ*(F3*(p5*F8+p1*F5+p3*F2)+F0*(p4*F8+p3*F5+p0*F2)+F6*(p2*F8+p5*F5+p4*F2));
      // maxima :   F4*(p5*F8+p1*F5+p3*F2)+F1*(p4*F8+p3*F5+p0*F2)+F7*(p2*F8+p5*F5+p4*F2) 
      s[5] = iJ*(F4*(p5*F8+p1*F5+p3*F2)+F1*(p4*F8+p3*F5+p0*F2)+F7*(p2*F8+p5*F5+p4*F2));
    }
  ...
  static void
  CastemFiniteStrainComputeLogarithmicStrainAndDerivative3D(CastemReal* const P,
							  CastemReal* const E,
							  const CastemReal* const F)
  {
    using namespace tfel::math;
    ...
    // logarithmic strain
    StensorView<3u,CastemReal>{E} = (log_vp(0)*n0+log_vp(1)*n1+log_vp(2)*n2)/2;
    E[3] *= cste;
    E[4] *= cste;
    E[5] *= cste;
    // computing P
   ...
   void
   CastemFiniteStrain::computeLogarithmicStrainAndDerivative(CastemReal* const P,
							   CastemReal* const E,
							   const CastemReal* const F,
							   const CastemInt NTENS,
							   const CastemInt NDI)
   ...
   void
   CastemFiniteStrain::computeDualStressOfLogarithmicStrainFromCauchyStress(CastemReal* const s,
									  const CastemReal* const STRESS,
									  ...
									  const CastemReal Fzz)
   {
     ...
     if(NTENS==3u){
       ...
       sk2[3]    *= cste;
       const st2tost2<2u,CastemReal> iP(invert(ConstST2toST2View<2u,CastemReal>{P}));
       StensorView<2u,CastemReal>{s} = ConstStensorView<2u,CastemReal>{sk2}|iP;
       s[3] *= icste;
     } else if(NTENS==6u){
       sk2[3] *= cste;
       sk2[4] *= cste;
   ...
   void
   CastemFiniteStrain::computeCauchyStressFromDualStressOfLogarithmicStrain(CastemReal* const STRESS,
