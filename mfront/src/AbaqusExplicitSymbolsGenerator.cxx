...
namespace mfront {
  ...
  void AbaqusExplicitSymbolsGenerator::writeSpecificSymbols(
    ...
    if(mb.getSymmetryType()==mfront::ORTHOTROPIC){
      if(!mb.hasAttribute(AbaqusExplicitInterface::orthotropyManagementPolicy)){
	out << "MFRONT_SHAREDOBJ unsigned short " << i.getFunctionNameBasis(name)
	    << "_OrthotropyManagementPolicy = 0u;\n\n";    
      } else {
	const auto omp =
	  mb.getAttribute<std::string>(AbaqusExplicitInterface::orthotropyManagementPolicy);
	if(omp=="MFront"){
	  out << "MFRONT_SHAREDOBJ unsigned short " << i.getFunctionNameBasis(name)
	  ...
	  tfel::raise("AbaqusExplicitSymbolsGenerator::writeSpecificSymbols: "
  ...
  void AbaqusExplicitSymbolsGenerator::writeBehaviourTypeSymbols(
    ...
    if(mb.getBehaviourType()==BehaviourDescription::STANDARDSTRAINBASEDBEHAVIOUR){
      ...
      tfel::raise("AbaqusExplicitSymbolsGenerator::writeBehaviourTypeSymbols: "
  ...
  void AbaqusExplicitSymbolsGenerator::writeBehaviourKinematicSymbols(
    ...
    if(mb.getBehaviourType()==BehaviourDescription::STANDARDSTRAINBASEDBEHAVIOUR){
      ...
      tfel::raise("AbaqusExplicitSymbolsGenerator::writeBehaviourKinematicSymbols: "
		  "unsupported behaviour type");
    }
