...
namespace mfront {
  ...
  void AsterInterface::endTreatment(const BehaviourDescription& mb,
    ...
    out << "char *" << this->getFunctionName(name)
        << "_getIntegrationErrorMessage(){\n"
    ...
    out << "MFRONT_SHAREDOBJ void\n"
        ...
        << ")\n";
    out << "{\n";
    out << "char * msg = " << this->getFunctionName(name)
        << "_getIntegrationErrorMessage();\n";
...
#error "AsterInterface::getGlobalIncludes : unsuported architecture"
  ...
  void AsterInterface::writeUMATxxBehaviourTypeSymbols(
      std::ostream& out,
      const std::string& name,
      const BehaviourDescription& mb) const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(b,
                     "AsterInterface::writeUMATxxBehaviourTypeSymbols: " + m);
    };
  ...
  void AsterInterface::writeUMATxxBehaviourKinematicSymbols(
      std::ostream& out,
      const std::string& name,
      const BehaviourDescription& mb) const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(
          b, "AsterInterface::writeUMATxxBehaviourKinematicSymbols: " + m);
    };
    out << "MFRONT_SHAREDOBJ unsigned short " << this->getFunctionName(name)
        << "_BehaviourKinematic = ";
    if (mb.getBehaviourType() ==
  ...
  void AsterInterface::writeUMATxxSpecificSymbols(
    ...
    if (bd.getBehaviourType() ==
      ...
      if ((this->afsf == SIMO_MIEHE) ||
          (this->afsf == UNDEFINEDFINITESTRAINFORMULATION)) {
        out << "1u;\n";
