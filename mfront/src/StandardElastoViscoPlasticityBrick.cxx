...
namespace mfront {
  ...
  std::string StandardElastoViscoPlasticityBrick::getName() const {
    ...
    StandardElastoViscoPlasticityBrick::getDescription() const {
    ...
    void StandardElastoViscoPlasticityBrick::initialize(const Parameters&,
      ...
      for (const auto& e : d) {
        if ((e.first == "elastic_potential") ||
            (e.first == "stress_potential")) {
          // already treated
    ...
    StandardElastoViscoPlasticityBrick::treatPorosityEvolutionAlgorithmSection(
      ...
      if (d.is<std::string>()) {
        this->porosity_evolution_algorithm =
            get_algorithm(d.get<std::string>());
    ...
    StandardElastoViscoPlasticityBrick::getSupportedModellingHypotheses()
        const {
    ...
    void StandardElastoViscoPlasticityBrick::completeVariableDeclaration()
        const {
      constexpr const auto uh =
          tfel::material::ModellingHypothesis::UNDEFINEDHYPOTHESIS;
      ...
      for (const auto& nm : this->nucleation_models) {
        nm->completeVariableDeclaration(
            this->bd, this->dsl, this->buildInelasticFlowsMap(),
