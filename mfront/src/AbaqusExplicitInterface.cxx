...
namespace mfront {
  ...
  AbaqusExplicitInterface::treatKeyword(BehaviourDescription& bd,
    ...
    auto throw_if = [](const bool b, const std::string& m) {
      if (b) {
        throw(
            std::runtime_error("AbaqusExplicitInterface::treatKeyword: " + m));
      }
    };
    if (!i.empty()) {
      ...
      if (std::find(keys.begin(), keys.end(), key) == keys.end()) {
        throw(
            std::runtime_error("AbaqusExplicitInterface::treatKeyword: "
                               "unsupported key '" +
                               key + "'"));
      }
    }
  ...
  void AbaqusExplicitInterface::getTargetsDescription(
    ...
    if (ppolicy == "ThreadPool") {
      insert_if(d[lib].ldflags,
                "$(shell " + tfel_config +
                    " --libs --material --system --mfront-profiling)");
    } else {
  ...
  void AbaqusExplicitInterface::endTreatment(const BehaviourDescription& mb,
    ...
    auto throw_if = [](const bool b, const std::string& m) {
      if (b) {
        throw(
            std::runtime_error("AbaqusExplicitInterface::endTreatment: " + m));
      }
    };
    ...
    if (!out) {
      throw(
          std::runtime_error("AbaqusExplicitInterface::endTreatment : "
                             "could not open file '" +
                             fname + "'"));
    }
        ...
        if (mb.getBehaviourType() ==
          ...
          if (this->fss == NATIVEFINITESTRAINSTRATEGY) {
            ...
            throw(std::runtime_error(
                "AbaqusExplicitInterface::writeVUMATFunction: "
  ...
  static void writeGetRotationMatrix(
    ...
    if ((h == AbaqusExplicitInterface::ModellingHypothesis::PLANESTRESS) ||
      ...
      throw(
          std::runtime_error("writeGetRotationMatrix: unsupported hypothesis"));
    }
  }
  ...
  void AbaqusExplicitInterface::writeBehaviourDataConstructor(
    ...
    this->writeVariableInitializersInBehaviourDataConstructorI(
        out, persistentVarsHolder, iprefix + "d.stateOld", "", "");
    this->writeVariableInitializersInBehaviourDataConstructorI(
        out, externalStateVarsHolder, iprefix + "d.fieldOld", "", "");
    out << "\n{\n";
  ...
  void AbaqusExplicitInterface::writeIntegrationDataConstructor(
    ...
    if (!externalStateVarsHolder.empty()) {
      this->writeVariableInitializersInBehaviourDataConstructorI(
          out, externalStateVarsHolder, iprefix + "d.dfield", "d", "");
    }
    out << "\n{\n";
    if (!externalStateVarsHolder.empty()) {
      this->writeVariableInitializersInBehaviourDataConstructorII(
  ...
  void AbaqusExplicitInterface::writeChecks(std::ostream& out,
    ...
    if ((h == ModellingHypothesis::AXISYMMETRICAL) ||
      ...
      throw(
          std::runtime_error("AbaqusExplicitInterface::writeChecks: "
  ...
  void AbaqusExplicitInterface::writeLogarithmicStrainIntegration(
    ...
    out << "stensor<" << dime << "," << t << "> eto;\n"
        << "stensor<" << dime << "," << t << "> deto;\n"
        << "std::tie(eto ,P0) = "
           "C0.computeIsotropicFunctionAndDerivative(f,df,std::numeric_limits<"
        << t << ">::epsilon());\n"
        << "std::tie(deto,P1) = "
           "C1.computeIsotropicFunctionAndDerivative(f,df,std::numeric_limits<"
        << t << ">::epsilon());\n"
        << "deto -= eto;\n";
    if (h == ModellingHypothesis::PLANESTRESS) {
