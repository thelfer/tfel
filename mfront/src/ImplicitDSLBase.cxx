...
namespace mfront {
  ...
  void ImplicitDSLBase::treatTheta() {
    ...
    flux >> theta;
    if ((flux.fail()) || (!flux.eof())) {
      this->throwRuntimeError("ImplicitDSLBase::treatTheta", "Failed to read theta value.");
    }
    if ((theta < 0.) || (theta > 1.)) {
  ...
  void ImplicitDSLBase::treatEpsilon() {
    ...
    flux >> epsilon;
    if ((flux.fail()) || (!flux.eof())) {
      this->throwRuntimeError("ImplicitDSLBase::treatEpsilon", "Failed to read epsilon value.");
    }
    if (epsilon < 0) {
  ...
  void ImplicitDSLBase::writeBehaviourParserSpecificMembers(const Hypothesis h) {
      ...
      for (p2 = d.getIntegrationVariables().begin(); p2 != d.getIntegrationVariables().end();
        ...
        if ((p->arraySize != 1u) || (p2->arraySize != 1u)) {
          this->behaviourFile << "/*!\n"
                              ...
                              << "of variable " << p->name << " by variable " << p2->name << "\n"
                              << " */\n";
        }
        if ((p->arraySize != 1u) && (p2->arraySize == 1u)) {
          if (flag == SupportedTypes::Scalar) {
            if (flag2 == SupportedTypes::Scalar) {
              this->behaviourFile << "real&\n"
                                  << "df" << p->name << "_dd" << p2->name << "(tfel::math::tmatrix<"
              ...
              this->behaviourFile << "typename tfel::math::TVectorFromTinyMatrixRowView2<N," << n3
                                  << "," << n3 << "," << n << "," << n2 << ",real>::type\n"
                                  << "df" << p->name << "_dd" << p2->name << "(tfel::math::tmatrix<"
                                  << n3 << "," << n3 << ",real>& tjacobian,\n"
                                  << "const unsigned short idx){\n"
              ...
              // Le résultat est un tenseur, une ligne dans la matrice jacobienne
              this->behaviourFile << "typename tfel::math::StensorFromTinyMatrixRowView2<N," << n3
                                  << "," << n3 << "," << n << "," << n2 << ",real>::type\n"
                                  << "df" << p->name << "_dd" << p2->name << "(tfel::math::tmatrix<"
                                  << n3 << "," << n3 << ",real>& tjacobian,\n"
                                  << "const unsigned short idx){\n"
            ...
            if (flag2 == SupportedTypes::Scalar) {
              ...
              this->behaviourFile << "typename tfel::math::StensorFromTinyMatrixColumnView2<N,"
              ...
              this->behaviourFile << "typename tfel::math::TMatrixFromTinyMatrixView2<N," << n3
                                  << "," << n3 << "," << n << "," << n2 << ",real>::type\n"
                                  << "df" << p->name << "_dd" << p2->name << "(tfel::math::tmatrix<"
                                  << n3 << "," << n3 << ",real>& tjacobian,\n"
                                  << "const unsigned short idx){\n"
            ...
            if (flag2 == SupportedTypes::Scalar) {
              ...
              this->behaviourFile << "typename tfel::math::ST2toST2FromTinyMatrixView2<N," << n3
                                  << "," << n3 << "," << n << "," << n2 << ",real>::type\n"
                                  << "df" << p->name << "_dd" << p2->name << "(tfel::math::tmatrix<"
                                  << n3 << "," << n3 << ",real>& tjacobian,\n"
                                  << "const unsigned short idx){\n"
    ...
    if (this->mb.hasCode(h, BehaviourData::ComputeStress)) {
      this->behaviourFile << "void\ncomputeStress(void){\n"
                          ...
                          << "using namespace tfel::math;\n"
                          << "using std::vector;\n";
      writeMaterialLaws("ImplicitDSLBase::writeBehaviourParserSpecificMembers", this->behaviourFile,
                        this->mb.getMaterialLaws());
      ...
      for (size_type i2 = 0; i2 <= i; ++i2) {
        ...
        if (flag == SupportedTypes::Scalar) {
          if (v.arraySize == 1u) {
            ...
            this->behaviourFile << "for(unsigned short idx2=0;idx2!=" << v.arraySize
                                << ";++idx2){\n";
            this->behaviourFile << "partial_jacobian_" << v.name << "(idx2)(idx)=vect_e(" << n2
                                << "+idx2);\n";
            this->behaviourFile << "}\n";
          }
          ...
          if (v.arraySize == 1u) {
            ...
            this->behaviourFile << "for(unsigned short idx2=0;idx2!=" << v.arraySize
                                << ";++idx2){\n";
            this->behaviourFile << "for(unsigned short idx3=0;idx3!=TVectorSize;++idx3){\n";
            this->behaviourFile << "partial_jacobian_" << v.name << "(idx2)(idx3,idx)=vect_e(" << n2
          ...
          if (v.arraySize == 1u) {
            ...
            this->behaviourFile << "for(unsigned short idx2=0;idx2!=" << v.arraySize
                                << ";++idx2){\n";
            this->behaviourFile << "for(unsigned short idx3=0;idx3!=StensorSize;++idx3){\n";
            this->behaviourFile << "partial_jacobian_" << v.name << "(idx2)(idx3,idx)=vect_e(" << n2
  ...
  void ImplicitDSLBase::writeComputeNumericalJacobian(const Hypothesis h) {
    ...
    this->checkBehaviourFile();
    this->behaviourFile << "void\ncomputeNumericalJacobian(tfel::math::tmatrix<" << n << "," << n
                        << ",real>& njacobian)\n"
                        << "{\n"
                        << "using namespace std;\n"
                        << "using namespace tfel::math;\n"
  ...
  void ImplicitDSLBase::writeBehaviourIntegrator(const Hypothesis h) {
    ...
    if (this->mb.hasCode(h, BehaviourData::ComputePredictor)) {
      this->behaviourFile << this->mb.getCode(h, BehaviourData::ComputePredictor) << '\n';
    }
    if (this->mb.getAttribute(BehaviourData::profiling, false)) {
      writeStandardPerformanceProfilingBegin(this->behaviourFile, mb.getClassName(),
  ...
  void ImplicitDSLBase::writeComputeFdF(const Hypothesis h) {
    ...
    this->behaviourFile << "/*!\n"
                        ...
                        << "using std::vector;\n";
    if (this->mb.getAttribute(BehaviourData::profiling, false)) {
      writeStandardPerformanceProfiling(this->behaviourFile, mb.getClassName(), "ComputeFdF");
    }
    writeMaterialLaws("ImplicitDSLBase::writeBehaviourParserSpecificMembers", this->behaviourFile,
                      this->mb.getMaterialLaws());
    ...
    for (const auto& v : d.getIntegrationVariables()) {
      if (this->getTypeFlag(v.type) == SupportedTypes::Scalar) {
        if (v.arraySize == 1u) {
          ...
          this->behaviourFile << "typename tfel::math::TinyVectorFromTinyVectorView<" << v.arraySize
                              << "," << n2 << "," << n << ",real,false>::type"
                              << " f" << v.name << "(this->fzeros);\n";
        }
      } else {
        if (v.arraySize == 1u) {
          this->behaviourFile << "typename tfel::math::" << this->getVectorMappingClass(v) << "<N,"
                              << n2 << "," << n << ",real>::type"
                              << " f" << v.name << "(this->fzeros);\n";
        } else {
          this->behaviourFile << "typename tfel::math::" << this->getVectorMappingClass(v) << "<N,"
                              << n2 << "," << n << "," << v.arraySize << ",real>::type"
                              << " f" << v.name << "(this->fzeros);\n";
        }
      }
      n += this->getTypeSize(v.type, v.arraySize);
      ...
      for (const auto& v : d.getIntegrationVariables()) {
        ...
        for (const auto& v2 : d.getIntegrationVariables()) {
          if ((v.arraySize == 1u) && (v2.arraySize == 1u)) {
            this->behaviourFile << "// derivative of variable f" << v.name << " by variable "
                                << v2.name << "\n"
    ...
    if (this->solver->requiresJacobianToBeReinitialisedToIdentityAtEachIterations()) {
      for (const auto& v : d.getIntegrationVariables()) {
        this->jacobianPartsUsedInIntegrator.insert("df" + v.name + "_dd" + v.name);
      }
    ...
    if ((this->solver->usesJacobian()) && (!(this->solver->requiresNumericalJacobian()))) {
      for (const auto& v : d.getIntegrationVariables()) {
        for (const auto& v2 : d.getIntegrationVariables()) {
          if ((v.arraySize == 1u) && (v2.arraySize == 1u)) {
            this->behaviourFile << "static_cast<void>(df" << v.name << "_dd" << v2.name
                                << "); /* suppress potential warnings */\n";
          }
          if (this->jacobianPartsUsedInIntegrator.find("df" + v.name + "_dd" + v2.name) !=
            ...
            if (bfnf) {
              ...
              if (bvnf) {
                ...
                if ((v.arraySize != 1u) && (v2.arraySize != 1u)) {
                  this->behaviourFile << "for(unsigned short idx=0;idx!=" << v.arraySize
                                      << ";++idx){\n";
                  this->behaviourFile << "for(unsigned short idx2=0;idx2!=" << v2.arraySize
                                      << ";++idx2){\n";
                  ...
                  this->behaviourFile << "for(unsigned short idx=0;idx!=" << v.arraySize
                                      << ";++idx){\n";
                  this->behaviourFile << "df" << v.name << "_dd" << v2.name << "(idx) *= " << vnf
                                      << "/(" << fnf << ");\n";
                  this->behaviourFile << "}\n";
                } else if ((v.arraySize == 1u) && (v2.arraySize != 1u)) {
                  this->behaviourFile << "for(unsigned short idx=0;idx!=" << v2.arraySize
                                      << ";++idx){\n";
                  this->behaviourFile << "df" << v.name << "_dd" << v2.name << "(idx) *= " << vnf
                                      << "/(" << fnf << ");\n";
                  this->behaviourFile << "}\n";
                } else if ((v.arraySize == 1u) && (v2.arraySize == 1u)) {
                  this->behaviourFile << "df" << v.name << "_dd" << v2.name << " *= " << vnf << "/("
                                      << fnf << ");\n";
                }
              } else {
                if ((v.arraySize != 1u) && (v2.arraySize != 1u)) {
                  ...
                  this->behaviourFile << "for(unsigned short idx2=0;idx2!=" << v2.arraySize
                                      << ";++idx2){\n";
                  this->behaviourFile << "df" << v.name << "_dd" << v2.name << "(idx,idx2) *= "
                                      << "real(1)/(" << fnf << ");\n";
                  this->behaviourFile << "}\n";
                  this->behaviourFile << "}\n";
                } else if ((v.arraySize != 1u) && (v2.arraySize == 1u)) {
                  this->behaviourFile << "for(unsigned short idx=0;idx!=" << v.arraySize
                                      << ";++idx){\n";
                  this->behaviourFile << "df" << v.name << "_dd" << v2.name << "(idx) *= "
                                      << "real(1)/(" << fnf << ");\n";
                  this->behaviourFile << "}\n";
                } else if ((v.arraySize == 1u) && (v2.arraySize != 1u)) {
                  this->behaviourFile << "for(unsigned short idx=0;idx!=" << v2.arraySize
  ...
  std::string ImplicitDSLBase::getBehaviourConstructorsInitializers(const Hypothesis h) {
  ...
  void ImplicitDSLBase::writeBehaviourIntegrationVariablesIncrements(const Hypothesis h) {
    ...
    for (const auto& v : d.getIntegrationVariables()) {
      if ((!getDebugMode()) && (v.lineNumber != 0u)) {
        this->behaviourFile << "#line " << v.lineNumber << " \"" << this->fileName << "\"\n";
      }
      if (this->getTypeFlag(v.type) == SupportedTypes::Scalar) {
        if (v.arraySize == 1u) {
  ...
  void ImplicitDSLBase::endsInputFileProcessing() {
    ...
    if ((!this->mb.getAttribute(BehaviourDescription::computesStiffnessTensor, false)) &&
        (this->mb.getElasticSymmetryType() == ISOTROPIC) &&
        (this->mb.areElasticMaterialPropertiesDefined())) {
      auto add_lv = [&h](BehaviourDescription& bd, const std::string& t, const std::string& n,
    ...
    if (!this->mb.areAllMechanicalDataSpecialised()) {
      if (this->mb.hasCode(ModellingHypothesis::UNDEFINEDHYPOTHESIS,
        ...
        CodeBlock predictor;
        const auto& d = this->mb.getBehaviourData(ModellingHypothesis::UNDEFINEDHYPOTHESIS);
        for (const auto& v : d.getIntegrationVariables()) {
          if (this->integrationVariablesIncrementsUsedInPredictor.find('d' + v.name) !=
              this->integrationVariablesIncrementsUsedInPredictor.end()) {
            if (this->mb.hasAttribute(h, v.name + "_normalisation_factor")) {
        ...
        this->mb.setCode(ModellingHypothesis::UNDEFINEDHYPOTHESIS, BehaviourData::ComputePredictor,
                         predictor, BehaviourData::CREATEORAPPEND, BehaviourData::AT_END);
      }
    }
