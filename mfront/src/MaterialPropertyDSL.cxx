...
namespace mfront{
  ...
  void MaterialPropertyDSL::addStaticVariableDescription(const StaticVariableDescription& v)
  {
  ...
  void MaterialPropertyDSL::treatParameter()
  {
    ...
    VariableDescription p("real",this->current->value,1u,this->current->line);
  ...
  void MaterialPropertyDSL::treatFunction()
  {
	...
	if((this->current->value==this->md.output.name)||
	   (this->md.inputs.contains(this->current->value))){
	  treated = false;
	  if(newInstruction){
	  ...
	  if(!treated){
	    this->md.f.body  += this->current->value;
	  }
	} else {
	  this->md.f.body  += this->current->value;
	}
  ...
  void MaterialPropertyDSL::analyse()
  {
	...
	if(this->md.output.name==this->current->value){
