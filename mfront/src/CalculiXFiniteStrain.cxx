/*! 
 * \file  mfront/src/CalculiXFiniteStrain.cxx
 * \brief
...
#include"MFront/CalculiX/CalculiXFiniteStrain.hxx"

namespace calculix
{
  
  void
  CalculiXFiniteStrain::computeGreenLagrangeStrain(CalculiXReal* const e,
						 const CalculiXReal* const F,
  ...
  } // end of CalculiXFiniteStrain::computeGreenLagrangeStrain


  void
  CalculiXFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress(CalculiXReal* const STRESS,
  								      const CalculiXReal* const F,
  								      const CalculiXInt NTENS,
  								      const bool ps,
								      const CalculiXReal Fzz)
  {
    // warning F is given in the fortran convention
    // (F0 F3 F6
    //  F1 F4 F7
    //  F2 F5 F8) 
    const CalculiXReal F0 = F[0];
    const CalculiXReal F1 = F[1];
    const CalculiXReal F2 = F[2];
    const CalculiXReal F3 = F[3];
    const CalculiXReal F4 = F[4];
    const CalculiXReal F5 = F[5];
    const CalculiXReal F6 = F[6];
    const CalculiXReal F7 = F[7];
    const CalculiXReal F8 = !ps ? F[8] : Fzz;
    // determinant
    const CalculiXReal J  = F0*(F4*F8-F5*F7)-F3*(F1*F8-F2*F7)+(F1*F5-F2*F4)*F6;
    // inverse of the determinant
    const CalculiXReal iJ = 1/J;
    // inverse of F
    // maxima          :     F4*F8-F5*F7 
    const CalculiXReal iF0 = iJ*(F4*F8-F5*F7);
    // maxima          :     F2*F7-F1*F8 
    const CalculiXReal iF1 = iJ*(F2*F7-F1*F8);
    // maxima          :     F1*F5-F2*F4 
    const CalculiXReal iF2 = iJ*(F1*F5-F2*F4);
    // maxima          :     F5*F6-F3*F8 
    const CalculiXReal iF3 = iJ*(F5*F6-F3*F8); 
    // maxima          :     F0*F8-F2*F6 
    const CalculiXReal iF4 = iJ*(F0*F8-F2*F6); 
    // maxima          :     F2*F3-F0*F5 
    const CalculiXReal iF5 = iJ*(F2*F3-F0*F5); 
    // maxima          :     F3*F7-F4*F6 
    const CalculiXReal iF6 = iJ*(F3*F7-F4*F6); 
    // maxima          :     F1*F6-F0*F7 
    const CalculiXReal iF7 = iJ*(F1*F6-F0*F7); 
    // maxima          :     F0*F4-F1*F3 
    const CalculiXReal iF8 = iJ*(F0*F4-F1*F3); 
    // sk2
    const CalculiXReal p0 = STRESS[0];
    const CalculiXReal p1 = STRESS[1];
    const CalculiXReal p2 = STRESS[2];
    const CalculiXReal p3 = (NTENS>=4) ? STRESS[3] : CalculiXReal(0);
    const CalculiXReal p4 = (NTENS==6) ? STRESS[4] : CalculiXReal(0);
    const CalculiXReal p5 = (NTENS==6) ? STRESS[5] : CalculiXReal(0);
  ...
  } // end of CalculiXFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress

  void
  CalculiXFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress(CalculiXReal* const sk2,
								      const CalculiXReal* const STRESS,
  								      const CalculiXReal* const F,
  								      const CalculiXInt NTENS,
  								      const bool ps,
								      const CalculiXReal Fzz)
  {
    ...
    CalculiXFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress(sk2,F,NTENS,ps,Fzz);
  } // end of CalculiXFiniteStrain::computeSecondPiolaKirchhoffStressFromCauchyStress

  void
  CalculiXFiniteStrain::computeCauchyStressFromSecondPiolaKirchhoffStress(CalculiXReal* const s,
									const CalculiXReal* const F,
									const CalculiXInt NTENS,
									const bool ps,
									const CalculiXReal Fzz)
  {
    ...
    const CalculiXReal F8 = !ps ? F[8] : Fzz;
    // determinant
    const CalculiXReal inv_J = 1/(F0*(F4*F8-F5*F7)-F3*(F1*F8-F2*F7)+(F1*F5-F2*F4)*F6);
    const CalculiXReal p0 = s[0];
    const CalculiXReal p1 = s[1];
    const CalculiXReal p2 = s[2];
    const CalculiXReal p3 = (NTENS>=4) ? s[3] : CalculiXReal(0);
    const CalculiXReal p4 = (NTENS==6) ? s[4] : CalculiXReal(0);
    const CalculiXReal p5 = (NTENS==6) ? s[5] : CalculiXReal(0);
  ...
  } // end of CalculiXFiniteStrain::computeCauchyStressFromSecondPiolaKirchhoffStress

  void
  CalculiXFiniteStrain::computeCalculiXTangentOperatorFromCSE(CalculiXReal* const D,
							      const CalculiXReal* const CSE,
							      const CalculiXReal* const F,
							      const CalculiXReal* const STRESS,
							      const CalculiXInt NTENS,
							      const bool ps){
  ...
  void
  CalculiXFiniteStrain::applyNativeFiniteStrainCorrection(CalculiXReal* DDSDDE,
							const CalculiXReal* const F1,
							const CalculiXReal* const s,
							const CalculiXInt NTENS)
  {
    using namespace tfel::math;
    // 1/J d(Js)/dde = ds/dde+1/J*(s^dJ_dde)
    auto add = [&DDSDDE,NTENS](const CalculiXInt i,
			       const CalculiXInt j,
			       const CalculiXReal v){
      *(DDSDDE+i+j*NTENS)+=v;
    };
    if(NTENS==6u){
      // tridimensional
      const auto F = tensor<3u,CalculiXReal>::buildFromFortranMatrix(F1);
      ...
      throw(std::runtime_error("CalculiXFiniteStrain::applyNativeFiniteStrainCorrection: "
			       "invalid NTENS value ("+std::to_string(NTENS)+")"));
    }
  } // end of CalculiXFiniteStrain::applyNativeFiniteStrainCorrection
