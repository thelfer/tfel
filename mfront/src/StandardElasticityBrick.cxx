...
namespace mfront {
  ...
  StandardElasticityBrick::StandardElasticityBrick(AbstractBehaviourDSL& dsl_,
    ...
    this->checkOptionsNames(
        d, {"plane_stress_support", "generic_tangent_operator", "generic_prediction_operator"},
        this->getName());
      ...
      if (agps || ps) {
        if (this->bd.getAttribute(BehaviourDescription::requiresStiffnessTensor, false)) {
          if (!this->bd.hasAttribute(BehaviourDescription::requiresUnAlteredStiffnessTensor)) {
            this->bd.setAttribute(BehaviourDescription::requiresUnAlteredStiffnessTensor, true,
                                  false);
          }
          throw_if(
              !this->bd.getAttribute<bool>(BehaviourDescription::requiresUnAlteredStiffnessTensor),
  ...
  void StandardElasticityBrick::endTreatment() const {
    ...
    this->bd.setCode(uh, BehaviourData::Integrator, integrator, BehaviourData::CREATEORAPPEND,
      ...
      if (h == ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRESS) {
        if ((this->bd.getAttribute(BehaviourDescription::requiresStiffnessTensor, false)) ||
          ...
          if ((this->bd.getElasticSymmetryType() != mfront::ISOTROPIC) &&
            ...
            if (this->bd.hasAttribute(BehaviourDescription::requiresUnAlteredStiffnessTensor)) {
              ...
              m += "                       "
                   "(this->D(0,1))/(this->D(1,1))*(this->sigzz+(this->theta)*(this->dsigzz));\n";
              ...
              m += "                       "
                   ...
                   "(this->D(1,1)))*prediction_strain(2)+\n";
              m += "                       "
                   "(this->D(2,1))/(this->D(1,1))*(this->sigzz+(this->theta)*(this->dsigzz));\n";
          ...
          if (this->bd.getElasticSymmetryType() == mfront::ISOTROPIC) {
            auto b =
                this->bd.getAttribute("StandardElasticityBrick::UseLocalLameCoeficients", false);
            const std::string lambda = b ? "this->sebdata.lambda" : "this->lambda";
            const std::string mu = b ? "this->sebdata.mu" : "this->mu";
            m += "StrainStensor prediction_stress;\n";
            m += "StrainStensor prediction_strain = this->eel+(this->theta)*this->deto;\n";
            m += "prediction_stress(0) = 2*(" + mu + ")*((" + lambda + ")/(" + lambda + "+2*(" +
                 mu + "))*(prediction_strain(0)+prediction_strain(2))+prediction_strain(0))+\n";
            m += "(" + lambda + ")/(" + lambda + "+2*(" + mu +
                 "))*(this->sigzz+theta*(this->dsigzz));\n";
            m += "prediction_stress(2) = 2*(" + mu + ")*((" + lambda + ")/(" + lambda + "+2*(" +
                 mu + "))*(prediction_strain(0)+prediction_strain(2))+prediction_strain(2))+\n";
            m += "(" + lambda + ")/(" + lambda + "+2*(" + mu +
                 "))*(this->sigzz+theta*(this->dsigzz));\n";
            ...
            tfel::raise_if(
                !this->bd.getAttribute<bool>(BehaviourDescription::requiresStiffnessTensor),
        ...
        if ((this->bd.getAttribute(BehaviourDescription::requiresStiffnessTensor, false)) ||
          ...
          if (this->bd.getElasticSymmetryType() == mfront::ISOTROPIC) {
            ...
            m += "prediction_stress(0) = 2*(" + mu + ")*((" + lambda + ")/(" + lambda + "+2*(" +
                 mu + "))*(prediction_strain(0)+prediction_strain(1))+prediction_strain(0));\n";
            m += "prediction_stress(1) = 2*(" + mu + ")*((" + lambda + ")/(" + lambda + "+2*(" +
                 mu + "))*(prediction_strain(0)+prediction_strain(1))+prediction_strain(1));\n";
            m += "prediction_stress(2) = stress(0);\n";
  ...
  void StandardElasticityBrick::declareComputeStressWhenStiffnessTensorIsDefined() const {
    ...
    this->bd.setCode(h, BehaviourData::ComputeStress, smts, BehaviourData::CREATE,
                     BehaviourData::AT_BEGINNING, false);
    this->bd.setCode(h, BehaviourData::ComputeFinalStress, sets, BehaviourData::CREATE,
                     BehaviourData::AT_BEGINNING, false);
  ...
  void StandardElasticityBrick::treatIsotropicBehaviour(LocalDataStructure& d) const {
    ...
    if (this->bd.areElasticMaterialPropertiesDefined()) {
      ...
      this->bd.setCode(uh, BehaviourData::BeforeInitializeLocalVariables, init,
                       BehaviourData::CREATEORAPPEND, BehaviourData::AT_BEGINNING, true);
      // Hooke law
      smts.code =
    ...
    this->bd.setCode(uh, BehaviourData::ComputeStress, smts, BehaviourData::CREATE,
                     BehaviourData::AT_BEGINNING, false);
    this->bd.setCode(uh, BehaviourData::ComputeFinalStress, sets, BehaviourData::CREATE,
                     BehaviourData::AT_BEGINNING, false);
  ...
  void StandardElasticityBrick::addAxisymmetricalGeneralisedPlaneStressSupport() const {
    const auto agps = ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRESS;
    CodeBlock integrator;
    ...
    this->bd.setCode(agps, BehaviourData::Integrator, integrator, BehaviourData::CREATEORAPPEND,
                     BehaviourData::AT_BEGINNING);
  }
  ...
  void StandardElasticityBrick::addPlaneStressSupport() const {
    ...
    this->bd.setCode(ps, BehaviourData::Integrator, integrator, BehaviourData::CREATEORAPPEND,
                     BehaviourData::AT_BEGINNING);
  }
  ...
  void StandardElasticityBrick::addGenericTangentOperatorSupport() const {
    ...
    if ((this->bd.getAttribute(BehaviourDescription::requiresStiffnessTensor, false)) ||
        (this->bd.getAttribute(BehaviourDescription::computesStiffnessTensor, false))) {
      const std::string D =
          this->bd.getAttribute(BehaviourDescription::requiresStiffnessTensor, false)
              ? "this->D"
              : "this->D_tdt";
