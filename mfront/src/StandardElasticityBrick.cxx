...
namespace mfront {
  ...
  void StandardElasticityBrick::completeVariableDeclaration() const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(
          b, "StandardElasticityBrick::completeVariableDeclaration: " + m);
    };
    using tfel::glossary::Glossary;
    const auto uh = ModellingHypothesis::UNDEFINEDHYPOTHESIS;
    if (getVerboseMode() >= VERBOSE_DEBUG) {
      getLogStream()
          << "StandardElasticityBrick::completeVariableDeclaration: begin\n";
    }
    ...
    if (getVerboseMode() >= VERBOSE_DEBUG) {
      getLogStream()
          << "StandardElasticityBrick::completeVariableDeclaration: end\n";
    }
  }
        ...
        if ((this->bd.getAttribute(
          ...
          if (this->bd.getElasticSymmetryType() == mfront::ISOTROPIC) {
            auto b = this->bd.getAttribute(
                "StandardElasticityBrick::UseLocalLameCoeficients", false);
  ...
  void StandardElasticityBrick::treatIsotropicBehaviour(
    ...
    if (this->bd.areElasticMaterialPropertiesDefined()) {
      ...
      this->bd.setAttribute("StandardElasticityBrick::UseLocalLameCoeficients",
                            true, false);
  ...
  void StandardElasticityBrick::addGenericTangentOperatorSupport() const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(
          b, "StandardElasticityBrick::addGenericTangentOperatorSupport: " + m);
    };
    const auto& idsl = dynamic_cast<const ImplicitDSLBase&>(this->dsl);
  ...
  void StandardElasticityBrick::addGenericPredictionOperatorSupport() const {
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(
          b,
          "StandardElasticityBrick::addGenericPredictionOperatorSupport: " + m);
    };
    CodeBlock tangentOperator;
    // modelling hypotheses supported by the behaviour
    const auto bmh = bd.getModellingHypotheses();
