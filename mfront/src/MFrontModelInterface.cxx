...
namespace mfront {
  ...
  MFrontModelInterface::treatKeyword(const std::string& k,
    ...
    if (std::find(i.begin(), i.end(), "mfront") != i.end()) {
      throw(
          std::runtime_error("MfrontMaterialPropertyInterface::"
  ...
  void MFrontModelInterface::writeOutputFiles(const FileDescription& fd,
    ...
    auto throw_if = [](const bool b, const std::string& m) {
      if (b) {
        throw(
            std::runtime_error("MFrontModelInterface::writeOutputFiles: " + m));
      }
    };
    auto write_bounds = [&throw_if](std::ostream& out,
                                    const VariableDescription& v,
                                    const std::string& vn) {
      auto write = [&out, &v, &vn, &throw_if](const std::string& bn,
                                              const std::string& p) {
      ...
      if (v.hasAttribute(VariableDescription::physicalBound)) {
        write(VariableDescription::physicalBound, "OutOfBoundsPolicy::Strict");
      }
    };
