...
namespace mfront {
  ...
  std::vector<std::string> DSLBase::getDefaultReservedNames() {
    ...
    names.insert(
        names.end(),
        {"std",   "cout",   "cerr",    "endl",   "cos",   "sin",   "tan",
         "acos",  "asin",   "atan",    "atan2",  "cosh",  "sinh",  "tanh",
         "acosh", "asinh",  "atanh",   "exp",    "frexp", "ldexp", "log",
         "log10", "modf",   "exp2",    "expm1",  "ilogb", "log1p", "log2",
         "logb",  "scalbn", "scalbln", "pow",    "sqrt",  "cbrt",  "hypot",
         "erf",   "erfc",   "tgamma",  "lgamma", "abs"});
    // tfel namespaces
  ...
  std::string DSLBase::getTemporaryVariableName(const std::string& p) {
    ...
    for (unsigned int i = 0; i != std::numeric_limits<unsigned int>::max();
         ++i) {
  ...
  void DSLBase::openFile(const std::string& f,
    ...
    for (const auto& c : ecmds) {
      ...
      try {
        ...
        throw(
            std::runtime_error("DSLBase::openFile : "
  ...
  void DSLBase::treatImport() {
    ...
    for (const auto& f : files) {
      this->importFile(SearchFile::search(f), std::vector<std::string>(), {});
    }
  ...
  void DSLBase::treatIntegerConstant() {
    ...
    this->addStaticVariableDescription(
        StaticVariableDescription("int", name, line, value.second));
  ...
  std::vector<std::string> DSLBase::readSpecifiedValues(
    ...
    auto throw_if = [](const bool b, const std::string& m,
      ...
      if (b) {
        throw(std::runtime_error("DSLBase::readSpecifiedValues : " + m +
                                 "\n"
                                 "Error at line " +
                                 std::to_string(l)));
      }
  ...
  DSLBase::handleMaterialPropertyDescription(const std::string& f) {
    ...
    try {
      ...
      const auto& mname = minterface.getFunctionName(mpd.material, mpd.law);
  ...
  void DSLBase::treatLonelySeparator() {
    if (getPedanticMode()) {
      getLogStream() << this->fileName << ":" << this->current->line << ":"
                     << this->current->offset
