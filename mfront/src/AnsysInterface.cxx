...
namespace mfront {
  ...
  void AnsysInterface::writeUMATxxAdditionalSymbols(
      std::ostream&,
      const std::string&,
      const Hypothesis,
  ...
  void AnsysInterface::writeUMATxxSpecificSymbols(
      std::ostream&,
      const std::string&,
      const BehaviourDescription&,
      const FileDescription&) const {}
  ...
  AnsysInterface::buildMaterialPropertiesList(const BehaviourDescription& mb,
    ...
    if (mb.getSymmetryType() == mfront::ORTHOTROPIC) {
      if ((h == ModellingHypothesis::AXISYMMETRICAL) ||
          ...
          (h == ModellingHypothesis::PLANESTRESS)) {
        this->appendToMaterialPropertiesList(
            mprops, "real", "FirstOrthotropicAxis_1", "v1x", false);
        this->appendToMaterialPropertiesList(
            mprops, "real", "FirstOrthotropicAxis_2", "v1y", false);
      } else if (h == ModellingHypothesis::TRIDIMENSIONAL) {
        this->appendToMaterialPropertiesList(
            mprops, "real", "FirstOrthotropicAxis_1", "v1x", false);
        this->appendToMaterialPropertiesList(
            mprops, "real", "FirstOrthotropicAxis_2", "v1y", false);
        this->appendToMaterialPropertiesList(
            mprops, "real", "FirstOrthotropicAxis_2", "v1z", false);
        this->appendToMaterialPropertiesList(
            mprops, "real", "SecondOrthotropicAxis_1", "v2x", false);
        this->appendToMaterialPropertiesList(
            mprops, "real", "SecondOrthotropicAxis_2", "v2y", false);
        this->appendToMaterialPropertiesList(
            mprops, "real", "SecondOrthotropicAxis_2", "v2z", false);
      } else {
  ...
  AnsysInterface::gatherModellingHypothesesAndTests(
      const BehaviourDescription& mb) const {
  ...
  std::string AnsysInterface::getModellingHypothesisTest(
      const Hypothesis h) const {
    ...
    tfel::raise(
        "AnsysInterface::getModellingHypothesisTest : "
  ...
  void AnsysInterface::writeInputFileExample(const BehaviourDescription& mb,
    ...
    try {
      ...
      // header
      out << "/comm, \n"
          << "/comm, File generated by MFront from the " << fd.fileName
          << " source\n"
          << "/comm, Example of how to use the " << mb.getClassName()
          << " behaviour law\n"
  ...
  void AnsysInterface::endTreatment(const BehaviourDescription& mb,
    ...
    std::ofstream out;
    {
      ...
      if (out) {
        ...
        const auto fn =
            root + "/share/doc/mfront-" VERSION "/ansys/usermat.cppp";
  ...
  void AnsysInterface::writeUMATxxBehaviourTypeSymbols(
    ...
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(b,
                     "AnsysInterface::writeUMATxxBehaviourTypeSymbols: " + m);
    };
  ...
  void AnsysInterface::writeUMATxxBehaviourKinematicSymbols(
    ...
    auto throw_if = [](const bool b, const std::string& m) {
      tfel::raise_if(
          b, "AnsysInterface::writeUMATxxBehaviourKinematicSymbols: " + m);
    };
    out << "MFRONT_SHAREDOBJ unsigned short " << this->getFunctionName(name)
        << "_BehaviourKinematic = ";
    if (mb.getBehaviourType() ==
