...
namespace mfront {
  ...
  static std::string getLibraryLinkFlags(const TargetsDescription& t,
    ...
    if (!describes(t, name)) {
      throw(
          std::runtime_error("getLibraryLinkFlags : no library "
                             "named '" +
                             name + "'.\nInternal Error."));
    }
    const auto& l = t[name];
  ...
  void generateMakeFile(const TargetsDescription& t,
    ...
    if (!m) {
      throw(
          runtime_error("generateMakeFile : can't open file '" + mfile + "'"));
    }
    ...
    for (const auto& l : t) {
      tmp_cppflags.insert(tmp_cppflags.end(), l.cppflags.begin(),
                          l.cppflags.end());
    }
