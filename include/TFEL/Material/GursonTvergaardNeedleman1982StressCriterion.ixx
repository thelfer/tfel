...
std::ostream &operator<<(std::ostream &os,
  ...
  os << "{f_c: " << p.f_c << ", f_r: " << p.f_r << ", q_1: " << p.q_1 << ", q_2: " << p.q_2 << ", q_3: " << p.q_3 << "}";
  return os;
} // end of operator <<

template <typename StressStensor>
GursonTvergaardNeedleman1982StressType<StressStensor> computeGursonTvergaardNeedleman1982Stress(
   ...
   *     + 2 q1 f_{*} \cosh\left( \frac{3\, q2\, \sigma_m}{2\,\sigma^{\star}}\right)
   *     - 1 - q3 (f_{*})^2
   * \]
   *
   ...
   */
    ...
    // special cases
    if (std::abs(pr) < seps) {
      const auto iomf = 1 / std::max(1 - 2 * p.q_1 * fstar + p.q_3 * fstar * fstar, real(1.e-12));
      return seq * std::sqrt(iomf);
    }
    if (seq < seps) {
      const auto argach = (1 + p.q_3 * fstar * fstar) / std::max(2 * p.q_1 * fstar, real(1.e-12));
    ...
    // second order)
    const auto x0 = std::sqrt((4 * (1 + p.q_3 * fstar * fstar - 2 * p.q_1 * fstar)) / (4 * seq2 + 9 * p.q_1 * p.q_2 * p.q_2 * fstar * pr2));
    //
    const auto SdS = [seq2, pr, fstar, p](const istress x) {
      ...
      const auto S = seq2 * x * x + 2 * p.q_1 * fstar * ch - 1 - p.q_3 * fstar2;
...
const auto r = tfel::math::scalarNewtonRaphson(SdS, c, x0, size_type{100});
 ...
 return 1 / std::get<1>(r);
  ...
  template <typename StressStensor>
  std::tuple<GursonTvergaardNeedleman1982StressType<StressStensor>,
  ...
  computeGursonTvergaardNeedleman1982StressNormal(
    ...
    // normal
    const auto dS_dss =
      (-2 * power<2>(seq) * iss - 3 * q1q2 * fstar  * pr * sh) * power<2>(iss);
  ...
  computeGursonTvergaardNeedleman1982StressSecondDerivative(
    ...
    const auto d2S_dsig2    = (2. * M + (q1q2 * p.q_2 * (fstar / 2) * ch * (id ^ id)))  * power<2>(iss)  ;
    const auto d2S_dsigdss  = (-6. * s * iss - q1q2 * fstar * sh * id - //
			       3 * q1q2 * p.q_2 * (fstar / 2) * pr * iss * ch * id) * power<2>(iss) ;
    const auto d2S_dss2     = ((6 * seq2 * iss) + (3 * q1q2 * fstar * pr) * //
			       (2 * sh + 3 * p.q_2 * pr * (iss / 2) * ch)) * power<3>(iss);
    const auto dn_dsig = - idS_dss * (d2S_dsig2 + (d2S_dsigdss ^ n)) + //
