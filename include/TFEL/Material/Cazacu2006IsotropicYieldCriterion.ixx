...
namespace tfel{
  ...
  namespace material{
    ...
    namespace internals{
       ...
       * \param[out] d2Psi_ds2: second derivative of the Cazacu equivalent stress
       * \param[in] dPsi_dvp: first derivative of the Cazacu
       * equivalent stress with respect to the eigenvalue
       * \param[in] d2Psi_dvp2: second derivative of the Cazacu
       * equivalent stress with respect to the eigenvalue
      ...
      computeCazacu2006IsotropicStressSecondDerivative(tfel::material::CazacuStressSecondDerivativeType<StressStensor>& d2Psi_ds2,
						       ...
						       const tfel::math::tvector<6u,tfel::material::CazacuInvertStressType<StressStensor>>& d2Psi_dvp2,
	...
	d2Psi_ds2 = (d2Psi_dvp2[0]*(n0^n0)+d2Psi_dvp2[1]*(n1^n1)+d2Psi_dvp2[2]*(n2^n2));
	if(std::abs(vp(0)-vp(1))<e){
      ...
      computeCazacu2006IsotropicStressSecondDerivative(tfel::material::CazacuStressSecondDerivativeType<StressStensor>& d2Psi_ds2,
						       const tfel::math::tvector<3u,tfel::material::CazacuBaseType<StressStensor>>& dPsi_dvp,
						       const tfel::math::tvector<3u,tfel::material::CazacuInvertStressType<StressStensor>>& d2Psi_dvp2,
						       const tfel::material::CazacuStressEigenTensorType<StressStensor>& n0,
						       const tfel::material::CazacuStressEigenTensorType<StressStensor>& n1,
						       const tfel::material::CazacuStressEigenTensorType<StressStensor>& n2,
						       const tfel::math::tvector<3u,CazacuStressType<StressStensor>>& vp,
    ...
    computeCazacu2006IsotropicStress(const StressStensor& sig,
	...
	return seq*0;
      ...
      return seq*std::pow(std::pow(std::abs(std::abs(vp[0])-k*vp[0]),a)+
    ...
    computeCazacu2006IsotropicStressNormal(const StressStensor& sig,
					   const CazacuExponentType a,
					   const CazacuBaseType<StressTensor> k,
					   const CazacuStressType<StressStensor> e)
    {
      ...
      const real rPsi_am1[3] = {std::pow(r[0],a-1),
				std::pow(r[1],a-1),
				std::pow(r[2],a-1)};
      const real rPsi_a   =  rPsi_am1[0]*r[0]+rPsi_am1[1]*r[1]+rPsi_am1[2]*r[2];
    ...
    computeCazacu2006IsotropicStressSecondDerivative(const StressStensor& sig,
						     const CazacuExponentType a,
						     const CazacuBaseType<StressTensor> k,
						     const CazacuStressType<StressStensor> e)
    {
      ...
      const auto seq   = sigmaeq(s);
      if(seq<e){
    	return std::make_tuple(stress{0},normal{real{0}},n
      ...
      // 	{cste*( (drvp2_am2[0]+drvp2_am2[1])/2-dPsi_dsvp[0]*dPsi_dsvp[0]), // s1s1
      // 	 cste*( (drvp2_am2[0]+drvp2_am2[2])/2-dPsi_dsvp[1]*dPsi_dsvp[1]), // s2s2
      // 	 cste*( (drvp2_am2[1]+drvp2_am2[2])/2-dPsi_dsvp[2]*dPsi_dsvp[2]), // s3s3
