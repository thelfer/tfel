/*!
 * \file   include/TFEL/Material/PhaseDescription.hxx
...
#ifndef LIB_TFEL_MATERIAL_PHASEDESCRIPTION_HXX
#define LIB_TFEL_MATERIAL_PHASEDESCRIPTION_HXX
...
namespace tfel::material::homogenization::elasticity {
  ...
  requires(tfel::math::checkUnitCompatibility<tfel::math::unit::Stress,
    ...
    [[nodiscard]] tfel::math::st2tost2<N, StressType> getElasticityOfPhase()
        const {
      return this->stiffness;
    }

    void changeElasticityOfPhase(const tfel::math::st2tost2<N, StressType>& C) {
      this->stiffness = C;
      this->isotropic = false;
    }
    void changeElasticityOfPhase(const IsotropicModuli<StressType>& IM) {
      this->stiffness = computeIsotropicStiffnessTensor<StressType>(IM);
      this->isotropic = true;
    }

    [[nodiscard]] bool is_isotropic() const { return this->isotropic; }
    [[nodiscard]] bool isIsotropic() const { return this->isotropic; }
  ...
  requires(tfel::math::checkUnitCompatibility<
           ...
           StressType>()) struct InclusionDistribution : Phase<N, StressType> {
    using real = tfel::types::real<StressType>;
    ...
    [[nodiscard]] virtual std::unique_ptr<InclusionDistribution<N, StressType>>
    clone() const = 0;

    [[nodiscard]] virtual tfel::math::st2tost2<N, real> computeMeanLocalisator(
        const tfel::math::st2tost2<N, StressType>& C0,
        int max_iter_anisotropic_integration = 12) = 0;
    [[nodiscard]] virtual tfel::math::st2tost2<N, real> computeMeanLocalisator(
        const IsotropicModuli<StressType>& IM0) = 0;
    [[nodiscard]] virtual tfel::math::st2tost2<N, compliance>
    computeDerivativesOfMeanLocalisator(const IsotropicModuli<StressType>& IM0,
                                        const std::array<real, 4>& dKG) = 0;
    virtual ~InclusionDistribution() = default;

  };  // end of InclusionDistribution
  ...
  requires(tfel::math::checkUnitCompatibility<
    ...
    [[nodiscard]] std::unique_ptr<InclusionDistribution<3u, StressType>> clone()
        const override {
    ...
    computeDerivativesOfMeanLocalisator(
      ...
      if (this->isIsotropic()) {
        ...
        const auto dbeta0 = (5 * (dmu0 * alpha0 + mu0 * dalpha0) * denb -
                             5 * mu0 * alpha0 * (dk0 + 2 * dmu0)) /
                            denb / denb;
        const auto da_ = (3 * (dki - dk0) * alpha0 - 3 * (ki - k0) * dalpha0) /
                         alpha0 / alpha0;
        const auto db_ =
            (6 * (dmui - dmu0) * beta0 - 6 * (mui - mu0) * dbeta0) / beta0 /
            beta0;
        const auto denJ = 1. + 3. * (ki - k0) / alpha0;
        const auto daJ = -1. / denJ / denJ * da_;
        const auto denK =
            1. + 6. * (mui - mu0) * (k0 + 2. * mu0) / 5. / mu0 / alpha0;
        const auto daK = -1. / denK / denK * db_;
        return daJ * tfel::math::st2tost2<3u, real>::J() +
               daK * tfel::math::st2tost2<3u, real>::K();
      } else {
        tfel::reportContractViolation(
            "I cannot compute the derivatives of the mean localisator "
            "for a distribution of inclusions with elasticities defined as "
            "st2tost2. "
            "Try to use a IsotropicModuli for the definition of inclusion "
            "elasticity.");
      }
    }
  ...
  requires(tfel::math::checkUnitCompatibility<
    ...
    [[nodiscard]] tfel::math::st2tost2<3u, real> computeMeanLocalisator(
      ...
      tfel::reportContractViolation(
          ...
          "oriented ellipsoidal inclusions in an anisotropic matrix. Try to "
    ...
    computeDerivativesOfMeanLocalisator(
      ...
      if (tfel::math::ieee754::fpclassify(semiL[1] - semiL[2]) == FP_ZERO) {
        ...
        return DerivativesOfMeanLocalisator<3u, StressType>(
            IM0, KGi, e, real(1. / 3.) * tfel::math::stensor<3u, real>::Id(),
            real(1. / 3.) * tfel::math::st2tost2<3u, real>::J() +
                real(2. / 15.) * tfel::math::st2tost2<3u, real>::K(),
            dKG);
      } else {
        tfel::reportContractViolation(
            "I cannot compute the derivatives of the mean localisator "
  ...
  requires(tfel::math::checkUnitCompatibility<
    ...
    [[nodiscard]] std::unique_ptr<InclusionDistribution<3u, StressType>> clone()
        const override {
      return std::make_unique<TransverseIsotropicDistribution<StressType>>(
          *this);
    }
    ...
    [[nodiscard]] tfel::math::st2tost2<3u, real> computeMeanLocalisator(
      ...
      if (ind == 0) {
        ...
        tfel::reportContractViolation(
            "Bad index of ellipsoid semi-length. The index must be 0,1 or "
            "2.");
      }
  ...
  requires(tfel::math::checkUnitCompatibility<
    ...
    [[nodiscard]] std::unique_ptr<InclusionDistribution<3u, StressType>> clone()
        const override {
      return std::make_unique<UserDefinedDistributionOfSpheroids<StressType>>(
          *this);
    }
    ...
    [[nodiscard]] tfel::math::st2tost2<3u, real> computeMeanLocalisator(
      ...
      const auto KGi = computeKGModuli<StressType>(Ci);
      auto semiL = (this->inclusion).semiLengths;
      return EllipsoidMeanLocalisator<
          3u, StressType>::UserDefinedDistributionOfSpheroids(IM0, KGi,
                                                              semiL[0],
                                                              semiL[1], A2, A4);
    }
    ...
    computeDerivativesOfMeanLocalisator(
      ...
      auto semiL = (this->inclusion).semiLengths;
      const auto e = real(semiL[0] / semiL[1]);
      const tfel::math::st2tost2<3u, compliance> dA =
          DerivativesOfMeanLocalisator<3u, StressType>(IM0, KGi, e, A2, A4,
                                                       dKG);
      return dA;
    }
  };  // End of UserDefinedDistributionOfSpheroids

  /////Definition of 'Grain' objects
...
#endif /* LIB_TFEL_MATERIAL_PHASEDESCRIPTION_HXX */
