...
namespace tfel::material::homogenization::elasticity {
  ...
  requires(tfel::math::checkUnitCompatibility<
    ...
    const auto dmuP_dmu0 =
        5 * (k0 + 4 * mu0 / 3) / 2 / (k0 + 2 * mu0) +
        5 * mu0 / 2 * (4 * (k0 + 2 * mu0) / 3 - 2 * (k0 + 4 * mu0 / 3)) /
            (k0 + 2 * mu0) / (k0 + 2 * mu0);

    const auto den_mu = 1 + (1 - f) * (mui - mu0) / muP;
    const auto den_k = 1 + (1 - f) * (ki - k0) / kP;
    const auto dermuHS_mu0 =
        1 + f *
                (-den_mu - (mui - mu0) * (1 - f) *
                               (-muP - (mui - mu0) * dmuP_dmu0) / muP / muP) /
                den_mu / den_mu;
    const auto derkHS_mu0 =
        f * (-(ki - k0) * (1 - f) * (-4 * (ki - k0) / 3) / kP / kP) / den_k /
        den_k;
    const auto dermuHS_mui =
        f * (den_mu - (mui - mu0) * (1 - f) / muP) / den_mu / den_mu;
    const auto eeq20 =
        1 / (1 - f) * (3 * derkHS_mu0 * em2 + dermuHS_mu0 * eeq2);
  ...
  requires(tfel::math::checkUnitCompatibility<
    ...
    const auto dermuHS_k0 = f * (mui - mu0) * (1 - f) * (mui - mu0) * dmuP_dk0 /
                            muP / muP / den_mu / den_mu;
    const auto derkHS_k0 =
        1 + f * (-den_k - (ki - k0) * (1 - f) * (-kP - (ki - k0)) / kP / kP) /
                den_k / den_k;
    const auto derkHS_ki =
        f * (den_k - (ki - k0) * (1 - f) / kP) / den_k / den_k;
