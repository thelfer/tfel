...
namespace tfel::material::homogenization::elasticity {
  ...
  namespace internals {
    ...
    requires(tfel::math::checkUnitCompatibility<tfel::math::unit::Stress,
                                                StressType>())
            TFEL_HOST_DEVICE std::array < types::squared_compliance<StressType>,
    6 > computeDerivativesOfHillTensorComponents(
      ...
      const auto dbeta0 = ((6 * dk0 + 2 * dmu0) * den -
    ...
    requires(tfel::math::checkUnitCompatibility<tfel::math::unit::Stress,
                                                StressType>())
            TFEL_HOST_DEVICE std::array < types::compliance<StressType>,
    6 > computeDerivativesOfLocalisationTensorComponents(
            const KGModuli<StressType>& kg0,
      ...
      const auto dp =
          computeDerivativesOfHillTensorComponents(kg0, e, {dkg[0], dkg[1]});
      ...
      const tmatrix<2, 2, real> J = {{1. / 3, sqrt(2) / 3},
                                     {sqrt(2) / 3, 2. / 3}};
      const tmatrix<2, 2, real> K = {{2. / 3, -sqrt(2) / 3},
                                     {-sqrt(2) / 3, 1. / 3}};
   ...
   * oblate)
   * \param[in] dkg0 : for a derivation w.r.t. the bulk modulus, this
   * std::array<real,2> must be {1,0}, and {0,1} for a derivation w.r.t. the
   * shear modulus.
   */
  template <tfel::math::ScalarConcept StressType>
  requires(tfel::math::checkUnitCompatibility<
      ...
      st2tost2<3u, types::squared_compliance<StressType>> computeDerivativesOfAxisymmetricalHillTensor(
          const KGModuli<StressType>& kg0,
          const tfel::math::tvector<3u, types::real<StressType>>& n_,
          const types::real<StressType>& e,
          const std::array<types::real<StressType>, 2>& dkg0,
          const tfel::math::base_type<StressType> precision = tfel::material::
    ...
    if (std::abs(e - 1) < precision) {
      ...
      const auto dmuP = 5 * dmu0 / 2 * kP / (k0 + 2 * mu0) +
                        5 * mu0 / 2 *
                            (dkP * (k0 + 2 * mu0) - kP * (dk0 + 2 * dmu0)) /
                            (k0 + 2 * mu0) / (k0 + 2 * mu0);
      const auto dPSphere =
          -dkP / 3 / kP / kP * tfel::math::st2tost2<3u, real>::J() -
          dmuP / 2 / muP / muP * tfel::math::st2tost2<3u, real>::K();
      return dPSphere;
    } else {
      const auto dp =
          internals::computeDerivativesOfHillTensorComponents(kg0, e, dkg0);
      using namespace tfel::math;
      return dp[0] * TransverseIsotropicWalpoleBasis<real>::E1(n_) +
             dp[1] * TransverseIsotropicWalpoleBasis<real>::E2(n_) +
             dp[2] * TransverseIsotropicWalpoleBasis<real>::E3(n_) +
             dp[3] * TransverseIsotropicWalpoleBasis<real>::E4(n_) +
             dp[4] * TransverseIsotropicWalpoleBasis<real>::F(n_) +
             dp[5] * TransverseIsotropicWalpoleBasis<real>::G(n_);
    }
   ...
   * \param[in] e: aspect ratio of the spheroid (e>1 : prolate, e<1 :
   * oblate)
   * \param[in] dkg : for a derivation w.r.t. the matrix bulk modulus, this
   * std::array<real,4> must be {1,0,0,0}; for a derivation w.r.t. the matrix
   * shear modulus, it must be {0,1,0,0}, etc...
   */
  template <tfel::math::ScalarConcept StressType>
  requires(tfel::math::checkUnitCompatibility<
           tfel::math::unit::Stress,
    ...
    if (std::abs(e - 1) < precision) {
      ...
      const auto dASphere =
          -ddenJ / denJ / denJ * tfel::math::st2tost2<3u, real>::J() -
          ddenK / denK / denK * tfel::math::st2tost2<3u, real>::K();
      return dASphere;
    } else {
      const auto da =
          internals::computeDerivativesOfLocalisationTensorComponents(kg0, kgi,
                                                                      e, dkg);
      using namespace tfel::math;
      return da[0] * TransverseIsotropicWalpoleBasis<real>::E1(n_) +
             da[1] * TransverseIsotropicWalpoleBasis<real>::E2(n_) +
             da[2] * TransverseIsotropicWalpoleBasis<real>::E3(n_) +
             da[3] * TransverseIsotropicWalpoleBasis<real>::E4(n_) +
             da[4] * TransverseIsotropicWalpoleBasis<real>::F(n_) +
             da[5] * TransverseIsotropicWalpoleBasis<real>::G(n_);
    }
