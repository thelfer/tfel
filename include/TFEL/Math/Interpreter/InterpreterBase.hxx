  ...
  namespace math
  {
	...
	InterpreterBase(){
	  ...
	  this->functions->insert(MVType("pi",
					 ExternalFunctionPtr(new Evaluator(M_PI))));
	...
	unsigned short currentLineNbr;
      ...
      InterpreterBase<Child>::registerCallBack(std::map<std::string,
					       typename InterpreterBase<Child>::MemFuncPtr>& m,
					       const std::string& name,
					       const typename InterpreterBase<Child>::MemFuncPtr f)
      {
	using namespace std;
	...
	this->registerCallBack(this->setCallBacks,"gsl",&Child::treatSetGSL);
      ...
      template<typename Child>
      void
      InterpreterBase<Child>::treatSet(typename InterpreterCommon::TokensContainer::const_iterator& p,
				       const typename InterpreterCommon::TokensContainer::const_iterator pe)
      {
	using namespace std;
