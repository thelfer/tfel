...
#ifndef _LIB_TFEL_MATH_CADNA_HXX_
#define _LIB_TFEL_MATH_CADNA_HXX_
...
namespace tfel{
  ...
  namespace math{
    ...
    struct ResultType<cadna::numeric_type<T1>,cadna::numeric_type<T2>,Op>{
      ...
      using dispatch = typename std::conditional<b,internals::ResultType1<T1,T2,Op>,
    ...
    struct ResultType<cadna::numeric_type<T1>,T2,Op>{
      ...
      using dispatch = typename std::conditional<b,internals::ResultType2<T1,T2,Op>,
						 internals::InvalidResult>::type;
      using type = typename dispatch::type;
    };
    ...
    struct ResultType<T1,cadna::numeric_type<T2>,Op>{
      static constexpr bool b = (cadna::is_cxx_fundamental_type<T1>::value&&
				 std::is_floating_point<T2>::value);
      using dispatch = typename std::conditional<b,internals::ResultType3<T1,T2,Op>,
						 internals::InvalidResult>::type;
      using type = typename dispatch::type;
    };
    ...
    {
      using type = cadna::numeric_type<typename UnaryResultType<T,Power<N,D>>::type>;
    };
  ...
  namespace typetraits{
    ...
    namespace internals{
      ...
      {
	static constexpr const bool value = std::is_same<typename tfel::typetraits::Promote<T1,T2>::type,T2>::value;
      };
    }
    ...
    {
      static constexpr const bool b = (cadna::is_cxx_fundamental_type<T1>::value&&
				       std::is_floating_point<T2>::value);
      using dispatch = typename std::conditional<b,internals::IsAssignableTo<T1,T2>,
						 std::false_type>::type;
...
#endif /* _LIB_TFEL_MATH_CADNA_HXX_ */
