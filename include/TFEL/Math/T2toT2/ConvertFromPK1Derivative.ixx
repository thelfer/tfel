...
namespace tfel::math::internals {
  ...
  struct DPK1_DF_to_DTAU_DF_Converter<1u> {
    ...
    TFEL_HOST_DEVICE constexpr static void exe(
        tfel::math::t2tost2<1u, stress>& dtau,
  ...
  struct DPK1_DF_to_DTAU_DF_Converter<2u> {
    ...
    template <typename stress, typename real>
    TFEL_HOST_DEVICE constexpr static void exe(
        tfel::math::t2tost2<2u, stress>& dtau,
  ...
  struct DPK1_DF_to_DTAU_DF_Converter<3u> {
    ...
    template <typename stress, typename real>
    TFEL_HOST_DEVICE constexpr static void exe(
        tfel::math::t2tost2<3u, stress>& dtau,
      ...
      dtau(3, 0) =
          cste * (dP(7, 0) * F[5] + dP(1, 0) * F[3] + F[0] * dP(4, 0) + P[4]);
      dtau(4, 0) =
          cste * (dP(2, 0) * F[5] + dP(8, 0) * F[3] + F[0] * dP(6, 0) + P[6]);
      ...
      dtau(5, 1) =
          cste * (dP(2, 1) * F[7] + dP(6, 1) * F[4] + dP(8, 1) * F[1] + P[8]);
      // f90(diff(tau[1],F[2]));
      ...
      dtau(3, 3) =
          cste * (dP(7, 3) * F[5] + dP(1, 3) * F[3] + F[0] * dP(4, 3) + P[1]);
      dtau(4, 3) =
          cste * (dP(2, 3) * F[5] + dP(8, 3) * F[3] + F[0] * dP(6, 3) + P[8]);
      ...
      dtau(5, 4) =
          cste * (dP(2, 4) * F[7] + dP(6, 4) * F[4] + dP(8, 4) * F[1] + P[6]);
      // f90(diff(tau[1],F[5]));
      ...
      dtau(3, 5) =
          cste * (dP(7, 5) * F[5] + dP(1, 5) * F[3] + F[0] * dP(4, 5) + P[7]);
      dtau(4, 5) =
          cste * (dP(2, 5) * F[5] + dP(8, 5) * F[3] + F[0] * dP(6, 5) + P[2]);
      ...
      dtau(2, 6) = dP(8, 6) * F[8] + dP(6, 6) * F[6] + dP(2, 6) * F[2] + P[6];
      dtau(3, 6) = cste * (dP(7, 6) * F[5] + dP(1, 6) * F[3] + F[0] * dP(4, 6));
      dtau(4, 6) = cste * (dP(2, 6) * F[5] + dP(8, 6) * F[3] + F[0] * dP(6, 6));
      dtau(5, 6) = cste * (dP(2, 6) * F[7] + dP(6, 6) * F[4] + dP(8, 6) * F[1]);
      // f90(7iff(tau[1],F[7]));
      ...
      dtau(5, 7) =
          cste * (dP(2, 7) * F[7] + dP(6, 7) * F[4] + dP(8, 7) * F[1] + P[2]);
      // f90(diff(tau[1],F[8]));
...
namespace tfel::math {
  ...
  convertFirstPiolaKirchoffStressDerivativeToKirchhoffStressDerivative(
    ...
    tfel::math::internals::DPK1_DF_to_DTAU_DF_Converter<N>::exe(dtau, dP, F, s);
  }  // end of
     // convertFirstPiolaKirchoffStressDerivativeToKirchhoffStressDerivative

  template <unsigned short N, typename stress, typename real>
  TFEL_HOST_DEVICE constexpr t2tost2<N, stress>
  convertFirstPiolaKirchoffStressDerivativeToKirchhoffStressDerivative(
      const t2tot2<N, stress>& dP,
      const tensor<N, real>& F,
      const stensor<N, stress>& s) noexcept {
