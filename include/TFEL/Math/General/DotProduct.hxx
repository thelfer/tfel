...
namespace tfel{
  ...
  namespace math{
    ...
    /*!
     * \class dotProduct 
     * \brief dotProduct calculates a generalized inner product of the ranges [first1,first1+N) and [first2,first2+N)
     * \param N number of element of each ranges
     *
     * Contrary to inner_product, dotProduct takes automatically into account the fact that complex must be conjugated
     *
     * \see inner_product
     *
     * \author Thomas Helfer
    ...
    {
      /*! 
       * The first version of dotProduct returns init plus the inner product of the two ranges [1]. That is, it first initializes the result to init and then computes:
    ...
    struct dotProduct<0u>
    {
      ...
      template <typename InputIterator1, typename InputIterator2, typename T>
      static TFEL_MATH_INLINE
      T exe(InputIterator1, InputIterator2, T init)
      {
	return init;
      }
    ...
    struct DotProduct
    {
      ...
      static TFEL_MATH_INLINE
      const T
      exe(const T1&a,const T2& b)
      {
	return a(M)*b(M)+DotProduct<N-1,M+1>::template exe<T,T1,T2>(a,b);
      }
    };
