...
namespace tfel{
  ...
  namespace math{
	...
	applyFromValue(const T2& v, Tn ...vs)
	{
	  return fsarray_initializer<N-1>::template applyFromValue<T,T2,Tn...>(v, v, vs...);
	}
	template <typename T,typename T2, typename ...Tn>
	static constexpr array_container<sizeof...(Tn)+N,T>
	applyFromInitializerList(const std::initializer_list<T2>& v, Tn ...vs)
	{
	  return fsarray_initializer<N-1>::template applyFromInitializerList<T,T2,T2,Tn...>(v, *(v.begin()+N-1), vs...);
	}
	template <typename T,typename Container, typename ...Tn>
	static constexpr array_container<sizeof...(Tn)+N,T>
	applyFromArray(const Container& v, Tn ...vs)
	{
	  return fsarray_initializer<N-1>::template applyFromArray<T,Container,decltype(v[N-1]),Tn...>(v,v[N-1], vs...);
	}
      }; // end of struct fsarray_initializer<unsigned short N>
      /*!
       * \brief partial specialisation of fsarray_initializer to end the recursion
       */
      template <>
	...
	template <typename T, typename T2, typename ...Tn>
	static constexpr array_container<sizeof...(Tn)+1,T>
	applyFromArray(const T2& v, Tn ...vs)
	{
    ...
    constexpr fsarray<N,T>::fsarray(const fsarray<N,T2>& values)
      : array_container<N,T>(tfel::math::internals::fsarray_initializer<N>::template applyFromArray<T>(values))
    {}
    ...
    constexpr fsarray<N,T>::fsarray(const T2& value)
      : array_container<N,T>(tfel::math::internals::fsarray_initializer<N>::template applyFromValue<T>(value))
    {}
    ...
    template<unsigned short N, typename T>
    typename fsarray<N,T>::reference
    fsarray<N,T>::operator[](const size_type i) noexcept
    {
      return this->v[i];
    }

    template<unsigned short N, typename T>
    constexpr typename fsarray<N,T>::const_reference
    fsarray<N,T>::operator[](const size_type i) const noexcept
    {
      return this->v[i];
    }
