...
namespace tfel::math {
  ...
  namespace internals {
    ...
    struct fsarray_initializer<1u> {
      ...
      static constexpr array_container<sizeof...(Tn), T> buildArray(Tn... vs) {
  ...
  template <unsigned short N, typename T>
  typename fsarray<N, T>::const_iterator fsarray<N, T>::begin() const noexcept {
    return this->v;
  }
  ...
  template <unsigned short N, typename T>
  typename fsarray<N, T>::const_iterator fsarray<N, T>::cend() const noexcept {
    return this->v + N;
  }
