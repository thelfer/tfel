  ...
  namespace math{
    ...
    struct GteSymmetricEigensolver3x3
    {
      ...
      static int exe(tvector<3u,real>& vp,
		     ...
		     real a00, real a01, real a02, real a11, real a12, real a22,
		     bool aggressive);
    ...
    private:
      ...
      // entries.
      static bool converged(bool aggressive, real bDiag0, real bDiag1,
			    real bSuper);
    ...
    template <typename real>
    struct GteNISymmetricEigensolver3x3
    {
      // The input matrix must be symmetric, so only the unique elements must
      // be specified: a00, a01, a02, a11, a12, and a22.  The eigenvalues are
      // sorted in ascending order: vp0 <= vp1 <= vp2.
    
      static void exe(tvector<3u,real>& vp, tmatrix<3u,3u,real>& m,
		      real a00, real a01, real a02, real a11, real a12, real a22);
    
    private:
      
      static tvector<3u,real> cross_product(const tvector<3u,real>& U,
					    const tvector<3u,real>& V);

      static void computeOrthogonalComplement(const tvector<3u,real>& W,
					      tvector<3u,real>& U,
					      tvector<3u,real>& V);

      static void computeEigenvector0(real a00, real a01, real a02, real a11, real a12, real a22,
				      real& vp0, tvector<3u,real>& m0);

      static void computeEigenvector1(real a00, real a01, real a02, real a11, real a12, real a22,
				      const tvector<3u,real>& m0, real& vp1, tvector<3u,real>& m1);
    };
