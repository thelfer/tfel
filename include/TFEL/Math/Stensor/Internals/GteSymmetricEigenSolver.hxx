  ...
  namespace math {
    ...
    namespace internals {
      ...
      struct GteSymmetricEigensolver3x3 {
        ...
        //
        // The order of the eigenvalues is specified by sortType: -1
        // (decreasing), 0 (no sorting), or +1 (increasing).  When sorted, the
        // eigenvectors are ordered accordingly, and {m[0], m[1], m[2]} is
        // guaranteed to be a right-handed orthonormal set.  The return value is
       ...
       private:
        ...
        // sqrt call.  The normalized pair is (cs,sn) with cs <= 0.  If (u,v) =
        // (0,0), the function returns (cs,sn) = (-1,0).  When used to generate
        // a Householder reflection, it does not matter whether (cs,sn) or
        // (-cs,-sn) is used.  When generating a Givens reflection, cs =
        // cos(2*theta) and sn = sin(2*theta).  Having a negative cosine for the
        // double-angle term ensures that the single-angle terms c = cos(theta)
        // and s = sin(theta) satisfy |c| <= |s|.
        ...
        // superdiagonal test is "|bDiag0| + |bDiag1| + |bSuper| == |bDiag0| +
        // |bDiag1|, which means bSuper is effectively zero compared to the
        // sizes of the diagonal entries.
        ...
        // (i0,i1,i2) is a permutation of (0,1,2) so that d[i0] <= d[i1] <=
        // d[i2]. The 'bool' return indicates whether the permutation is odd. If
        // it is not, the handedness of the Q matrix must be adjusted.
