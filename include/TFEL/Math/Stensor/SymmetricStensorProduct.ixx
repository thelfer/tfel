...
namespace tfel{
  ...
  namespace math{
      ...
      stensor<2u,decltype(a[0]*b[0]*a[0])>>::type
    {
	  ...
	  (2*a[1]*a[3]*b[3]+2*a[1]*a[1]*b[1]+a[3]*a[3]*b[0])/2,
	  a[2]*a[2]*b[2],
	  ((a[3]*a[3]+2*a[0]*a[1])*b[3]+2*a[1]*a[3]*b[1]+2*a[0]*a[3]*b[0])/2};
    } // end of symmetric_product_aba
      ...
      stensor<3u,decltype(a[0]*b[0]*a[0])>>::type
    {
	  ...
	  // third line
	  (2*a[2]*a[5]*b[5]+2*a[2]*a[4]*b[4]+cste*a[4]*a[5]*b[3]+2*a[2]*a[2]*b[2]+a[5]*a[5]*b[1]+a[4]*a[4]*b[0])/2,
	  // fourth line
	  ((a[3]*a[5]+cste*a[1]*a[4])*b[5]+(cste*a[0]*a[5]+a[3]*a[4])*b[4]+(a[3]*a[3]+2*a[0]*a[1])*b[3]+cste*a[4]*a[5]*b[2]+2*a[1]*a[3]*b[1]+2*a[0]*a[3]*b[0])/2,
	  // fifth line
	  ((a[4]*a[5]+cste*a[2]*a[3])*b[5]+(a[4]*a[4]+2*a[0]*a[2])*b[4]+(cste*a[0]*a[5]+a[3]*a[4])*b[3]+2*a[2]*a[4]*b[2]+cste*a[3]*a[5]*b[1]+2*a[0]*a[4]*b[0])/2,
	  // sixtth line
	  ((a[5]*a[5]+2*a[1]*a[2])*b[5]+(a[4]*a[5]+cste*a[2]*a[3])*b[4]+(a[3]*a[5]+cste*a[1]*a[4])*b[3]+2*a[2]*a[5]*b[2]+2*a[1]*a[5]*b[1]+cste*a[3]*a[4]*b[0])/2};
    } // end of symmetric_product_aba
