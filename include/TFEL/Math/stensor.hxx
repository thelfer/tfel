...
namespace tfel {
  ...
  namespace math {
    ...
    struct TFEL_VISIBILITY_LOCAL stensor
       ...
       */
      template <EigenSolver = TFELEIGENSOLVER>
      TFEL_MATH_INLINE2 void computeEigenVectors(
          tvector<3u, T>&,
          tmatrix<3u, 3u, tfel::typetraits::base_type<T>>&,
          const bool = false) const;
      /*!
     ...
     * Piola-Kirchhoff stress \f[ \tenseur{S} =
     * J\,\tenseur{U}^{-1}\,.\,\tsigma\,.\,\tenseur{U}^{-1} \f] \param[in] s:
     * corotationnal cauchy stress \param[in] U: stretch tensor \return the
