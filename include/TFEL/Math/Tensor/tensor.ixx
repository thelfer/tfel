  ...
  namespace math {
    ...
    typename std::enable_if<
        tfel::typetraits::IsScalar<T2>::cond &&
            std::is_same<
                typename ResultType<typename TensorTraits<Child>::NumType,
                                    T2,
                                    OpMult>::type,
    ...
    // Write to Tab
    template <unsigned short N, typename T>
    void tensor<N, T>::write(
        typename tfel::typetraits::BaseType<T>::type* const t) const {
      typedef typename tfel::typetraits::BaseType<T>::type base;
      typedef tfel::fsalgo::copy<TensorDimeToSize<N>::value> Copy;
      TFEL_STATIC_ASSERT(
          (tfel::typetraits::IsSafelyReinterpretCastableTo<T, base>::cond));
    ...
    // Return Id
    template <unsigned short N, typename T>
    const tensor<N, T>& tensor<N, T>::Id(void) {
      typedef typename tfel::typetraits::BaseType<T>::type base;
    ...
    computeDeterminantDerivative(const TensorType& F) {
      tensor<TensorTraits<TensorType>::dime,
             typename ComputeUnaryResult<
                 typename TensorTraits<TensorType>::NumType, Power<2>>::Result>
          dJ;
    ...
    typename std::enable_if<
        ...
        tensor<1u, typename StensorTraits<T>::NumType>>::type
    unsyme(const T& s) {
    ...
    typename std::enable_if<
        ...
        tensor<2u, typename StensorTraits<T>::NumType>>::type
    unsyme(const T& s) {
    ...
    typename std::enable_if<
        ...
        tensor<3u, typename StensorTraits<T>::NumType>>::type
    unsyme(const T& s) {
      using value_type = typename StensorTraits<T>::NumType;
      constexpr const auto cste = Cste<value_type>::isqrt2;
