  ...
  namespace math {
    ...
    struct StensorSymmetricProductDerivative<1u, T> {
      ...
      exe(const StensorType& s) {
        ...
        return {2 * s[0], zero, zero, zero,    2 * s[1],
                zero,     zero, zero, 2 * s[2]};
      }
    ...
    struct StensorSymmetricProductDerivative<3u, T> {
      ...
      exe(const StensorType& s) {
        ...
        return {2 * s[0],    zero,         zero,         s[3],
                ...
                s[4],        s[5],         s[3],         s[3],
                zero,        s[1] + s[0],  s[5] * icste, s[4] * icste,
                s[4],        zero,         s[4],         s[5] * icste,
                s[2] + s[0], s[3] * icste, zero,         s[5],
                s[5],        s[4] * icste, s[3] * icste, s[2] + s[1]};
      }
