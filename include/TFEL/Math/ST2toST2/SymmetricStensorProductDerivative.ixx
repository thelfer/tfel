  ...
  namespace math{
      ...
      st2tost2<1u,decltype(a[0]*b[0])>>::type
    {
	  ...
	  zero,zero,2*a[2]*b[2]};
			...
			using real = tfel::typetraits::base_type<NumType>;
			constexpr const auto zero = real(0);
			return {a[3]*b[3]+2*a[0]*b[0],zero,zero,a[0]*b[3]+a[3]*b[1],
			    zero,a[3]*b[3]+2*a[1]*b[1],zero,a[1]*b[3]+a[3]*b[0],
			    zero,zero,2*a[2]*b[2],zero,
			    a[1]*b[3]+a[3]*b[0],a[0]*b[3]+a[3]*b[1],zero,a[3]*b[3]+a[1]*b[1]+a[0]*b[0]};
		      } // end of symmetric_product_aba
					  ...
					  st2tost2<3u,decltype(a[0]*b[0])>>::type
					{
					  ...
					  return {a[4]*b[4]+a[3]*b[3]+2*a[0]*b[0],zero,zero,
					      ...
					      (a[5]*b[5]+a[4]*b[4]+2*a[3]*b[3]+2*a[1]*b[1]+2*a[0]*b[0])/2,
					      ...
					      (a[5]*b[5]+2*a[4]*b[4]+a[3]*b[3]+2*a[2]*b[2]+2*a[0]*b[0])/2,
					      ...
					      (2*a[5]*b[5]+a[4]*b[4]+a[3]*b[3]+2*a[2]*b[2]+2*a[1]*b[1])/2};
					} // end of symmetric_product_aba
