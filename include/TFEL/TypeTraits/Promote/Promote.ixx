...
namespace tfel{
  ...
  namespace typetraits{
    ...
    namespace internals{
       ...
       * \brief  An helper class which return the greatest type, ie the type which has the maximal positive value.
       * \param  A, first  type
       * \param  B, second type
	...
	typedef typename std::conditional<((std::numeric_limits<A>::max_exponent10 < std::numeric_limits<B>::max_exponent10 ) || 
       ...
       * They can be the same type on some plateform, so the algorithm used in PromoteNumericsGreater_ can't tell which is the greatest one.
       * \see PromoteNumericsGreater_
       */
      template<>
	 ...
	 * The result
	 */
	typedef typename tfel::typetraits::internals::PromoteNumericsGreater_<A,B>::type type;
      };
    ...
    struct Promote
    {
       ...
       * The result
       */
      typedef typename tfel::typetraits::internals::ChooseType<A,B,std::numeric_limits<A>::is_signed,std::numeric_limits<B>::is_signed>::type type;
    };
