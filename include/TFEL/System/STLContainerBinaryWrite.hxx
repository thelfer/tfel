...
#ifndef LIB_TFEL_SYSTEM_STLCONTAINERBINARYWRITE_H_
#define LIB_TFEL_SYSTEM_STLCONTAINERBINARYWRITE_H_ 
...
namespace tfel
{
  ...
  namespace system
  {
    ...
    struct BinaryWriter<std::map<Key,Data,Compare,Alloc> >
    {
       ...
       * \param[in] v: value
       */
      static void exe(const int f,const std::map<Key,Data,Compare,Alloc>& v)
      {
	binary_write(f,v.size());
	for(const auto& e : v){
	  binary_write(f,e);
...
#endif /* LIB_TFEL_SYSTEM_STLCONTAINERBINARYWRITE_H_ */
