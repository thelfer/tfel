...
namespace tfel {
  ...
  namespace system {
    ...
    TFEL_INLINE2 basic_rstream<Child, Traits>::operator int(void) {
      return static_cast<Child* const>(this)->getFileDescriptor();
    }  // end of basic_rstream<Child,Traits>::operator int

    template <typename Child, typename Traits>
    TFEL_INLINE2 basic_rstream<Child, Traits>::operator int(void) const {
    ...
    template <typename Child, typename Traits>
    const std::string basic_rstream<Child, Traits>::getLine(void) {
      using namespace std;
      string res;
      char c;
      this->operator>>(c);
