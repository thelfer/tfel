...
#ifndef LIB_TFEL_TRANSFORM_HXX_
#define LIB_TFEL_TRANSFORM_HXX_
...
namespace tfel {
  ...
  namespace fsalgo {
    ...
    struct transform {
       ...
       * each iterator i in the range [first, first+N), and assigns the result
       * of that operation to *o, where o is the corresponding output iterator.
       ...
       * second input range and where o is the corresponding output iterator.
       * That is, for each n such that 0 <= i < N, it performs the assignment
       * *(result + i) = op(*(first1 + i), *(first2 + i). The return value is
       * result + N.
       *
       * \param  InputIterator1  iterator to the first  element to be
       * transformed. \param  InputIterator2  iterator to the second element to
       * be transformed. \param  OutputIterator iterator to which result is
       * placed. \param  UnaryFunction  function used to perform transformation
       *
       * \return OutputIterator iterator after the last element transformed.
       *
       * \pre
       ...
       * - InputIterator1's and InputIterator2's value types must be
       * convertible, respectively, to BinaryFunction's first and second
       * argument types.
       * - UnaryFunction's result type must be convertible to a type in
       * OutputIterator's set of value types.
       *
