...
#ifndef _LIB_TFEL_EQUAL_HXX_
#define _LIB_TFEL_EQUAL_HXX_ 
...
namespace tfel{
  ...
  namespace fsalgo{
    ...
    /*! 
     * \class equal
     * \brief Equal returns true if the two ranges [first1, first1+N) and [first2, first2 + N) are identical when compared element-by-element, and otherwise returns false
     * \param N number of element to be compared.  
     *
    ...
    struct equal
    {
      ...
      bool exe(InputIterator p, InputIterator2 q)
      {
	return (*p == *q)&&(equal<N-1>::exe(++p,++q));
       ...
       * \return true if all elements are one-by-one equal given the predicate used.
       *
       * \pre
       * -  InputIterator1 is a model of Input Iterator.
       * -  InputIterator2 is a model of Input Iterator.
      ...
      static TFEL_FSALGORITHM_INLINE
      bool exe(InputIterator p, InputIterator2 q, BinaryPredicate binary_pred)
      {
	return (binary_pred(*p,*q))&&(equal<N-1>::exe(++p,++q,binary_pred));
      }
    ...
    struct equal<0u>
    {
      ...
      template<typename InputIterator,typename InputIterator2>
      static TFEL_FSALGORITHM_INLINE
      bool exe(InputIterator, InputIterator2)
      {
	return true;
      }
      ...
      template<typename InputIterator,typename InputIterator2, 
	       typename BinaryPredicate>
      static TFEL_FSALGORITHM_INLINE
      bool exe(InputIterator, InputIterator2, BinaryPredicate)
      {
	return true;
      }
    };
