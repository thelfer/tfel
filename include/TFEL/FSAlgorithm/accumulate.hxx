...
#ifndef _LIB_TFEL_ACCUMULATE_HXX_
#define _LIB_TFEL_ACCUMULATE_HXX_ 
...
namespace tfel{
  ...
  namespace fsalgo{
    ...
    /*!
     * \class accumulate
     * \brief accumulate computes the sum (or some other binary operation) of N elements of a container.
     * \param N number of element to be summed.  
     * 
	...
	return accumulate<N-1>::exe(++p,result);
      ...
      static TFEL_FSALGORITHM_INLINE
      T exe(InputIterator p, T init, BinaryFunction binary_op)
      {
	...
	return init;
       ...
       */
      template <class InputIterator, class T, class BinaryFunction>
      static TFEL_FSALGORITHM_INLINE T exe(InputIterator, T init, BinaryFunction)
      {
	return init;
      }
    };
...
#endif /* _LIB_TFEL_ACCUMULATE_HXX */
