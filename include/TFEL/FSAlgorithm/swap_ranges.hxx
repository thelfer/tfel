...
#ifndef _LIB_TFEL_SWAP_RANGES_HXX_
#define _LIB_TFEL_SWAP_RANGES_HXX_ 
...
namespace tfel{
  ...
  namespace fsalgo{
    ...
    /*!
     * \class swap_ranges
     * \brief swap_ranges swaps each of the elements in the range [first1, first1+N) with the corresponding element in the range [first2, first2 + N). That is, for each integer n such that 0 <= i < N, it swaps *(first1 + i) and *(first2 + i). The return value is first2 + N.
     * \param N size of the ranges to be swapped.
     *
    ...
    struct swap_ranges
    {
      ...
      ForwardIterator2 exe(ForwardIterator1 p, ForwardIterator2 q)
      {
	std::swap(*q,*p);
    ...
    struct swap_ranges<0u>
    {
      ...
      template<typename ForwardIterator1,typename ForwardIterator2>
      static TFEL_FSALGORITHM_INLINE
      ForwardIterator2 exe(ForwardIterator1, ForwardIterator2 q)
      {
	return q;
      }
    };
...
#endif /* _LIB_TFEL_SWAP_RANGES_HXX */
