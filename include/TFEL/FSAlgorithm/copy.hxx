...
namespace tfel{
  ...
  namespace fsalgo{
    ...
    {
      /*!
       * copy the element pointed to by p to the element pointed to by q and the call copy<N-1>::exe. Since calls are inlined, this recursion has no cost at runtime.
       *
      ...
      static TFEL_FSALGORITHM_INLINE OutputIterator exe(InputIterator p, OutputIterator q)
      {
	*q = *p;
    ...
    struct copy<0u>
    {
       ...
       */
      template<typename InputIterator,typename OutputIterator>
      static TFEL_FSALGORITHM_INLINE OutputIterator exe(InputIterator, OutputIterator q)
      {
	return q;
      }
    };
