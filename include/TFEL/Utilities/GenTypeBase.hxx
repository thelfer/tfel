...
namespace tfel {
  ...
  namespace utilities {
    ...
    struct GenTypeBase
      ...
      TFEL_INLINE requires<T1, T1 &> emplace(Args &&...args) {
      ...
      TFEL_INLINE requires<T1, void> set(T1 &&src) {
       ...
       */
      template <typename T1>
      TFEL_INLINE requires<T1, GenTypeBase &> operator=(T1 &&src) {
       ...
       */
      template <typename T1>
      TFEL_INLINE requires<T1, const T1 &> get() const {
       ...
       */
      template <typename T1>
      TFEL_INLINE requires<T1, T1 &> get() {
        // a silly trick to avoir a gcc warning
        union {
          storage_t *c;
          T1 *ptr;
     ...
     protected:
       ...
       */
      template <typename T1>
      TFEL_INLINE requires<T1, void> set_uninitialised() {
        this->clear();
