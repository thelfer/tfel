...
#ifndef _LIB_TFEL_TRANSFORM_HXX_
#define _LIB_TFEL_TRANSFORM_HXX_ 
...
namespace tfel{
  ...
  namespace fsalgo{
    ...
    struct transform
    {
      ...
      /*!
       * The first version of transform::exe performs the operation op(*i) for each iterator i in the range [first, first+N), and assigns the result of that operation to *o, where o is the corresponding output iterator. The return value is result + N.
       *
      ...
      OutputIterator exe(InputIterator p, OutputIterator q, UnaryFunction op)
      {
	*q = op(*p);
	return transform<N-1>::exe(++p,++q,op);
      ...
      static TFEL_FSALGORITHM_INLINE
      OutputIterator exe(InputIterator1 p, InputIterator2 q, OutputIterator r, BinaryFunction op)
      {
	*r = op(*p,*q);
    ...
    struct transform<0u>
    {
      ...
      template<typename InputIterator,typename OutputIterator, 
	       typename UnaryFunction>
      static TFEL_FSALGORITHM_INLINE
      OutputIterator exe(InputIterator, OutputIterator q, UnaryFunction)
      {
	return q;
      }
      ...
      template<typename InputIterator,typename InputIterator2,
	       typename OutputIterator, typename BinaryFunction>
      static TFEL_FSALGORITHM_INLINE 
      OutputIterator exe(InputIterator, InputIterator2, OutputIterator r, BinaryFunction)
      {
	return r;
      }
