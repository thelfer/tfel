...
#ifndef _LIB_TFEL_MIN_ELEMENT_HXX_
#define _LIB_TFEL_MIN_ELEMENT_HXX_ 
...
namespace tfel{
  ...
  namespace fsalgo{
    ...
    /*!
     * \class min_element 
     * \brief min_element finds the smallest element in the range [first, first+N)
    ...
    {      
      ...
      ForwardIterator exe_(ForwardIterator p, ForwardIterator q)
      {
	...
	return min_element<N-1>::exe_(++p,result);
      ...
      static TFEL_FSALGORITHM_INLINE
      ForwardIterator exe_(ForwardIterator p, ForwardIterator q,BinaryPredicate comp)
      {
	ForwardIterator result;
	if(comp(*p,*q)){
	  result = p;
    ...
    public:
            ...
            /*! 
       * It returns the first iterator i in [first, first+N) such that no other iterator in [first, last) points to a value greater than *i. The return value is first+N if and only if [first, last) is an empty range.
       * 
       * \param ForwardIterator, iterator to the elements.
       * 
       * \return ForwardIterator to the minimum element
      ...
      template<typename ForwardIterator>
      static TFEL_FSALGORITHM_INLINE 
      ForwardIterator exe(ForwardIterator p)
      {
	...
	return min_element<N-1>::exe_(++p,result);
            ...
            /*! 
       * This version version returns the first iterator i in [first, first+N) such that, for every iterator j in [first, first+N), comp(*i, *j) is false, where comp is a binary predicate given in argument
       * 
       * \param ForwardIterator, iterator to the elements.
       * \param BinaryPredicate, comparison function
       * 
    ...
    /*!
     * \brief partial specialisation used to end recursion when using the min_element algorithm.
     *
     * \author Helfer Thomas
     * \date   30 Jun 2006      
     */
	...
	return p;
      ...
      template<typename ForwardIterator,typename BinaryPredicate>
      static TFEL_FSALGORITHM_INLINE
      ForwardIterator exe(ForwardIterator p,BinaryPredicate)
      {
	return p;
      }
...
#endif /* _LIB_TFEL_MIN_ELEMENT_HXX */
