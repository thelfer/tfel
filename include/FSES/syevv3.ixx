...
namespace fses {
  ...
  int syevv3(MatrixType& Q,VectorType& w,MatrixType2& A)
  {
    ...
    real n0tmp, n1tmp;  // "Templates" for the calculation of n0/n1 - saves a few FLOPS
    real thresh;        // Small number used as threshold for floating point comparisons
...
#ifndef FSES_EVALS_ONLY
    wmax = std::abs(at(w,0));
    if ((t=std::abs(at(w,1))) > wmax)
      wmax = t;
    if ((t=std::abs(at(w,2))) > wmax)
      wmax = t;
	...
	for (j=0; j < 3; j++) {
	  if (std::fpclassify(at(Q,j,0)) != FP_ZERO){
	    ...
	    norm          = one / std::sqrt(square(at(Q,j,0)) + square(at(Q,(j+1)%3,0)));
