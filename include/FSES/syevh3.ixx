...
namespace fses{
  ...
  int syevh3(MatrixType& Q,VectorType& w,const MatrixType2& A)
  {
    ...
    at(Q,2,0) = (at(A,0,0) - at(w,0)) * (at(A,1,1) - at(w,0)) - at(Q,2,1);
    ...
    // not cause problems: If at(w,0) = at(w,1), then A - at(w,0) * I has rank 1,
    // i.e. all columns of A - at(w,0) * I are linearly dependent.
    if (norm <= error) {
      return syevq3(Q, w,A);
    ...
    at(Q,2,1)  = (at(A,0,0) - at(w,1)) * (at(A,1,1) - at(w,1)) - at(Q,2,1);
