...
namespace fses{
	       ...
	       const real d[3], const real z[3], int i0, int i1, int i2)
  {
    ...
    if (z[0] > 0) {
      ...
      a[1] = d[i1];
      a[2] = d[i2];
      a[3] = std::abs(d[0] + three*z[0]) + std::abs(d[1] + three*z[1]) + std::abs(d[2] + three*z[2]);
    } else {    
      a[0] = -std::abs(d[0] + three*z[0]) - std::abs(d[1] + three*z[1]) - std::abs(d[2] + three*z[2]);
      a[1] = d[i0];
      a[2] = d[i1];
      a[3] = d[i2];
    }
    ...
    p = square(alpha) - three*beta;    // Transformation that removes the x^2 term
	...
	if (std::abs(F) <= std::numeric_limits<real>::epsilon() * (eight * error + std::abs(x*dF)))
	  break;
