...
namespace numodis{
  ...
  struct TFELNUMODIS_VISIBILITY_EXPORT TripleNode
  {
    ...
    TripleNode(const FrankRead& lt0,
	       ...
	       const FrankRead& lt2,
	       const double alpha2)
      :_lt0(lt0),_alpha0(alpha0),
       _lt1(lt1),_alpha1(alpha1),
       _lt2(lt2),_alpha2(alpha2)
    {}
  
  private:

    double computeFx(double phi0,
		     double beta0,
		     double ph1,
		     double beta1) const;

    double computeEquilibriumBeta0(double phi,double tol=0.00001,int Nmax = 20) const;
  
  
    //! line tension properties of the first segment of the node
    const FrankRead& _lt0;

    //! angle between the junction direction and the Burgers vector of the first segment
    double _alpha0;

    //! line tension properties of the second segment of the node
    const FrankRead& _lt1;

    //! angle between the junction direction and the Burgers vector of the second segment
    double _alpha1;

    //! line tension properties of the junction
    const FrankRead& _lt2;
  
    //! angle between the junction direction and the Burgers vector of the junction
    double _alpha2;
