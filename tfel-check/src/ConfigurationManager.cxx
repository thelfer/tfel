...
namespace tfel{
  ...
  namespace check{
    ...
    ConfigurationManager::extract(const std::string& n){
    ...
    ConfigurationManager::ConfigurationManager(const ConfigurationManager&) = default;
    ...
    ConfigurationManager::getConfigurationManager(const std::string& n)
    {
      if(n.empty()){
	return *this;
      }
	...
	return *this;
      ...
      throw_if(this->subordinates.find(r.first)==this->subordinates.end(),
	       "invalid location '"+n+"' "
	       "(no subordinates called '"+r.first+"')");
