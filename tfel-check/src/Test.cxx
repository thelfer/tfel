...
namespace tfel{
  ...
  namespace check{
    ...
    void Test::compare() {
      this->comparison->setParameters(this->c1, this->c2, this->prec,
				      this->precision2, this->ci,
				      this->interpolation->getType(),
				      this->interpolation->isConform(),
				      this->colIntegralInterpolated,
				      this->integralInterpolation);

      this->comparison->compare();
    }

    bool Test::hasSucceed() const {
      return this->comparison->hasSucceed();
    }
    ...
    bool Test::isAllowLessResults() const {
      return this->allowLessResults;
    }

    void Test::setAllowLessResults(const bool b) {
      this->allowLessResults = b;
    }

    void Test::setColA(const std::shared_ptr<Column>& c) {
      this->c1 = c;
    }

    void Test::setColB(const std::shared_ptr<Column>& c) {
      this->c2 = c;
    }
    ...
    const std::string& Test::getFileA() const {
      return this->f1;
    }

    void Test::setFileA(const std::string& f) {
      this->f1 = f;
    }

    const std::string& Test::getFileB() const {
      return this->f2;
    }

    void Test::setFileB(const std::string& f) {
      this->f2 = f;
    }
    ...
    double Test::getPrec() const {
      return this->prec;
    }

    void Test::setPrec(const double v) {
      this->prec = v;
    }

    double Test::getPrecision2() const {
      return this->precision2;
    }

    void Test::setPrecision2(const double v) {
      this->precision2 = v;
    }
    ...
    const std::shared_ptr<Interpolation>& Test::getIntegralInterpolation() const {
      return this->integralInterpolation;
    }

    void Test::setIntegralInterpolation(const std::shared_ptr<Interpolation>& i) {
      this->integralInterpolation = i;
    }

    const std::shared_ptr<Column>& Test::getColA() const {
      return this->c1;
    }

    const std::shared_ptr<Column>& Test::getColB() const {
      return this->c2;
    }
    ...
    void Test::applyInterpolation() {
      ...
      if (!(this->interpolation->isConform()) && this->comparison->getName() != "area") {
	// tests if we can interpolate or not, based on columns' size and type of interpolation
	...
	if (this->c1->getValues().size() <= this->c2->getValues().size()) {
	  ...
	  //      if (beforeInterFile)		// stores the before interpolation values
	  ...
	  this->interpolation->interpolate(cXBTmp, cbTmp);  // interpolation of col B
	  this->c2->clearValues();
	  this->c2->resizeValues(this->c1->getValues().size());

	  for(size_type i = 0; i < ci1->getValues().size(); ++i){
	    this->c2->setValue(i, this->interpolation->getValue(ci1->getValues().at(i)));
	  }
	  //      if (afterInterFile)		// stores the after interpolation values
	  //        for(size_type i = 0; i < ci1->getValues().size(); ++i)
	...
	} else if (this->c2->getValues() < this->c1->getValues() && this->allowLessResults) {
	  ...
	  this->interpolation->interpolate(cXATmp, caTmp);  // interpolation of col A

	  this->c1->clearValues();
	  this->c1->resizeValues(this->c2->getValues().size());

	  for(size_type i = 0; i < ci2->getValues().size(); ++i){
	    this->c1->setValue(i,this->interpolation->getValue(ci2->getValues().at(i)));
	  }
