...
namespace tfel_check {
  ...
  AbsoluteComparison::AbsoluteComparison() {
    this->name = "absolute";
  }
  ...
  void AbsoluteComparison::compare() {
    ...
    for (vector<double>::size_type idx = 0; idx != this->c1->getValues().size();
	 ++idx) {
      absoluteError = std::abs(this->c1->getValues()[idx] - this->c2->getValues()[idx]);
    ...
    if (!s) {
      ...
      this->msgLog += " at line " + std::to_string(errorLineNumber)
