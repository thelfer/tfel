...
namespace tfel_check {
  ...
  void TestLauncher::registerCallBack(const std::string& name,
    ...
    if (!this->callBacks.insert({name, f}).second) {
      throw(
          std::runtime_error("TestLauncher::registerCallBack: "
                             "keyword '" +
    ...
    switch (clockevent) {
      case START:
        ...
        if (r == static_cast<clock_t>(-1))
          throw(
              runtime_error("Failed times system call in "
      ...
      default:
        throw(
            runtime_error("Wrong clockevent in "
                          "TestLauncher::ClockAction"));
    }
    return 0.;
  }
    ...
    for (const auto& c : this->commands) {
      ...
      try {
        ...
        manager.execute(c, "", this->testname + "-" + step + ".out",
                        this->environments);
        this->ClockAction(STOP);
        this->log.addTestResult(this->testname, step, c, this->ClockAction(GET),
                                true);
      } catch (std::exception& e) {
        this->log.addTestResult(this->testname, step, c, 0.0, false, e.what());
