...
namespace tfel_check {
      ...
      integralInterpolation(new NoInterpolation())
  {
    ...
    this->log.addDriver(std::make_shared<PCJUnitDriver>("TEST-" + this->testname + ".xml"));
    ...
    this->registerCallBack("@Precision",
			   &TestLauncher::treatPrecision);
  ...
  void TestLauncher::treatEnvironment() {
	...
	this->checkNotEndOfFile("TestLauncher::treatEnvironment");
	if(this->current->value=="}"){
	  this->throwRuntimeError("TestLauncher::treatEnvironment",
  ...
  void TestLauncher::setInterpolation(const std::string& m) {
    ...
    if (this->current->value == "Spline") {
      ...
      this->throwRuntimeError(m,"unknown interpolation type "
			      "(read '"+this->current->value+"')");
    }
    ++(this->current);
  }
  ...
  void TestLauncher::treatInterpolation() {
	...
	this->throwRuntimeError("TestLauncher::treatInterpolation",
				"missing argument after '"
				+std::prev(this->current)->value+"'\n"
  ...
  void TestLauncher::analyseInputFile() {
	...
	this->throwRuntimeError("TestLauncher::analyseInputFile",
  ...
  bool TestLauncher::execute() {
	...
	this->log.addTestResult(this->testname, step,
				c,0.0,false,e.what());
	...
	this->log.addTestResult(this->testname,step,"Comparing '"+
			  c.getFileA() + "' and '" + c.getFileB()+ "'",
			  0.0,false,e.what());
	gsuccess = false;
	success = false;
	i++;
      }
      this->glog.addSimpleTestResult("** " + step + " '" +
			       c.getFileA()+"' and '"+c.getFileB()
			       + "', column '" +
			       c.getColA()->getName() + "' ",
			       success);
