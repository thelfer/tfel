...
to generate a package for your personal use.
...
`MFront` will try to generate the most effective code for your
...
binaries with the same compiler version as the one used to produce
those binaries.
...
On `Debian wheezy`, the following generators are available:
...
generator used, various packages can be built:
...
The `LIB_SUFFIX` is used to specify that the libraries must be placed
