...
~~~~{#lst:mfront:material_properties:UO2YoungModulus-v0 .cpp caption="First implementation of the Young' modulus of uranium dioxide" }
@DSL MaterialLaw;
@Law YoungModulus;
@Input T, f;
@Function {
...
## Choice of a more explicit name for the material property

`YoungModulus` is a poor name for the material law. A good practice is
...
>
> ~~~~{.bash}
> $ mfront --@Library=UraniumDioxide --obuild --interface=python UO2_YoungModulus_Martin1989.mfront
> Treating target : all
> The following libraries have been built :
...
> - default_out_of_bounds_policy  : string specifying the default out of bounds policy. Allowed values are `None`, `Warning`, `Strict`.
> - out_of_bounds_policy_runtime_modification: boolean stating if the runtime modification of the out of bounds policy is allowed.
> - parameters_as_static_variables: boolean stating if parameters shall be treated as static variables.
> - parameters_initialization_from_file: boolean stating if the values of parameters can be changed from an external file.
> - build_identifier              : string specifying a build identifier. This option shall only be specified on the command line.
> ~~~~

> **Specifying a DSL option on the command line**
>
...
variable which has a default value that can be changed a runtime. In
practice, parameters can be seen as global variables. How to change the
...
### Modification of parameters using external txt files

Parameters can be generaly be modified using an external txt file,
although it may depend on the interface considered.
...
The file may contain commented line starting with the `#` character.

This file is read, if present in the current working directory, only at
the very first evaluation of the material property.
...
### Modification of parameters using the `ExternalLibraryManager`
...
Using parameters may have runtime performances since a global variable
is used to store the values of the parameters
 
#### Disabling the runtime modification of parameters

Since Version 4.1, yhe boolean DSL option
...
point values associated with units. Quantities are implemented by the
`TFEL/Math` library and supported in `MFront` since Version 4.0.

Quantities allow the compiler to perform dimensional analysis at
compile-time. 
...
> ~~~~
>
> The `gcc` compiler generates the following error:
> 
> ~~~~{.bash}
> TFEL/Math/Quantity/qtOperations.hxx:302:19: error: static assertion failed: invalid operation
>     static_assert(std::is_same_v<UnitType, UnitType2>, "invalid operation");
> ~~~~

~~~~{#lst:mfront:material_properties:UO2YoungModulus-assembly .nasm caption="Assembly of the code generated by the `c` interface without the use of quantities"}
UO2_YoungModulus_Martin1989:
.LFB4378:
	.cfi_startproc
	vmulsd	.LC0(%rip), %xmm0, %xmm2
	vmovsd	.LC1(%rip), %xmm3
	vdivsd	.LC3(%rip), %xmm1, %xmm1
	vfmadd213sd	.LC2(%rip), %xmm0, %xmm3
...
~~~~{#lst:mfront:material_properties:UO2YoungModulus-assembly-qt .nasm caption="Assembly of the code generated by the `c` interface with the use of quantities"}
UO2_YoungModulus_Martin1989:
.LFB4714:
	.cfi_startproc
	vmulsd	.LC0(%rip), %xmm0, %xmm2
...
Quantities have been implemented to have a negligible
runtime overhead, if any (see comment below).

Listings @lst:mfront:material_properties:UO2YoungModulus-assembly and
lst:mfront:material_properties:UO2YoungModulus-assembly-qt show the
assembly resulting from the compilation of the code generated by the `c`
interface with and without the usage of quantities.
