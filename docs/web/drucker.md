...
respect to the stress \(\tsigma\) is readily obtained by chain
rule:
\[
...
\(\sigmaeq^{D}\) with respect to \(J_{2}\) and \(J_{3}\) are given by the
following formulae:
...
> more cumbersome and one has to take care of:
>
...
>   (\(1D\), \(2D\) or \(3D\)).
>
> This is the approach used most of the time by the `TFEL` developers
...
> a concrete example. Those functions are declared in the
...
coefficients (each) which describe the material orthotropy. They are
defined through an auxiliary linear transformation \(\tenseurq{C}\) as
follows:
\[
