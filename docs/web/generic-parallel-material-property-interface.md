...
where the configuration file `config-cuda.json` provide the required
information to call a `CUDA` compiler (`nvcc` or `clang++`) with the
appropriate flags. This file may also contain the options associated
with the `CUDA` backend, see below.
...
- the user is responsible for consistently passing pointers to memory
  ...
  uniform values. Managed memory can also be used.
