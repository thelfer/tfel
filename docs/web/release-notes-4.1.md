...
## The `CamClayElasticity` brick and the `CamClay` stress potential {#sec:tfel:4.1:mfront:cam_clay_stress_potential}

The `CamClay` stress potential is described in details on [this
page](CamClayStressPotential.html).

The Cam-Clay non linear elastic behaviour is the basis of various
behaviours on geomechanics. It is basically based on the following
relationship between the rate of the hydrostatic pressure \(\dot{p}\)
and the rate of the trace of the elastic strain
\(\trace{\tdepsilonel}\):
\[
\dot{p} = -K\,\trace{\tdepsilonel}
\]
where the bulk modulus \(K\) is assumed dependant of the hydrostatic pressure as
follows:
\[
K\paren{p} = \Frac{1+e_{0}}{\kappa}\,p
\]{#eq:tfel:4.1:mfront:camclay:bulk_modulus}

In Equation @eq:tfel:4.1:mfront:camclay:bulk_modulus, the hydrostatic
pressure \(p=-\Frac{1}{3}\,\trace{\tsigma}\) is assumed strictly
positive and \(e_{0}\) and \(\kappa\) denotes repectively the initial
void ratio and the unloading/reloading slope of the material.
...
## Issue #114: [mfront] Implement the `CamClay` stress potential

The `CamClay` stress potential is described in Section
@sec:tfel:4.1:mfront:cam_clay_stress_potential.

For more details, see : <https://github.com/thelfer/tfel/issues/114>.
