...
function takes two template parameters:
...
> **Note** In his paper, Barlat and coworkers use the following convention for
...
The `TFEL/Material` library also provides an overload of the
`makeBarlatLinearTransformation` whose template parameters are the
...
![Comparison of various approximations of the inverse Langevin
...
are indistinguishable. See Jedynak for a quantitative discussion of the
error generated by those approximations [@jedynak_approximation_2015].
It is worth noting that all those approximations mostly differ near the
pole of the inverse Langevin function \(\mathcal{L}^{-1}\paren{x}\).
...
can't be declared `constexpr` because the tangent function is not
...
It can be converted to the other structures describing isotropic moduli as follows:
...
the isotropic basis, and constructs the isotropized version of \(\tenseurq{C}\).
...
submitted to an external uniform strain field at infinity \(\tenseur E\),
...
For the other schemes, the isotropic character of the matrix
when computing the strain localisators will depend
on what `micro_1.isIsotropicMatrix()` returns. Hence, it is important
to initialize the matrix or the microstructure with the appropriate
elastic moduli. If isotropic, it will work in all cases, whereas
