...
Note that providing interfaces is optional. Not specifying an interface
...
- `date`: this field gives the date at which the `MFront`
  file has been written if one has been given using the `@Date` keyword.
- `description`: this field returns the description of the file, if one
...
useful if at least one interface was defined.

The targets description can be printed. The result is a JSON-like
