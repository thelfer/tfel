...
\subsection{Autres interfaces} D'autres interfaces peuvent être ajoutées
...
démarrage\footnote{Ce mécanisme est utilisé par exemple par l'architecture
  \pleiades{} pour générer l'interface {\tt pleiades} aux propriétés
  matériau, interface qui évolue avec les versions de l'architecture.}.
...
mot clé {\tt @Material} ou le mot clé {\tt @Library}.
...
\clearpage
\newpage
\section{Analyseurs dédiés aux lois de comportements}
...
Nous distinguerons trois types de variables~:
\begin{itemize}
\item les coefficients qui peuvent être ou des coefficients de la loi
  de comportement (pente d'adoucissement) ou des variables d'états
  n'évoluant pas.
\item les variables internes, dont l'évolution est du ressort de la
  loi de comportement~;
\item les variables externes, dont l'évolution est donnée par ailleurs.
  Ces variables peuvent ou être des variables d'état du matériau ou
  des paramètres externes (flux de neutrons).
  ...
  et dont la valeur est donnée par la résolution de l'équilibre global
  du matériau). Ces deux variables sont traitées de manière particulière
  par \mfront{} et déclarées automatiquement~;
\end{itemize}
Ces variables peuvent être ou des {\em scalaires} ou des {\em tenseurs}.

Une loi de comportement consiste en la donnée~:
\begin{itemize}
\item d'un jeu de coefficients, variables internes et externes~;
\item d'un système d'équations différentielles donnant l'évolution
  des variables internes~;
\item d'une méthode d'intégration de ce système d'équations différentielles.
\end{itemize}
...
\begin{itemize}
  ...
  les lois de comportements viscoplastiques isotropes de la forme~:
  \[
  ...
  exclusivement les lois de comportements viscoplastiques isotropes de
  la forme~:
  \[
  ...
  exclusivement les lois de comportements plastiques isotropes de la
  forme~:
  \[
...
{\tt Init\-Local\-Vars}. Ce tenseur est calculé par la classe {\tt
...
niveau du fichier \mfront{} en utilisant la directive {\tt @Includes}.
...
\subsection{Interface disponible}
...
\begin{itemize}
  ...
  \texttt{Isotropic\-Plastic\-Mises\-Flow} où il vaut \(1\). En effet,
  dans le cas d'écoulement viscoplastique, une valeur de
  \(\pfrac{1}{2}\) offre une meilleure convergence. Dans le cas d'un
  écoulement plastique, une valeur de \(1\) garantie que le matériau se
  trouve dans le domaine d'élasticité en fin de pas de temps. La valeur
  de \(\theta\) peut être modifiée par le mot clé \texttt{@Theta}.
  Notons enfin que l'analyseur
  \texttt{Multiple\-Isotropic\-Mises\-Flows} permet de définir une
  valeur de \(\theta\) par écoulement, permettant une formulation mixte
  en cas de couplage entre plasticité et viscoplasticité.
\end{itemize}

\subsection{Algorithme de résolution}

Connaissant l'état du matériau à l'instant \(t\), nous voulons connaître
son comportement à l'instant \(t+\Delta t\). Pour cela, nous utilisons un
algorithme semi-implicite de type \(\theta\)-méthode où \(\theta\) est un
nombre compris entre \(0\) et \(1\) indiquant que les incréments des
variables internes sont calculés en \(t+\theta\Delta t\).

\paragraph{Expression de la contrainte équivalente et de la direction
  d'écoulement} Les contraintes sont une fonction explicite des
déformations élastiques et nous supposerons le comportement du matériau
isotrope de sorte que son tenseur d'élasticité s'écrit~:
\[
\tenseurq{D}=\lambda\tenseur{I}\otimes\tenseur{I}+2\mu\tenseur{I}
\]
où
\(\lambda\) et \(\mu\) sont les c{\oe}fficients de \nom{Lamé} du matériau.
...
\paragraph{Le mot clé \texttt{@Behaviour}} La second ligne, commençant par
le mot clé \texttt{@Behaviour}, donne le nom de la loi
de comportement.
...
\paragraph{Le mot clé \texttt{@InitLocalVariables}} Le mot clé
\texttt{@InitLocalVars} permet d'écrire du code appelé avant tout
...
\paragraph{Le mot clé \texttt{@FlowRule}} Le mot clé
\texttt{@FlowRule} permet de renseigner la fonction d'écoulement
...
\subsection{Algorithme disponibles}
...
\paragraph{Méthode de \nom{Runge-Kutta} d'ordre $\pfrac{5}{4}$}
...
fonction est la méthode itérative de \nom{Newton-Raphson} qui s'écrit
ainsi~:
\begin{equation}
  \label{eq:NR}
  \Delta\, Y^{n+1}=\Delta\, Y^{n}-\left.\derivtot{F}{\Delta\,Y}\right|_{\Delta Y=\Delta\,Y^{n+1}}
\end{equation}
où \(\derivtot{F}{\Delta\,Y}\) est la matrice jacobienne de la
fonction \(F\). 
...
\begin{equation}
  ...
  \derivtot{F}{\Delta\,Y} = I-\theta\Delta\,t\,\derivtot{G}{Y}\paren{\debutpas{Y}+\theta\Delta{Y},t+\theta\,\Delta t}
\end{equation}
où \(I\) est la matrice identité. 

Le calcul de la matrice jacobienne est généralement le point le plus
difficile de la méthode implicite. Nous verrons dans ce qui suit que
l'essentiel des développements mathématiques y est consacré. 
...
implicite se fait dans la partie {\tt @Integrator}. Les variables {\tt
...
\paragraph{Le mot clé \texttt{@Model}} La second ligne, commençant par
le mot clé \texttt{@Model}, donne le nom du modèle.
...
\paragraph{Le mot clé \texttt{@LocalParameter}} Le mot clé
\texttt{@LocalParameter} permet de déclarer une variable
locale, propre au modèle.

\paragraph{Le mot clé \texttt{@GlobalParameter}}
De la même manière, le mot clé \texttt{@GlobalParameter}
...
\paragraph{Le mot clé \texttt{@Domain}} 
Le mot clé \texttt{@Domain} permet de préciser le domaine
d'application du modèle. Sa valeur peut être une expression
rationnelle. Le mot clé \texttt{@Domains} permet quand à lui de
...
\paragraph{Le mot clé \texttt{@Function}} Le mot clé
\texttt{@Function} permet d'introduire l'implantation du modèle. Un
