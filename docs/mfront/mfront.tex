...
l'inclusion des règle de dépendances dans le fichier {\tt
  Makefile.mfront} généré. Cette option a été introduite pour éviter
certains problèmes sous \windows{}.

%--silent-build                   : active or desactivate silent build
%--nomelt                         : don't melt librairies sources
%--otarget                        : generate MakeFile with optimized compilations flags and build the specified target
%--target, -t                     : generate MakeFile and build the specified target
%--win32                          : specify that the target system is win32
...
code définis par l'utilisateur. Les paramètres de ces fonctions sont
...
En pas de générer des fichiers source, \mfront{} peut gérer leur
compilation en librairie.

\subsubsection{Génération de fichiers Makefile}

\subsubsection{Lancement du processus de compilation}
...
Nous verrons au paragraphe~\ref{sec:mfront:properties:bounds} comment
affecter des bornes aux variables d'entrée.

\subsubsection{Variables statiques}

La directive \mkey{StaticVariable}

\subsubsection{Paramètres}

La directive \mkey{Parameter}

\subsubsection{Constantes}

La directive \mkey{Constant}

\subsection{Gestion des bornes}
\label{sec:mfront:properties:bounds}
...
\paragraph{Le mot clé \mkey2{PhysicalBounds}{Physical\-Bounds}} Le mot
clé \mkey2{PhysicalBounds}{Physical\-Bounds} définit les bornes de
validité des différents variables d'entrée. Le dépassement d'une de ces
bornes doit arrêtés le calcul.

\paragraph{Le mot clé \mkey{Bounds}} Le mot clé \mkey{Bounds} définit
les bornes de validité expérimentale de la propriété. Les cas de
dépassement de ces bornes peuvent être d'origine diverse, et peuvent
parfois être acceptable. La plupart des interface permettent de
paramétrer leur réaction en cas de dépassement des bornes de validité.
Trois politiques sont généralement retenues~:
\begin{itemize}
...
Par défaut, la politique \texttt{NONE} est utilisée. La manière de
déclarer ces politiques dépend de l'interface~:
\begin{itemize}
...
La directive \mkey{Namespaces}

La directive \mkey{UseTemplate}
...
\subsection{Exemple de propriétés dont seuls des mesures sont connues}
...
\subsubsection{Gestions des bornes}
...
\mkey2{AuxiliaryStateVariable}{Auxiliary\-State\-Variable}). Cette
variable peut être calculée, dans le bloc suivant la directive
\mkey2{UpdateAuxiliaryStateVariables}{Update\-Auxiliary\-State\-Variables}
ainsi~:
\begin{center}
  \texttt{evp += deto-deel;}
\end{center}
...
\begin{equation}
  \label{eq:RK:epsilon}
  \Frac{1}{\displaystyle\sum_{i=1}^{N}N_{i}}\displaystyle\sum_{i=1}^{N}\left|
  y_{i}^{(a)}-y_{i}^{(b)}\right|<\varepsilon
\end{equation}
où~:
\begin{minipage}[t]{0.9\linewidth}
  \begin{itemize}
    \item \(N\) est le nombre de variables internes~;
    \item \(N_{i}\) le nombre de composantes de la variable interne
    \(y_{i}\)~;
...
La directive \mkey{@ComputeStress} permet de calculer les contraintes.
...
Cette méthode est appelée après la méthode \mkey{@ComputeStress}.
...
\subsubsection{Critère d'arrêt} Les algorithmes présentés s'arrêtent
quand la différence entre deux estimations des incréments des
déformations viscoplastiques cumulées est inférieur à un certain critère
\(\varepsilon\)~:
\[
\Frac{1}{\displaystyle\sum_{i=1}^{N}N_{i}}\displaystyle\sum_{i=1}^{N}\left|\Delta_{n}\Delta\,
y_{i}\right|<\varepsilon
\]
où\(\Delta_{n}\Delta\,y_{i}\) désigne la différence entre l'estimation
de l'incrément de la i\ieme{} variable à l'étape \(n+1\) et de son
estimation à l'étape \(n\) et \(N_{i}\) est le nombre de composantes de
la variable interne \(y_{i}\).
