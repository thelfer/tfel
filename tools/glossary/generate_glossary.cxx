...
// g++ -std=c++11 generate_glossary.cxx  -o generate_glossary -I/home/th202608/codes/tfel/trunk/install/include -L/home/th202608/codes/tfel/trunk/install/lib -lTFELUtilities  -lTFELGlossary && ./generate_glossary && mv Glossary.hxx ../../include/TFEL/Glossary/Glossary.hxx && mv Glossary.cxx ../../src/Glossary/Glossary.cxx && mv PythonGlossary.cxx ../../bindings/python/tfel/Glossary.cxx && pandoc -f markdown+tex_math_single_backslash --toc  glossary-pandoc.txt -o /tmp/glossary.docx && mv glossary-pandoc.txt ../../docs/web/glossary.md
  ...
  void execute(const std::string& f)
  {
	...
	try{
	  if(k=="alias"){
	    ...
	    tfel::raise("GlossaryTokenizer::execute: "
			"unknown key '"+k+"'");
	  }
	  CxxTokenizer::readSpecifiedToken("GlossaryTokenizer::execute",";",p,pe);
	  CxxTokenizer::checkNotEndOfLine("GlossaryTokenizer::execute",p,pe);
...
void generateCxxOutput(const GlossaryTokenizer& tokenizer)
{
  ...
  tfel::raise_if(!header,"generate_glossary: "
		 "can't open file 'Glossary.hxx'");
  tfel::raise_if(!src,"generate_glossary: "
  ...
  for(auto p=tokenizer.begin();p!=tokenizer.end();++p){
	...
	src << "Glossary::names+" << pos << ",Glossary::names+" << pos+names.size() << ",\n";
      }
    }
    pos+= names.size();
...
void generateBoostPythonBindings(const GlossaryTokenizer& tokenizer)
{
  ...
  tfel::raise_if(!psrc,"generate_glossary: "
...
void generateXMLOutput(const GlossaryTokenizer& tokenizer)
{
  ...
  tfel::raise_if(!xml,"generate_glossary: "
  ...
  for(auto p=tokenizer.begin();p!=tokenizer.end();++p){
	...
	<< "<type>" << p->getType() << "</type>\n"
	<< "<short_description>" << p->getShortDescription() << "</short_description>\n"
	<< "<description>\n"
	<< "<![CDATA[\n";
...
void generatePandocOutput(const GlossaryTokenizer& tokenizer)
{
  ...
  tfel::raise_if(!doc,"generatePandocOutput: "
