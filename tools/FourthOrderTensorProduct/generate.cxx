...
typedef std::pair<FourthOrderTensor,
		  FourthOrderTensor> FourthOrderTensorProduct;

static std::string
getDimeToSizeMetaFunction(const TensorType& t)
{
  if(t==T2){
...
static std::string
getRowDimeToSizeMetaFunction(const FourthOrderTensor& t)
{
  return getDimeToSizeMetaFunction(t.second);
}

static std::string
getColumnDimeToSizeMetaFunction(const FourthOrderTensor& t)
{
  return getDimeToSizeMetaFunction(t.first);
}

static unsigned short
getSize(const TensorType& t,
...
static unsigned short
getRowSize(const FourthOrderTensor& r,
	   const unsigned short d)
{
...
static unsigned short
getColumnSize(const FourthOrderTensor& r,
	      const unsigned short d)
{
...
toupper(const std::string& s){
  ...
  string r;
  r.resize(s.size());
  transform(s.begin(), s.end(), r.begin(),[&loc](const char c){return toupper(c,loc);});
  return r;
}

static std::string
getName(const TensorType& t)
{
  if(t==T2){
    return "T2";
  }
...
static std::string
getName(const FourthOrderTensor& t)
{
  return getName(t.first)+"to"+getName(t.second);
}

static std::string
getName(const FourthOrderTensorProduct& p)
{
  return getName(p.first)+getName(p.second);
}

static FourthOrderTensor
...
static bool
isProductValid(const FourthOrderTensor& t1,
...
void
writeFileHeader(std::ostream& out,
		const FourthOrderTensorProduct& p)
{
  ...
  out << "#include\"TFEL/Math/" << getName(p.first) << "/" << getName(p.first) << "Concept.hxx\"" << endl;
  if(p.second!=p.first){
    out << "#include\"TFEL/Math/" << getName(p.second) << "/" << getName(p.second) << "Concept.hxx\"" << endl;
  }
  if((r!=p.first)&&(r!=p.second)){
    out << "#include\"TFEL/Math/" << getName(r) << "/" << getName(r) << "Concept.hxx\"" << endl;
  }
  out << "#include\"TFEL/Math/" << getName(r) << "/" << getName(r) << "Expr.hxx\"" << endl;
  out << "" << endl;
  ...
  out << "  {" << endl;
  out << "" << endl;
  out << "    //! Empty structure used for partial specialisation of the" << endl;
  out << "    //! " << getName(r) << "Expr class" << endl;
  out << "    template<unsigned short N>" << endl;
  out << "    struct TFEL_VISIBILITY_LOCAL " << getName(p) << "ProductExpr" << endl;
  out << "    {}; // end of struct " << getName(p) << "ProductExpr" << endl;
  out << "" << endl;
}

void
writePartialSpecialisation(std::ostream& out,
			   const FourthOrderTensorProduct& p,
			   const unsigned short d)
{
  using namespace std;
  ...
  out << "struct TFEL_VISIBILITY_LOCAL " << getName(r) << "Expr<" << getName(r) << "ResultType," << getName(p) << "ProductExpr<" << d << "u> >" << endl;
  out << "  : public " << getName(r) << "Concept<" << getName(r) << "Expr<" << getName(r) << "ResultType," << getName(p) << "ProductExpr<" << d << "u> > >," << endl;
  out << "	public fsarray<" << getRowDimeToSizeMetaFunction(r) << "<" << getName(r) << "Traits<" << getName(r) << "ResultType>::dime>::value*" << endl;
  out << "		       " << getColumnDimeToSizeMetaFunction(r) << "<" << getName(r) << "Traits<" << getName(r) << "ResultType>::dime>::value," << endl;
  out << "		       typename " << getName(r) << "Traits<" << getName(r) << "ResultType>::NumType>" << endl;
  out << "{" << endl;
  ...
  out << "  }" << endl;
  out << "}; // end of struct " << getName(r) << "Expr<" << getName(r) << "ResultType," << getName(p) << "ProductExpr>" << endl;
  out << endl;
}

void
writeFileTrailer(std::ostream& out,
		 const FourthOrderTensorProduct& p)
{
  using namespace std;
