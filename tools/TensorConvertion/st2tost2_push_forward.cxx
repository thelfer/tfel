...
{
  static int exe(const unsigned short i,
...
protected:
    ...
    static int row_index(size_type i,
			 size_type j){
    ...
    static int row_index2(size_type i,
			  size_type j){
    if(index_base<N>::exe(i,j)!=-1){
      // i,j are valid for the space dimension considered
      if((i==j)&&(i<3)){
	  ...
	  for(size_type l=0;l!=3;++l){
		...
		for(size_type p=0;p!=3;++p){
		  ...
		  for(size_type q=0;q!=3;++q){
		    ...
		    if(rlq==-1){continue;}
		    // row_index2 takes into account minor symmetries of C_{mnpq}
		    const auto rmn = index<N>::row_index2(m,n);
		    const auto rpq = index<N>::row_index2(p,q);
		    if(nb!=0){std::cout << "+";};
		    if((nb%2==0)&&(nb!=0)){std::cout << "\n";};
		    std::cout << "F[" << rim << "]*F[" << rjn << "]*F[" << rkp << "]*F[" << rlq << "]*C(" << rmn << "," << rpq <<  ")";
			...
			std::cout << "*icste";
		      } else if(!(((rmn>2)&&(rpq<3))||((rpq>2)&&(rmn<3)))){
			std::cout << "*cste";
		      }
		    } else if((rij>2)&&(rkl>2)){
		      // on multiplie par 2, sauf si..
		      if((((rmn>2)&&(rpq<3))||((rpq>2)&&(rmn<3)))){
