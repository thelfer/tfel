...
struct MFM
  : public tfel::utilities::ArgumentParserBase<MFM>
{
...
private:
  ...
  void registerCommandLineCallBacks(){
    ...
    auto material_filter = [this](const EntryPoint& e,
				  const std::string& m){
      using tfel::system::ExternalLibraryManager;
      auto& elm = ExternalLibraryManager::getExternalLibraryManager();
    ...
    auto name_filter = [this](const EntryPoint& e,
			      const std::string& n){
    ...
    this->add_filter("--filter-by-interface",
		     ...
		     interface_filter);
    this->add_filter("--filter-by-material",
		     "filter entry points by material",
		     material_filter);
    this->add_filter("--filter-by-name",
		     "filter entry points by name",
		     name_filter);
    this->add_filter("--filter-by-type",
		     "filter entry points by name",
		     type_filter);
    this->registerCallBack("--verbose",
  ...
  void treatUnknownArgument() override
  {
	...
	this->epts.push_back(std::move(ep));
      ...
      if(vlevel>=VERBOSE_DEBUG){
	std::clog << "Error while treating library '"
		  << l << ": " << e.what() << "'\n";
      }
    }
