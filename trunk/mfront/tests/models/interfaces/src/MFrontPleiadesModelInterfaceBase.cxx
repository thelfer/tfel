...
namespace mfront{
  ...
  MFrontPleiadesModelInterfaceBase::getGenTypeMethod(const std::string& type) const
  {
    ...
    tfel::raise("MFrontPleiadesModelInterfaceBase::getGenTypeMethod : "
						 ...
						 const ModelDescription& md)
  {
	...
	if(p10->type=="short"){
	  ...
	  tfel::raise("MFrontPleiadesModelInterfaceBase::writeSrcFile: "
		      "type "+p10->type+" is not a supported type "
		      "for a static variable. "
		      "Supported types are `short`, `ushort`, `int`, "
		      "`uint`, `long`, `ulong`,"
		      "`float`, `double`, `real` and `ldouble`.\n"
		      "Error at line "+std::to_string(p10->lineNumber));
	}
      }
      this->srcFile << '\n';
    }
    ...
    // getNodesOfCELLZone
    this->srcFile << "const std::vector<int>&\n"
		  << md.className << "::getNodesOfCELLZone(Pleiades::PMetier::PMesh::PtrIMesh Mesh,\n"
  ...
  MFrontPleiadesModelInterfaceBase::writeAssignDefaultValue(const VariableDescription& v)
  {
    if(v.type=="string"){
      ...
      tfel::raise("MFrontPleiadesModelInterfaceBase::writeAssignDefaultValue: "
