...
namespace mtest{
			      ...
			      const std::string& type)
  {
    for(const auto& n : names){
      tfel::raise_if(m.find(n)==m.end(),"no '"+type+"' named "
		     "'"+n+"' declared");
    }
  }
			      ...
			      const std::string& type)
  {
	...
	tfel::raise_if(evm2.find(n)==evm2.end(),
		       "no "+type+" named "
		       "'"+n+"' declared");
      }
    }
					     ...
					     const bool check)
  {
    ...
    const bool is_mp = std::find(mpnames.begin(),mpnames.end(),n)==mpnames.end();
					   ...
					   const Parameters& d)
  {
    ...
    if(w=="LogarithmicStrain1D"){
      ...
      tfel::raise("SingleStructureScheme::setBehaviour: "
		  "unknown wrapper '"+w+"'");
    }
  }
								    ...
								    const real v)
  {
    ...
    tfel::raise_if(std::find(ivsnames.begin(),ivsnames.end(),n)==ivsnames.end(),
		   "SingleStructureScheme::setScalarInternalStateVariableInitialValue: "
    ...
    tfel::raise_if(type!=0,"SingleStructureScheme::setScalarInternalStateVariableInitialValue: "
								      ...
								      const std::vector<real>& v)
  {
    ...
    const auto type = this->b->getInternalStateVariableType(n);
    const auto pos  = this->b->getInternalStateVariablePosition(n);
    tfel::raise_if(type!=1,"SingleStructureScheme::"
				      ...
				      const real dt) const
  {
	...
	if(this->b->getSymmetryType()==0){
	  ...
	  tfel::raise("SingleStructureScheme::prepare: "
