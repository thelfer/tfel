...
#define COMPILEDTARGETDESCRIPTIONBASE_ADDPROPERTY_HELPERFUNCTIONS( X )   				  \
  ...
  CompiledTargetDescriptionBase_get##X(const mfront::CompiledTargetDescriptionBase& t)			  \
  {											  \
    return t. X;									  \
  }											  \
  static void										  \
  CompiledTargetDescriptionBase_set##X(mfront::CompiledTargetDescriptionBase& t,				  \
			    const std::vector<std::string>& h)				  \
  {											  \
    t. X = h;										  \
  }											  \
  static void										  \
  CompiledTargetDescriptionBase_addProperty##X(boost::python::class_<mfront::CompiledTargetDescriptionBase>& w) \
  {											  \
    w.add_property(#X,CompiledTargetDescriptionBase_get##X,CompiledTargetDescriptionBase_set##X);		  \
  }
...
void declareCompiledTargetDescriptionBase(){
  ...
  class_<CompiledTargetDescriptionBase> w ("CompiledTargetDescriptionBase",no_init);
