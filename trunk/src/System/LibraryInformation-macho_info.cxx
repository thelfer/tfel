...
namespace details {
  ...
  struct mach_header_template {
    ...
    std::uint32_t     flags[sizeof(AddressOffsetT) / sizeof(uint32_t)]; // Flags and reserved
  };
  ...
  private:
	...
	if (command.cmd != cmd_num) {
	  this->f_->seekg(pos + static_cast<std::ifstream::pos_type>(command.cmdsize));
	  continue;
	}
	...
	for (std::size_t j = 0; j < symbh.nsyms; ++j) {
	  f.f_->seekg(static_cast<std::streamoff>(symbh.symoff + j * sizeof(nlist_t)));
	  f.read_raw(symbol);
	  if (!symbol.n_strx) {
