 ...
 * 
 * g++ -fPIC -DPIC --shared -I ../include/ `tfel-config --includes --oflags --warning`
 * PleiadesModelInterface.cxx -o libPleiadesModelInterface.so
 * export
...
static std::string getGlossaryName(const std::string& v, const ModelDescription& mdata) {
...
void PleiadesModelInterface::declareReservedNames(std::set<std::string>& v) {
  v.insert({"declareMaterialProperty", "declarePrivateMaterialProperty", "getMaterialProperty",
            "declareLocalFields", "declareLocalFields", "initializeLocalFields",
            "getLocalFieldsLocalities", "getLocalField", "getOutputLocalField",
            "getMaterialPropertyField", "registerVar", "declareField", "declareFieldArray",
            "declareParameter", "getParameter", "declareField", "getOutputField", "getOutputFieldA",
            "setUniformValueA", "getOutputIterField", "getOutputIterFieldA", "getField",
            "getFieldA", "getIterField", "getIterFieldA", "getPreviousField", "getPreviousFieldA",
            "getPreviousIterField", "getPreviousIterFieldA"});
}  // end of PleiadesModelInterface::declareReservedNames

static void declareField(std::ostream& out, const ModelDescription& mdata,
                         const VariableDescriptionContainer& v, const std::string& type) {
  using namespace std;
  ...
  for (const auto& p : v) {
    // declareField
    out << "  declareField<double>(\"" << p.name << "\"," << getGlossaryName(p.name, mdata) << ",\""
        << Material << "\"," << type << ");" << endl;
    // depth
    if (p.hasAttribute(VariableDescription::depth)) {
      const auto d = p.getAttribute<unsigned short>(VariableDescription::depth);
      out << "  setTimeDepth(\"" + p.name + "\"," << d + 1 << ");\n";
    }
  }
}

static void getField(std::ostream& out, const ModelDescription&,
                     const VariableDescriptionContainer& v, const std::string& type) {
  using namespace std;
  VariableDescriptionContainer::const_iterator p;
  for (p = v.begin(); p != v.end(); ++p) {
    // getting the field
    if (type == "Output") {
      out << "  Field<double>& " << p->name << " = get" << type << "Field<double>(\"" << p->name
          << "\");" << endl;
    } else {
      out << "  const Field<double>& " << p->name << " = getField<double>(\"" << p->name << "\");"
          << endl;
    }
    // depth
    if (p->hasAttribute(VariableDescription::depth)) {
      const auto depth = p->getAttribute<unsigned short>(VariableDescription::depth);
      if (depth > 0) {
        for (unsigned short d = 1; d != depth + 1; ++d) {
          out << "  const Field<double>& " << p->name << "_" << d
              << " = getPreviousField<double>(\"" << p->name << "\"," << d << ");" << endl;
        }
      }
    }
  }
...
void PleiadesModelInterface::writeOutputFiles(const FileDescription& fdata,
  ...
  out << "namespace Pleiades\n"
      << "{\n\n";
  out << "/*!" << endl << "  * Implementation of the  " << mdata.className << " model" << endl
      << " */" << endl << "class " << mdata.className << endl << " : public ModelBase {" << endl
      << endl;
  ...
  for (auto& l : mdata.constantMaterialProperties) {
    out << "  " << l.type << " " << l.name << "; // constant material Property\n";
  }
  ...
  for (auto& l : mdata.parameters) {
    const auto hasDefaultValue = l.hasAttribute(VariableDescription::defaultValue);
    if (hasDefaultValue) {
      out << "  declareParameter<" << l.type << ">(\"" << l.name << "\", "
          << getGlossaryName(l.name, mdata) << ", "
          << l.getAttribute<double>(VariableDescription::defaultValue) << ");" << endl;
    } else {
      out << "  declareParameter<" << l.type << ">(\"" << l.name << "\", "
  ...
  for (auto& l : mdata.constantMaterialProperties) {
    ...
    out << "  declareParameter<" << l.type << ">(\"" << l.name << "\", " << FQName << ");" << endl;
  }
  ...
  if (!mdata.parameters.empty()) {
    for (auto& l : mdata.parameters) {
      out << "  this->" << l.name << " = getParameter<" << l.type << ">(\"" << l.name << "\");"
          << endl;
    }
  }
  ...
  for (const auto& l : mdata.outputs) {
    if (l.hasAttribute(VariableDescription::initialValue)) {
      out << "  getOutputField<double>(\"" << l.name << "\").setUniformValue("
          << l.getAttribute<double>(VariableDescription::initialValue) << ");" << endl;
    }
  }
  out << "}\n\n";  // end of initialize
  ...
  for (auto pf = mdata.functions.begin(); pf != mdata.functions.end(); ++pf) {
    ...
    out << "  for (unsigned int n=0; n != " << *(pf->modifiedVariables.begin())
        << ".getNumberOfValues(); ++n) {" << endl;
    out << "    this->" << pf->name << "(";
    for (po = pf->modifiedVariables.begin(); po != pf->modifiedVariables.end();) {
      out << *po << "[n]";
      if (++po != pf->modifiedVariables.end()) {
        out << ",";
      }
    }
    for (pi = pf->usedVariables.begin(); pi != pf->usedVariables.end(); ++pi) {
      out << "," << *pi << "[n]";
    }
...
PleiadesModelInterface::treatKeyword(const std::string& k, const std::vector<std::string>& i,
                                           TokensContainer::const_iterator c,
                                           const TokensContainer::const_iterator) {
  if (std::find(i.begin(), i.end(), "pleiades-2.0") != i.end()) {
    throw(std::runtime_error(
        "PleiadesModelInterface::treatKeyword: "
        "unsupported key '" +
        k + "'"));
  }
  return {false, c};
...
void PleiadesModelInterface::getTargetsDescription(TargetsDescription& td,
