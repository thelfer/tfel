...
namespace castem
{
    ...
    typedef typename 
      std::conditional<N==1,
		     ...
		     typename std::conditional<N==2,
					     ...
					     CastemOrthotropicSmallStrainBehaviourHandler3D<H,Behaviour> >::type
		     >::type type;
  ...
  template<tfel::material::ModellingHypothesis::Hypothesis H,
	   template<tfel::material::ModellingHypothesis::Hypothesis,
		    typename,bool> class Behaviour>
  struct TFEL_VISIBILITY_LOCAL CastemOrthotropicFiniteStrainBehaviourDispatcher
  {
    //! a simple alias
    typedef tfel::material::ModellingHypothesisToSpaceDimension<H> ModellingHypothesisToSpaceDimension;
    // spatial dimension
    ...
    typedef typename 
      std::conditional<N==1,
		     ...
		     typename std::conditional<N==2,
					     ...
					     CastemOrthotropicFiniteStrainBehaviourHandler3D<H,Behaviour> >::type
		     >::type type;
    ...
    : private CastemBehaviourHandler<STANDARDSTRAINBASEDBEHAVIOUR,H,Behaviour>
  {
	       ...
	       const StressFreeExpansionHandler& sfeh)
    {
      ...
      typedef CastemBehaviourHandler<STANDARDSTRAINBASEDBEHAVIOUR,H,Behaviour> CBHandler;
    ...
    : private CastemBehaviourHandler<STANDARDFINITESTRAINBEHAVIOUR,H,Behaviour>
  {
	       ...
	       const StressFreeExpansionHandler& sfeh)
    {
      ...
      typedef MechanicalBehaviourTraits<Behaviour<H,CastemReal,false> > MTraits;
      typedef CastemTraits<Behaviour<H,CastemReal,false> > Traits;
      typedef CastemBehaviourHandler<STANDARDFINITESTRAINBEHAVIOUR,H,Behaviour> CBHandler;
      const bool is_defined_ = MTraits::is_defined;
      const bool bs = Traits::requiresStiffnessTensor;
      const bool ba = Traits::requiresThermalExpansionCoefficientTensor;
      typedef typename std::conditional<
