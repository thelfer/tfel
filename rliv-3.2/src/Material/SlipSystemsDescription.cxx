  ...
  namespace material{
	...
	for(const auto& gg : generateSlipSystems<cs>(gn)){
	  ...
	  raise_if((std::equal(g.burgers.begin(),g.burgers.end(),b.begin()))&&
		   ...
		   "slip system "+to_string(g.burgers,'(',')')+
		   to_string(g.plane,'[',']')+' '+
	...
	raise("SlipSystemsDescription::SlipSystemsDescription: "
	...
	return to_array(s.normal(to_numodis<numodis::IPlane>(p)));
      ...
      raise_if(cs!=CrystalStructure::Cubic,"normal: internal error "
	       "(unsupported crystal structure)");
    ...
    {
      raise_if(cs!=CrystalStructure::HCP,"normal: internal error "
	       "(unsupported crystal structure)");
      auto s = numodis::HCP();
      return to_array(s.normal(to_numodis<numodis::IPlane>(p)));
    }
    ...
    void SlipSystemsDescription::setDislocationsMeanFreePathInteractionMatrix(const std::vector<long double>& v){
      auto throw_if = [](const bool c, const std::string& msg){
	raise_if(c,"SlipSystemsDescription::"
